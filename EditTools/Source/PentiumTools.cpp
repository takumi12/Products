...
void reload_menu_enc()
{
	HMENU hMenu = LoadMenu(hInst, MAKEINTRESOURCE(IDC_PENTIUMTOOLS));
	HMENU hSubMenu = GetSubMenu(hMenu, 1);

	//AppendMenu(hSubMenu, MF_STRING, IDM_RELOAD_SERVER_VERSION, "Reload Server Launcher");
	SetMenu(g_hWnd, hMenu);
}

void reload_menu_dec()
{
	HMENU hMenu = LoadMenu(hInst, MAKEINTRESOURCE(IDC_PENTIUMTOOLS));
	HMENU hSubMenu = GetSubMenu(hMenu, 2);

	//AppendMenu(hSubMenu, MF_STRING, 5555, "Reload Server Launcher");
	SetMenu(g_hWnd, hMenu);
}
