...
IMGUI_API bool ImGui::InputInt64(const char* label, unsigned long* v, int step, int step_fast, ImGuiInputTextFlags flags)
{
    // Hexadecimal input provided as a convenience but the flag name is awkward. Typically you'd use InputText() to parse your own data, if you want to handle prefixes.
    const char* format = (flags & ImGuiInputTextFlags_CharsHexadecimal) ? "%08X" : "%d";
...
bool ImGui::InputShort(const char* label, short* v, int step, int step_fast, ImGuiInputTextFlags flags)
{
    const char* format = (flags & ImGuiInputTextFlags_CharsHexadecimal) ? "%08X" : "%d";
...
bool ImGui::InputNumber(const char* label, int i, void* p_data, const char* format, ImGuiDataType data_type, ImGuiInputTextFlags flags)
{
    char LabelId[100];
    sprintf_s(LabelId, "%s_%d", label, i);
    return InputScalar(LabelId, data_type, p_data, NULL, NULL, format, flags);
}
...
bool ImGui::TreeNodeOpen(const char* label)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
   TreeNodeSetOpen(window->GetID(label), true);
   return true;
}
...
bool ImGui::SelectableHovered(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    bool Hovered = (hovered && !ImGui::IsMouseHoveringRect(bb.Min, bb.Max));
    ...
    if (hovered == false && Hovered == false && g.NavId == id)
    {
        selected = true;
    }
    if ((was_selected && g.NavId != id) && !hovered && !Hovered)
    {
        selected = true;
    }
