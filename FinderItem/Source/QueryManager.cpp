...
bool CQueryManager::Connect(const char* odbc, const char* user, const char* pass) // OK
{
...
bool CQueryManager::ExecQuery(const char* query, ...) // OK
{
	char buff[4096];
	...
	for (int n = 0; n < this->m_ColCount; n++)
	{
		...
		SQLBindCol(this->m_STMT, (n + 1), SQL_C_CHAR, m_SQLData[n], sizeof(m_SQLData[n]), &this->m_SQLDataLen[n]);
	}
...
int CQueryManager::FindIndex(const char* ColName) // OK
{
	for (int n = 0; n < this->m_ColCount; n++)
	{
		if (_stricmp(ColName, (char*)this->m_SQLColName[n]) == 0)
...
int CQueryManager::GetAsInteger(const char* ColName) // OK
{
	int index = this->FindIndex(ColName);
...
float CQueryManager::GetAsFloat(const char* ColName) // OK
{
	int index = this->FindIndex(ColName);
...
__int64 CQueryManager::GetAsInteger64(const char* ColName) // OK
{
	int index = this->FindIndex(ColName);
...
void CQueryManager::GetAsString(const char* ColName, char* OutBuffer, int OutBufferSize) // OK
{
	int index = this->FindIndex(ColName);
...
void CQueryManager::GetAsBinary(char* ColName, BYTE* Buffer, int Size) // OK
{
	int index = this->FindIndex(ColName);
	...
	{
		char SQLData[8192];
		strcpy_s(SQLData, this->m_SQLData[index]);

		this->ConvertStringToBinary(SQLData, sizeof(SQLData), Buffer, Size);
	}
}

void CQueryManager::GetAsBinary(const char* ColName, BYTE* Buffer, int Size) // OK
{
	int index = this->FindIndex(ColName);
