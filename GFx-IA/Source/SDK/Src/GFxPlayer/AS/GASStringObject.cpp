...
GASStringObject::GASStringObject(GASEnvironment* penv)
    : GASObject(penv), Value(penv->GetBuiltin(GASBuiltin_empty_))
{
    commonInit(penv);
}
...
void GASStringProto::StringConcat(const GASFnCall& fn)
{
    ...
    // Use resizable GString buffer (avoid hash re-calculation).
    GStringBuffer retVal(pThis->GetString().ToCStr(), pThis->GetString().GetSize());    
...
GPtr<GASArrayObject> GASStringProto::StringSplit(GASEnvironment* penv, const GASString& str, const char* delimiters, int limit)
{
    GPtr<GASArrayObject> retVal = *static_cast<GASArrayObject*>
                                  (penv->OperatorNew(penv->GetBuiltin(GASBuiltin_Array)));
            ...
            if (c1 == 0)
            {
                ...
                {                
                    if (c2 == 0)
                        ...
                        retVal->PushBack(GASValue(CreateStringFromCStr(psc, start)));
                }
                break;
            }
        }
...
void GASStringProto::StringLocaleCompare(const GASFnCall& fn)
{
...
static const GASNameFunction GAS_StringFunctionTable[] = 
{
    ...
    { "localeCompare",&GASStringProto::StringLocaleCompare}, //extension
...
{
    Value = psc->GetBuiltin(GASBuiltin_typeObject_);
...
GASObject* GASStringCtorFunction::CreateNewObject(GASEnvironment *penv) const 
{ 
    GASObject* o = GHEAP_NEW(penv->GetHeap()) GASStringObject(penv); 
    o->SetMemberRaw(penv->GetSC(), penv->GetBuiltin(GASBuiltin_length), GASValue(GASValue::UNSET));
    return o;
}

void GASStringCtorFunction::GlobalCtor(const GASFnCall& fn)
{
    if (fn.ThisPtr && fn.ThisPtr->GetObjectType() == GASObject::Object_String &&
        !fn.ThisPtr->IsBuiltinPrototype())
    {
        GASStringObject* nobj = static_cast<GASStringObject*>(fn.ThisPtr);
        GASValue retVal = (fn.NArgs > 0)? fn.Arg(0) : GASValue(fn.Env->GetBuiltin(GASBuiltin_empty_));
...
GASFunctionRef GASStringCtorFunction::Register(GASGlobalContext* pgc)
{
    GASStringContext sc(pgc, 8);
    GASFunctionRef  ctor(*GHEAP_NEW(pgc->GetHeap()) GASStringCtorFunction(&sc));
    GPtr<GASObject> proto = *GHEAP_NEW(pgc->GetHeap()) GASStringProto(&sc, pgc->GetPrototype(GASBuiltin_Object), ctor);
    pgc->SetPrototype(GASBuiltin_String, proto);
    pgc->pGlobal->SetMemberRaw(&sc, pgc->GetBuiltin(GASBuiltin_String), GASValue(ctor));
    return ctor;
}
