...
struct CSSToken
{
    ...
    {
        GASSERT( (pBase + Length) == tok.pBase ); // appending only works for contiguous tokens
        Length += tok.Length;
    }
};
...
template <> inline int  CSSTokenizer<wchar_t>::IsSpace(wchar_t c) { return G_iswspace(c); }

template <> inline int  CSSTokenizer<char>::IsDigit(char c) { return isdigit(c); }
template <> inline int  CSSTokenizer<wchar_t>::IsDigit(wchar_t c) { return G_iswdigit(c); }

template <> inline int  CSSTokenizer<char>::IsXDigit(char c) { return isxdigit(c); }
template <> inline int  CSSTokenizer<wchar_t>::IsXDigit(wchar_t c) { return G_iswxdigit(c); }

template <> inline int  CSSTokenizer<char>::IsAlpha(char c) { return isalpha(c); }
template <> inline int  CSSTokenizer<wchar_t>::IsAlpha(wchar_t c) { return G_iswalpha(c); }

template <> inline int  CSSTokenizer<char>::IsAlNum(char c) { return isalnum(c); }
template <> inline int  CSSTokenizer<wchar_t>::IsAlNum(wchar_t c) { return G_iswalnum(c); }
...
private:
    ...
    GArray< CSSToken<T> >   PropertyValue;
...
#ifdef GFC_CSS_VERBOSE_TRACE
    GString                   tokenString;
    GString                   gbuff;
    gbuff.AppendString(buffer, len);
    GFC_DEBUG_MESSAGE(1, "---------------------------");
    GFC_DEBUG_MESSAGE1(1, "Parsing:\n%s\n", gbuff.ToCStr());
#endif
        ...
        case PS_ReadStyleName:
            {
                ...
                case CSSToken<T>::TT_WhiteSpace:   
                    ...
                    {
#ifdef GFC_CSS_VERBOSE_TRACE
                        GFC_DEBUG_MESSAGE(1, "- Error reading style name");
#endif
                        state = PS_Error;
                    }
                    break;
                ...
                case CSSToken<T>::TT_Name:
                    ...
                    GFC_DEBUG_MESSAGE3(1, "- Found selector: '%s' @ %d,%d", 
...
#ifdef GFC_CSS_VERBOSE_TRACE
                        GFC_DEBUG_MESSAGE(1, "- Beginning selector block");
#endif
        ...
        case PS_ReadPropertyName:
            {
...
#ifdef GFC_CSS_VERBOSE_TRACE
                    GFC_DEBUG_MESSAGE(1, "- Closing selector block");
#endif
                ...
                case CSSToken<T>::TT_Name:
                    ...
                    GFC_DEBUG_MESSAGE3(1, "- Found property name: '%s' @ %d,%d", 
                    ...
                    {
#ifdef GFC_CSS_VERBOSE_TRACE
                        GFC_DEBUG_MESSAGE(1, "- Error reading property name");
#endif
                        state = PS_Error;
                    }
                    break;
        ...
        case PS_ReadPropertyValue:
            {
                ...
                case CSSToken<T>::TT_SemiColon:
                    ...
                    GFC_DEBUG_MESSAGE3(1, "- Found property value: '%s' @ %d,%d", 
...
#ifdef GFC_CSS_VERBOSE_TRACE
        tokenString.Clear(); tokenString.AppendString(token.pBase, token.Length);
        GFC_DEBUG_MESSAGE3(1, "   - Encountered '%s' @ %d,%d", tokenString.ToCStr(), 
...
{
protected:
    GFxTextStyleParserHandlerBase(GFxTextStyleManager* manager) : pManager(manager) 
    {
        GASSERT(pManager);
    }

    GFxTextStyle* GetStyle(const GFxTextStyleKey& key)
    {
        GFxTextStyle** ret = pManager->Styles.Get(key);
        return (ret) ? *ret : NULL;
    }
    ...
    const GFxTextStyleKey&  GetTempStyleKey(GFxTextStyleKey::KeyType t, const GString& val)
    {
        return pManager->GetTempStyleKey(t, val);
    }

    GFxTextStyleManager*        GetTextStyleManager() const { return pManager; }
    
private:
    GFxTextStyleManager*        pManager;
};
...
//
template <typename T>
class TextStyleParserHandler : public CSSHandler<T>, public GFxTextStyleParserHandlerBase
{
public:
    TextStyleParserHandler(GFxTextStyleManager* manager) : GFxTextStyleParserHandlerBase(manager) {}
...
private:
    void HandleColor            (const GArray<GFxTextStyle*>& styles, const GArray< CSSToken<T> >& value);
    void HandleDisplay          (const GArray<GFxTextStyle*>& styles, const GArray< CSSToken<T> >& value);
    void HandleFontFamily       (const GArray<GFxTextStyle*>& styles, const GArray< CSSToken<T> >& value);
    void HandleFontSize         (const GArray<GFxTextStyle*>& styles, const GArray< CSSToken<T> >& value);
    void HandleFontStyle        (const GArray<GFxTextStyle*>& styles, const GArray< CSSToken<T> >& value);
    void HandleFontWeight       (const GArray<GFxTextStyle*>& styles, const GArray< CSSToken<T> >& value);
    void HandleKerning          (const GArray<GFxTextStyle*>& styles, const GArray< CSSToken<T> >& value);
    void HandleLetterSpacing    (const GArray<GFxTextStyle*>& styles, const GArray< CSSToken<T> >& value);
    void HandleMarginLeft       (const GArray<GFxTextStyle*>& styles, const GArray< CSSToken<T> >& value);
    void HandleMarginRight      (const GArray<GFxTextStyle*>& styles, const GArray< CSSToken<T> >& value);
    void HandleTextAlign        (const GArray<GFxTextStyle*>& styles, const GArray< CSSToken<T> >& value);
    void HandleTextDecoration   (const GArray<GFxTextStyle*>& styles, const GArray< CSSToken<T> >& value);
    void HandleTextIndent       (const GArray<GFxTextStyle*>& styles, const GArray< CSSToken<T> >& value);
...
{
    return GFxSGMLCharIter<T>::StrCompare(s2, s1, size);
}
...
void TextStyleParserHandler<T>::OpenCSSSelectorBlock(const CSSToken<T>& name, void* pdata)
{
    ...
    GFC_DEBUG_MESSAGE1(1, "[CSS] Opening selector block: '%s'", sname.ToCStr());
#endif

    GString keystr;
    // figure out key type
    GFxTextStyleKey::KeyType type = GFxTextStyleKey::CSS_Tag;
    if (name.Length > 0 && name.pBase[0] == '.')
    {
        type = GFxTextStyleKey::CSS_Class;
    ...
    const GFxTextStyleKey& key = GetTempStyleKey(type, keystr);
    GFxTextStyle* pstyle = GetStyle(key);
    if (!pstyle)
    {
        GMemoryHeap* pheap = GMemory::GetHeapByAddress(GetTextStyleManager());
        pstyle = GHEAP_NEW(pheap) GFxTextStyle(pheap);
        AddStyle(key, pstyle);
    }
    else
    {
    ...
    GArray<GFxTextStyle*>* parr = (GArray<GFxTextStyle*>*)pdata;
    GASSERT(parr);
...
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Closing selector block");
#endif
                                                        ...
                                                        void* pdata)
{
    ...
    if (value.GetSize() > 0)
        ...
        - value[0].pBase);
    GFC_DEBUG_MESSAGE2(1, "[CSS] Found property: '%s' => '%s'", sname.ToCStr(), sval.ToCStr());
#endif

    GArray<GFxTextStyle*>* parr = (GArray<GFxTextStyle*>*)pdata;
    GASSERT(parr);
    GASSERT(parr->GetSize() > 0);
    ...
    if      (sname == "color")              HandleColor(*parr, value);
        ...
        GFC_DEBUG_MESSAGE1(1, "[CSS] Unknown property name '%s'", sname.ToCStr());
#endif
}
...
//
template <typename T>
void TextStyleParserHandler<T>::HandleColor(const GArray<GFxTextStyle*>& styles, 
                                            const GArray< CSSToken<T> >& value)
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Handled property 'color'");
#endif
    ...
    UInt32 colval = 0;
    GFxSGMLParser<T>::ParseHexInt(&colval, value[idx].pBase+1, value[idx].Length-1);
    // assign color value to all active selectors
    for (UPInt i=0; i < styles.GetSize(); i++)
        styles[i]->TextFormat.SetColor32(colval);        
}
...
//
template <typename T>
void TextStyleParserHandler<T>::HandleDisplay(const GArray<GFxTextStyle*>& styles, 
                                              const GArray< CSSToken<T> >& value)
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Handled property 'display'");
#endif
    GUNUSED(styles);
    ...
    GFxTextParagraphFormat::DisplayType display = GFxTextParagraphFormat::Display_Inline;
    // check against supported values
    if ( !StrNCmpChar("inline", ch, (sz < 6) ? sz : 6) ) 
    {
        display = GFxTextParagraphFormat::Display_Inline;
    }
    else if ( !StrNCmpChar("block", ch, (sz < 5) ? sz : 5) )
    {
        display = GFxTextParagraphFormat::Display_Block;
    }
    else if ( !StrNCmpChar("none", ch, (sz < 4) ? sz : 4) )
    {
        display = GFxTextParagraphFormat::Display_None;
    }
    for (UPInt i=0; i < styles.GetSize(); i++)
        styles[i]->ParagraphFormat.SetDisplay(display);        
}
...
//
template <typename T>
void TextStyleParserHandler<T>::HandleFontFamily(const GArray<GFxTextStyle*>& styles, 
                                                 const GArray< CSSToken<T> >& value)
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Handled property 'font-family'");
#endif
    GUNUSED2(styles, value);
    ...
    for (UPInt i=0; i < styles.GetSize(); i++)
        styles[i]->TextFormat.SetFontList(fontstr.ToCStr());
}
...
//
template <typename T>
void TextStyleParserHandler<T>::HandleFontSize(const GArray<GFxTextStyle*>& styles, 
                                               const GArray< CSSToken<T> >& value)
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Handled property 'font-size'");
#endif
    ...
    if (value[idx].Type != CSSToken<T>::TT_Number)
        return;
    Float num = 0;
    GFxSGMLParser<T>::ParseFloat(&num, value[idx].pBase, value[idx].Length);
    // fail if font size is 0
    if (num == 0)
        return;
    // assign value to styles
    for (UPInt i=0; i < styles.GetSize(); i++)
        styles[i]->TextFormat.SetFontSize(num);
}
...
//
template <typename T>
void TextStyleParserHandler<T>::HandleFontStyle(const GArray<GFxTextStyle*>& styles, 
                                                const GArray< CSSToken<T> >& value)
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Handled property 'font-style'");
#endif
    ...
    {
        for (UPInt i=0; i < styles.GetSize(); i++)
            styles[i]->TextFormat.SetItalic(false);        
    }
    else if ( !StrNCmpChar("italic", ch, (sz < 6) ? sz : 6) )
    {
        for (UPInt i=0; i < styles.GetSize(); i++)
            styles[i]->TextFormat.SetItalic(true);        
    }
}
...
//
template <typename T>
void TextStyleParserHandler<T>::HandleFontWeight(const GArray<GFxTextStyle*>& styles, 
                                                 const GArray< CSSToken<T> >& value)
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Handled property 'font-weight'");
#endif
    ...
    {
        for (UPInt i=0; i < styles.GetSize(); i++)
            styles[i]->TextFormat.SetBold(false);        
    }
    else if ( !StrNCmpChar("bold", ch, (sz < 4) ? sz : 4) )
    {
        for (UPInt i=0; i < styles.GetSize(); i++)
            styles[i]->TextFormat.SetBold(true);        
    }
}
...
//
template <typename T>
void TextStyleParserHandler<T>::HandleKerning(const GArray<GFxTextStyle*>& styles, 
                                              const GArray< CSSToken<T> >& value)
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Handled property 'kerning'");
#endif
    ...
    {
        for (UPInt i=0; i < styles.GetSize(); i++)
            styles[i]->TextFormat.SetKerning(true);
    }
    else if ( !StrNCmpChar("false", ch, (sz < 5) ? sz : 5) )
    {
        for (UPInt i=0; i < styles.GetSize(); i++)
            styles[i]->TextFormat.SetKerning(false);
    }
}
...
//
template <typename T>
void TextStyleParserHandler<T>::HandleLetterSpacing(const GArray<GFxTextStyle*>& styles, 
                                                    const GArray< CSSToken<T> >& value)
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Handled property 'letter-spacing'");
#endif
    ...
    // assign value to styles
    for (UPInt i=0; i < styles.GetSize(); i++)
        styles[i]->TextFormat.SetLetterSpacing(num);
}
...
//
template <typename T>
void TextStyleParserHandler<T>::HandleMarginLeft(const GArray<GFxTextStyle*>& styles, 
                                                 const GArray< CSSToken<T> >& value)
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Handled property 'margin-left'");
#endif
    ...
    // assign value to styles
    for (UPInt i=0; i < styles.GetSize(); i++)
        styles[i]->ParagraphFormat.SetLeftMargin((UInt)num);
}
...
//
template <typename T>
void TextStyleParserHandler<T>::HandleMarginRight(const GArray<GFxTextStyle*>& styles, 
                                                  const GArray< CSSToken<T> >& value)
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Handled property 'margin-right'");
#endif
    ...
    // assign value to styles
    for (UPInt i=0; i < styles.GetSize(); i++)
        styles[i]->ParagraphFormat.SetRightMargin((UInt)num);
}
...
//
template <typename T>
void TextStyleParserHandler<T>::HandleTextAlign(const GArray<GFxTextStyle*>& styles, 
                                                const GArray< CSSToken<T> >& value)
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Handled property 'text-align'");
#endif
    ...
    {
        for (UPInt i=0; i < styles.GetSize(); i++)
            styles[i]->ParagraphFormat.SetAlignment( GFxTextParagraphFormat::Align_Left );
    }
    else if ( !StrNCmpChar("center", ch, (sz < 6) ? sz : 6) )
    {
        for (UPInt i=0; i < styles.GetSize(); i++)
            styles[i]->ParagraphFormat.SetAlignment( GFxTextParagraphFormat::Align_Center );
    }
    else if ( !StrNCmpChar("right", ch, (sz < 5) ? sz : 5) )
    {
        for (UPInt i=0; i < styles.GetSize(); i++)
            styles[i]->ParagraphFormat.SetAlignment( GFxTextParagraphFormat::Align_Right );
    }
    else if ( !StrNCmpChar("justify", ch, (sz < 7) ? sz : 7) )
    {
        for (UPInt i=0; i < styles.GetSize(); i++)
            styles[i]->ParagraphFormat.SetAlignment( GFxTextParagraphFormat::Align_Justify );
    }
}
...
//
template <typename T>
void TextStyleParserHandler<T>::HandleTextDecoration(const GArray<GFxTextStyle*>& styles, 
                                                     const GArray< CSSToken<T> >& value)
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Handled property 'text-decoration'");
#endif
    ...
    {
        for (UPInt i=0; i < styles.GetSize(); i++)
            styles[i]->TextFormat.SetUnderline(false);        
    }
    else if ( !StrNCmpChar("underline", ch, (sz < 9) ? sz : 9) )
    {
        for (UPInt i=0; i < styles.GetSize(); i++)
            styles[i]->TextFormat.SetUnderline(true);        
    }
}
...
//
template <typename T>
void TextStyleParserHandler<T>::HandleTextIndent(const GArray<GFxTextStyle*>& styles, 
                                                 const GArray< CSSToken<T> >& value)
{
#ifdef GFC_CSS_VERBOSE_TRACE
    GFC_DEBUG_MESSAGE(1, "[CSS] Handled property 'text-indent'");
#endif
    ...
    // assign value to styles
    for (UPInt i=0; i < styles.GetSize(); i++)
        styles[i]->ParagraphFormat.SetIndent((SInt)num);
}
...
//
void GFxTextStyle::Reset()
{
    TextFormat = GFxTextFormat(TextFormat.GetHeap());
...
// Destructor
//
GFxTextStyleManager::~GFxTextStyleManager()
{
    ClearStyles();
}
...
bool GFxTextStyleManager::ParseCSS(const char* buffer, UPInt len)
{
    ...
    wchar_t* pwbuf = (wchar_t*)GALLOC((len + 1) * sizeof(wchar_t), GFxStatMV_Text_Mem);
    len = GUTF8Util::DecodeString(pwbuf, buffer, len);
    bool rv = ParseCSSImpl<wchar_t>(pwbuf, len);
    GFREE(pwbuf);
    return rv;
}

bool GFxTextStyleManager::ParseCSS(const wchar_t* buffer, UPInt len)
{
...
//
const GFxTextStyle* GFxTextStyleManager::GetStyle(GFxTextStyleKey::KeyType type, const GString& name) const
{
    GFxTextStyle* const* ret = Styles.Get(GetTempStyleKey(type, name));
    return (ret) ? *ret : NULL;
}

const GFxTextStyle* GFxTextStyleManager::GetStyle(GFxTextStyleKey::KeyType type, const char* name, UPInt len) const
{
    if (len == GFC_MAX_UPINT)
        len = G_strlen(name);
    GString str(name, len);
    return GetStyle(type, str);
}
const GFxTextStyle* GFxTextStyleManager::GetStyle(GFxTextStyleKey::KeyType type, const wchar_t* name, UPInt len) const
{
    if (len == GFC_MAX_UPINT)
        len = G_wcslen(name);
    GString str;
...
// Return the styles
//
const GFxTextStyleHash& GFxTextStyleManager::GetStyles() const
{
    return Styles;
}
...
//
void GFxTextStyleManager::AddStyle(const GFxTextStyleKey& key, GFxTextStyle* pstyle)
{
...
//
void GFxTextStyleManager::ClearStyle(GFxTextStyleKey::KeyType type, const GString& name)
{
    const GFxTextStyleKey & key = GetTempStyleKey(type, name);
    GFxTextStyle** style = Styles.Get(key);
    if (style)
    {
        delete *style;
        Styles.Remove(key);
...
void GFxTextStyleManager::ClearStyle(GFxTextStyleKey::KeyType type, const char* name, UPInt len /* = GFC_MAX_UPINT */)
{
    if (len == GFC_MAX_UPINT)
        len = G_strlen(name);
    GString str(name, len);
...
//
void GFxTextStyleManager::ClearStyles()
{
    for (GFxTextStyleHash::Iterator iter = Styles.Begin(); iter != Styles.End(); ++iter)
        delete iter->Second;
    Styles.Clear();    
}
