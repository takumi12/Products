...
// GFxTextDocView
//
GFxTextDocView::GFxTextDocView(GFxTextAllocator* pallocator, GFxFontManager* pfontMgr, GFxLog* plog)
    : pFontManager(pfontMgr), pHighlight(NULL), pLog(plog)
{
    pDocument = *GHEAP_AUTO_NEW(this) DocumentText(this, pallocator);
    FormatCounter = 1;
...
struct GFxImageSubstCmp
{
    ...
    static int InsLess(const GFxTextDocView::ImageSubstitutor::Element& p1, const Comparable& p2)
    {
        return (-StrCompare(p2.Str, p2.MaxSize, p1.SubString, p1.SubStringLen, true)) < 0;
    }
    static int Less(const GFxTextDocView::ImageSubstitutor::Element& p1, const Comparable& p2)
    {
        return (-StrCompare(p2.Str, p2.MaxSize, p1.SubString, p1.SubStringLen, false)) < 0;
    }
    static int Equals(const GFxTextDocView::ImageSubstitutor::Element& p1, const Comparable& p2)
    {
        return StrCompare(p2.Str, p2.MaxSize, p1.SubString, p1.SubStringLen, false) == 0;
    }
};
...
void GFxTextDocView::ImageSubstitutor::AddImageDesc(const GFxTextDocView::ImageSubstitutor::Element& elem)
{
    ...
    UPInt i = G_LowerBound(Elements, e, GFxImageSubstCmp::InsLess);
...
GFxTextImageDesc* GFxTextDocView::ImageSubstitutor::FindImageDesc(const wchar_t* pstr, UPInt  maxlen, UPInt * ptextLen)
{
    ...
    UPInt i = G_LowerBound(Elements, e, GFxImageSubstCmp::Less);
    if (i < Elements.GetSize() && GFxImageSubstCmp::Equals(Elements[i], e))
    {
...
UPInt GFxTextDocView::EditCommand(GFxTextDocView::CommandType cmdId, const void* command)
{
    ...
    case Cmd_ReplaceTextByChar:
        {
            ...
            {
                UPInt curDstLen = GetLength();
                UPInt ep = G_Clamp(endPos, UPInt(0), curDstLen), 
                      sp = G_Clamp(stPos, UPInt(0), curDstLen);
                curDstLen -= (ep - sp);   
    ...
    return res;
#else
    GUNUSED2(cmdId, command);
    return 0;
#endif //GFC_NO_TEXT_INPUT_SUPPORT
}

GString GFxTextDocView::GetText() const
{
    return pDocument->GetText();
}

GString GFxTextDocView::GetHtml() const
{
                               ...
                               const GFxTextFormat* txtFmt, const GFxTextParagraphFormat* paraFmt)
{
    ...
    if (utf8Len == GFC_MAX_UPINT)
        utf8Len = G_strlen(putf8Str); // actually, this is the SIZE, not length. TODO
...
void GFxTextDocView::AppendHtml(const wchar_t* pwStr, UPInt strLen, bool condenseWhite, GFxStyledText::HTMLImageTagInfoArray* pimgInfoArr)
{
    if (strLen == GFC_MAX_UPINT)
        strLen = G_wcslen(pwStr); 
...
void GFxTextDocView::AppendHtml(const char* putf8Str, UPInt utf8Len, bool condenseWhite, GFxStyledText::HTMLImageTagInfoArray* pimgInfoArr)
{
    if (utf8Len == GFC_MAX_UPINT)
        utf8Len = G_strlen(putf8Str); // actually, this is the SIZE, not length. TODO
...
void GFxTextDocView::SetViewRect(const GRectF& rect, UseType ut)
{
    if (rect != ViewRect)
    {
        ...
        if (ut == UseExternally)
        {
            ...
            UInt newW = (UInt)ViewRect.Width();
            UInt newH = (UInt)ViewRect.Height();
            if (GetTextAutoSize() != TAS_None ||
                ...
                (newW < oldW && LineBuffer.Geom.IsNoClipping()) ||
                ...
                (newH < oldH && LineBuffer.Geom.IsNoClipping()) 
                )))
            {
...
GFxFontHandle* GFxTextDocView::FindFont(FindFontInfo* pfontInfo, bool quietMode)
{
    GASSERT(pfontInfo);
            ...
            else
            {
                ...
                if (!pfontInfo->pCurrentFont)
                {
                    ...
                    if (!quietMode && !(RTFlags & RTFlags_FontErrorDetected) && pLog )
                    {
                        ...
                        GPtr<GFxFontHandle> h = *pfontMgr->CreateFontHandle(pfontInfo->pCurrentFormat->GetFontList(), 
                            pfontInfo->pCurrentFormat->IsBold(), 
                            pfontInfo->pCurrentFormat->IsItalic(),
                            DoesUseDeviceFont(), !pfontInfo->pCurrentFormat->IsSingleFontNameSet(), &searchInfo);
                        GString charPath = GetDocumentListener()? GetDocumentListener()->GetCharacterPath(): GString();
...
public:
    ...
    { 
        ...
        if (pComposStr && pComposStr->GetLength() > 0)
        {
                ...
                curTextPos + n >= pComposStr->GetPosition())
            {
                ...
                newN = G_PMin(pComposStr->GetPosition() - curTextPos, (UPInt)n);
    ...
    void TrackFontParams(GFxFontResource* pfont, Float scale)
    {
        ...
        MaxFontAscent  = G_Max(MaxFontAscent,  ascent * scale);
        MaxFontDescent = G_Max(MaxFontDescent, descent * scale);
        MaxFontLeading = G_Max(MaxFontLeading, pfont->GetLeading() * scale);
    }
};
...
GFxParagraphFormatter::GFxParagraphFormatter(GFxTextDocView* pdoc, GFxLog* plog) 
...
void GFxParagraphFormatter::InitParagraph(const GFxTextParagraph& paragraph)
{
    ...
    {
        pTempLine = ::new (TempLineBuff) GFxTextLineBuffer::Line();
...
void GFxParagraphFormatter::InitCustomWordWrapping()
{
    ...
    if (HasLineFormatHandler)
    {
        ...
        if (composStrInsideThePara && LineCursor.ComposStrLength > 0)
        {
            ...
            GASSERT((SPInt)composStrParaStartIdx >= 0);
...
void GFxParagraphFormatter::FinalizeLine()
{
    ...
    SInt lineHeight = RoundTwips(LineCursor.MaxFontAscent + LineCursor.MaxFontDescent);
    SInt leading = RoundTwips(fleading);

    LineCursor.LineWidth                      = G_Max(0, LineCursor.LineWidth);
    LineCursor.LineWidthWithoutTrailingSpaces = G_Max(0, LineCursor.LineWidthWithoutTrailingSpaces);
    ...
    case GFxTextParagraphFormat::Align_Right:   
        ...
        pcurLine->SetOffsetX(G_Max(0, RoundTwips(TextRectWidth) - LineCursor.LineWidthWithoutTrailingSpaces));
    ...
    case GFxTextParagraphFormat::Align_Center:  
        {
            ...
            pcurLine->SetOffsetX (G_Max(0, RoundTwips(adjTextRect / 2 - 
    ...
    ParaWidth    = G_Max(ParaWidth, lineOffsetX + LineCursor.LineWidth);
    ParaHeight   = NextOffsetY + lineHeight - ParaYOffset;
    NextOffsetY  += RoundTwips(lineHeight + leading);
...
bool GFxParagraphFormatter::HandleCustomWordWrap()
{
    ...
    if (Pass == 1 && HasLineFormatHandler)
    {
        ...
        switch(pParaFormat->GetAlignment())
        {
        case GFxTextParagraphFormat::Align_Left:   
            desc.Alignment = GFxTextDocView::LineFormatDesc::Align_Left;
            break;
        case GFxTextParagraphFormat::Align_Right:   
            desc.Alignment = GFxTextDocView::LineFormatDesc::Align_Right;
            break;
        case GFxTextParagraphFormat::Align_Center:   
            desc.Alignment = GFxTextDocView::LineFormatDesc::Align_Center;
            break;
        case GFxTextParagraphFormat::Align_Justify:   
            desc.Alignment = GFxTextDocView::LineFormatDesc::Align_Justify;
            break;
        default:;
        }
    ...
    if (HyphenationRequested)
    {
        ...
        SInt  dashGlyphWidth   = AlignTwipsToPixel(G_Max(
...
void GFxParagraphFormatter::Format(const GFxTextParagraph& paragraph)
{
    ...
    if (pParaFormat->IsBullet())
    {
        ...
        if (FindFontInfo.pCurrentFormat)
        {
            ...
            // is empty.
            GPtr<GFxFontHandle> pfontHandle = pDocView->FindFont(&FindFontInfo, false);
            GASSERT(pfontHandle);
        ...
        LineCursor.LeftMargin   = PixelsToTwips(GFX_BULLET_PARA_INDENT) + PixelsToTwips(GFX_BULLET_CHAR_ADVANCE);
        LineCursor.LineWidth    = LineCursor.LeftMargin;
        LineCursor.Indent       = -PixelsToTwips(GFX_BULLET_CHAR_ADVANCE);
    }
    else
    ...
    for(;!LineCursor.IsFinished(); LineCursor += DeltaText)
    {
        ...
        if (Pass == 1 && pDocView->IsWordWrap() && !pimgDesc)
        {
            ...
            if (GFxWWHelper::IsLineBreakOpportunityAt(GFxWWHelper::WWT_All, lastChar, chInfo.Character))
            {
        ...
        if (!pimgDesc)
        {
            ...
            isSpace = (chInfo.Character == 0 || (!isNbsp && GFxWWHelper::IsWhiteSpaceChar(chInfo.Character)));
            ...
            else if (chInfo.Character != '\t') // tab symbol does not need a glyph
            {
                ...
                if (GlyphIndex == -1)
                {
                    ...
                    if (pLog && !(pDocView->RTFlags & GFxTextDocView::RTFlags_FontErrorDetected))
                    {
                        ...
                        GString charPath   = pDocView->GetDocumentListener()? pDocView->GetDocumentListener()->GetCharacterPath(): GString();
                        GString charRanges = pFont->GetFont()->GetCharRanges();
                        UInt glyphsCount   = pFont->GetFont()->GetGlyphShapeCount();
            ...
            //GRectF glyphBounds;
            //GlyphWidth   = AlignTwipsToPixel(G_Max(
        ...
        if (pimgDesc)
        {
            ...
            LineCursor.MaxFontAscent  = G_Max(LineCursor.MaxFontAscent,  -p.y);
            LineCursor.MaxFontDescent = G_Max(LineCursor.MaxFontDescent, pimgDesc->GetScreenHeight() - -p.y);
        ...
        if (chInfo.Character == '\t')
        {
            ...
            else
            {
                ...
                Float tabStopSize = PixelsToTwips(floor((FontSize*2 + 8)/8)*8);
...
void GFxTextDocView::Format()
{
    ...
    while(!paraIter.IsFinished())
    {
        ...
        if (!linesIt.IsFinished())
        {
                ...
                ppara->GetModCounter() == curLine.GetParagraphModId())
            {
                ...
                for(; !linesIt.IsFinished(); ++linesIt)
                {
                    ...
                    textWidth  =  G_Max(textWidth, curLine.GetWidth());
                    ...
                    if (IsAutoSizeX())
                    {
                        ...
                        case GFxTextLineBuffer::Line::Align_Right:
                            curLine.SetOffsetX(G_Max(0, RoundTwips(GetTextRect().Width()) - curLine.GetWidth()));
                        ...
                        case GFxTextLineBuffer::Line::Align_Center:
                            curLine.SetOffsetX(G_Max(0, RoundTwips(GetTextRect().Width() / 2) - curLine.GetWidth() / 2));
        ...
        textWidth  =  G_Max(textWidth, formatter.ParaWidth);
    ...
    if (!linesIt.IsFinished())
    {
        linesIt.Remove(LineBuffer.GetSize() - linesIt.GetIndex());
    ...
    { 
        ...
        if (tas == TAS_Shrink)
        {
            ...
            scale = G_Min(vscale, hscale);
        }
        else if (tas == TAS_Fit)
        {
            // need either expand or shrink the text to fill the text rect
            hscale = vscale = GFC_MAX_FLOAT;
            if (textWidth >= 20)
                hscale = maxTextRect.Width()/textWidth;
            if (textHeight >= 20)
                vscale = maxTextRect.Height()/textHeight;
            scale = G_Min(vscale, hscale);
            if (scale == GFC_MAX_FLOAT)
    ...
    if (!valignApplied && valign != VAlign_None && !IsAutoSizeY())
    {
        ...
        if (textHeight < maxTextRect.Height())
        {
            ...
            case VAlign_Top:
                ...
                break;
            case VAlign_Center:
                newTextRect.Top = maxTextRect.Top + (maxTextRect.Height()/2 - textHeight/2);
                break;
            case VAlign_Bottom:
    ...
    {
        LineBuffer.Geom.SetNoClipping();
    }
    else
        LineBuffer.Geom.ClearNoClipping();
    ...
    ClearBitmapFontFlag();
    if (LineBuffer.Lines.GetSize() > 0 && LineBuffer.Lines[0]->GetNumGlyphs() > 0) 
    { 
        ...
        if (LineBuffer.Lines[0]->GetGlyphs()->HasFmtFont())
        {
            GFxTextLineBuffer::GFxFormatDataEntry* pfmtData = LineBuffer.Lines[0]->GetFormatData(); 
            GASSERT(pfmtData->pFont != 0); 
                                ...
                                bool nextFrame)
{
    ...
    param.TextParam.SetOutline(GetOutline());
...
UInt GFxTextDocView::GetMaxHScrollValue() const
{
    if (IsWordWrap())
        return 0;
    SInt editorDelta = (!IsReadOnly()) ? GFX_EDIT_HSCROLL_DELTA : 0;
    UInt v = UInt(G_Max(0.f, ((TextWidth > 0) ? Float(TextWidth) : 0) - 
    ...
    if (!MaxVScroll.IsValid(FormatCounter))
    {
        ...
        else
        {
            ...
            if (!linesIt.IsFinished())
            {
                ...
                MaxVScroll.SetValue(LineBuffer.GetSize() - l, FormatCounter);
            }
            else
                MaxVScroll.SetValue(0, FormatCounter);
        }
...
bool GFxTextDocView::IsReadOnly() const 
{ 
...
{
    if (lineIndex >= LineBuffer.GetSize()) 
        return GFC_MAX_UPINT;
    ForceReformat();
    //?x -= LineBuffer.Geom.VisibleRect.Left - ViewRect.Left;
...
UPInt GFxTextDocView::GetCursorPosAtPoint(Float x, Float y)
{
    ...
    if (it.IsFinished())
    {
        if (y <= 0)
            ...
            it = LineBuffer.Begin() + LineBuffer.GetSize() - 1;
    }
    if (!it.IsFinished())
    {
        return GetCursorPosInLineByOffset(it.GetIndex(), x);
    }
    return (LineBuffer.GetSize() == 0) ? 0 : GFC_MAX_UPINT;
}
    ...
    if (!it.IsFinished())
    {
        ...
        for(UInt i = 0; !git.IsFinished(); ++git, ++i)
        {
            ...
            if (i == indexInLine)
            {
                ...
                (*pCharRect) += GPointF(Float(advance) + GFX_TEXT_GUTTER, Float(line.GetOffsetY()));
                break;
            }
        ...
        for(UInt i = 0; !git.IsFinished(); ++git, ++i)
        {
            ...
            if (i == indexInLine)
            {
                ...
                else
                {
                    ...
                    rv = true;
                    GFxTextImageDesc* pimg = git.GetImage();
                    GASSERT(pimg);
                    pCharRect->Left = -pimg->GetBaseLineX();
                    pCharRect->Top  = -pimg->GetBaseLineY();
                    pCharRect->Right  = pCharRect->Left + pimg->GetScreenWidth();
                    pCharRect->Bottom = pCharRect->Top  + pimg->GetScreenHeight();
                    (*pCharRect) += GPointF(Float(advance) + GFX_TEXT_GUTTER, line.GetOffsetY() + GFX_TEXT_GUTTER);
                }
                break;
            }
            advance += glyph.GetAdvance();
...
{
    if (newBottomMostLine >= LineBuffer.GetSize())
        newBottomMostLine = LineBuffer.GetSize() - 1;
...
#ifndef GFC_NO_TEXT_INPUT_SUPPORT
    if (!pEditorKit)
        pEditorKit = *GHEAP_AUTO_NEW(this) GFxTextEditorKit(this);
#endif //GFC_NO_TEXT_INPUT_SUPPORT
    return pEditorKit;
}
...
{
    GUNUSED4(context, mat, cx, document);
#ifndef GFC_NO_TEXT_INPUT_SUPPORT
        ...
        FormatCounter != document.FormatCounter))
    {
        ...
        HighlightManager.DrawBackground(BackgroundDrawing,
                                        document.LineBuffer,
                                        document.GetTextRect(),
                                        firstVisibleChar,
                                        HScrollOffset,
                                        VScrollOffset);
    }
...
{
    if (!pHighlight)
        pHighlight = GHEAP_AUTO_NEW(this) HighlightDesc;
