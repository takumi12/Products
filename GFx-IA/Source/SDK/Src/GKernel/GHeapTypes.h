/**********************************************************************

Filename    :   GHeapTypes.h
Content     :   Types, defines, and constants
            :   
Created     :   July 14, 2008
Authors     :   Maxim Shemanarev, Boris Rayskiy, Michael Antonov

Copyright   :   (c) 2008 Scaleform Corp. All Rights Reserved.

Licensees may use this file in accordance with the valid Scaleform
Commercial License Agreement provided with the software.

This file is provided AS IS with NO WARRANTY OF ANY KIND, INCLUDING 
THE WARRANTY OF DESIGN, MERCHANTABILITY AND FITNESS FOR ANY PURPOSE.

**********************************************************************/

#ifndef INC_GHeapTypes_H
#define INC_GHeapTypes_H

#include <string.h>      // memset, memcpy, etc
#include "GTypes.h"
#include "GList.h"


#ifndef __PLACEMENT_NEW_INLINE
#define __PLACEMENT_NEW_INLINE

#if defined(GFC_CC_MSVC) || defined(GFC_OS_SYMBIAN)
GINLINE void*   operator new        (UPInt n, void *ptr)   { return ptr; GUNUSED(n); }
GINLINE void    operator delete     (void *ptr, void *ptr2) { return; GUNUSED2(ptr,ptr2); }
#else
// Needed for placement on many platforms including PSP.
#include <new>
#endif

#endif // __PLACEMENT_NEW_INLINE


#ifdef GFC_BUILD_DEBUG
#include <stdio.h>
#endif

// Heap assertions are defined separately to improve debug performance
// and to allow debug checks in Release configuration. 
//------------------------------------------------------------------------
#define GHEAP_ASSERT(a) GASSERT(a)

//------------------------------------------------------------------------
#ifdef GFC_64BIT_POINTERS
typedef UInt32 UPIntHalf;
typedef UInt16 UPIntQuarter;
#else
typedef UInt16 UPIntHalf;
typedef UInt8  UPIntQuarter;
#endif


// ***** Page Size Breakdowns

//  32 Bit System:
//
//  [Level]  [Table Entries]  [Bits]  [Bytes]  [Table Size]
//                            +12
//  Lv1      256              +8      1M        256*8 = 2K (*)
//  Lv2      4096             +12     4G       4096*8 = 32K
//                           = 32
//
//  64 Bit System:
//
//  [Level]  [Table Entries]  [Bits]  [Bytes]  [Table Size]
//                             +12
//   Lv1      256              +8      1M        256*16  = 4K (*)
//   Lv2      256              +8      256M      256*16  = 4k (*)
//   Lv3      4096             +12     256M*4K  4096*16 = 64k (*)
//   Lv4      4096             +12     256M*16M 4096*16 = 64k (*)
//   Lv5      4096             +12     16E      4096*16 = 64k
//
// (*) Dynamically allocated pages; these must all have the same size.


//------------------------------------------------------------------------
class GMemoryHeap;
class GMemoryHeapPT;
struct GHeapSegment : GListNode<GHeapSegment>
{
    UPInt           SelfSize;   // Size of GHeapSegment in bytes
    UPIntHalf       SegType;    // See GHeap_Segment...
    UPIntHalf       Alignment;  // Alignment for direct system allocs only
    UPInt           UseCount;   // Number of allocs to detect empty segments
    GMemoryHeapPT*  pHeap;      // Pointer to the heap itself
    UPInt           DataSize;   // The payload size in bytes
    UByte*          pData;      // Pointer to the data payload
};

//------------------------------------------------------------------------
enum GHeapConstants
{
    // Allocator flags. 
    //--------------------------------------------------------------------
    GHeap_AllowTinyBlocks           = 0x10,
    GHeap_AllowDynaSize             = 0x20,

    // Starter and DebugAlloc granularity constants.
    //--------------------------------------------------------------------
#ifdef GHEAP_DEBUG_INFO

#ifdef GFC_64BIT_POINTERS
    GHeap_StarterGranularity        = 64*1024,
    GHeap_StarterHeaderPageSize     = 8*1024,

    GHeap_DebugAllocGranularity     = 512*1024,
    GHeap_DebugAllocHeaderPageSize  = 8*1024,
    GHeap_DebugAllocPoolSize        = 4096,
#else  // GFC_64BIT_POINTERS
    GHeap_StarterGranularity        = 32*1024,
    GHeap_StarterHeaderPageSize     = 4*1024,

    GHeap_DebugAllocGranularity     = 256*1024,
    GHeap_DebugAllocHeaderPageSize  = 4*1024,
    GHeap_DebugAllocPoolSize        = 2048,
#endif // GFC_64BIT_POINTERS

#else  // GHEAP_DEBUG_INFO

#ifdef GFC_64BIT_POINTERS
    GHeap_StarterGranularity        = 32*1024,
    GHeap_StarterHeaderPageSize     = 8*1024,
#else
    GHeap_StarterGranularity        = 16*1024,
    GHeap_StarterHeaderPageSize     = 4*1024,
#endif

#endif // GHEAP_DEBUG_INFO


    // Granulator and Bookkeeper constants.
    //--------------------------------------------------------------------
#ifdef GFC_64BIT_POINTERS
    GHeap_GranulatorHeaderPageSize  = 8*1024,
    GHeap_BookkeeperGranularity     = 32*1024,
#else
    GHeap_GranulatorHeaderPageSize  = 4*1024,
    GHeap_BookkeeperGranularity     = 16*1024,
#endif


#ifndef GFC_64BIT_POINTERS

    //--------------------------------------------------------------------
    GHeap_Lv1_PageShift     = 12,
    GHeap_Lv2_PageShift     = 20,
    GHeap_Lv3_PageShift     = 32,

    GHeap_Lv1_TableSize     = 1 << (GHeap_Lv2_PageShift - GHeap_Lv1_PageShift),
    GHeap_Lv2_TableSize     = 1 << (GHeap_Lv3_PageShift - GHeap_Lv2_PageShift),
    GHeap_Lv2_PageSize      = 1 <<  GHeap_Lv2_PageShift,   
    GHeap_Lv2_PageMask      =       GHeap_Lv2_PageSize-1,

    GHeap_Root_TableSize    = GHeap_Lv2_TableSize,
    GHeap_Root_PageShift    = GHeap_Lv2_PageShift,

#else

    // 64-bit tables support.
    //--------------------------------------------------------------------
    GHeap_Lv1_PageShift     = 12,
    GHeap_Lv2_PageShift     = 20,
    GHeap_Lv3_PageShift     = 28,
    GHeap_Lv4_PageShift     = 40,
    GHeap_Lv5_PageShift     = 52,
    GHeap_Lv6_PageShift     = 64,

    GHeap_Lv1_TableSize     = 1 << (GHeap_Lv2_PageShift - GHeap_Lv1_PageShift),
    GHeap_Lv2_TableSize     = 1 << (GHeap_Lv3_PageShift - GHeap_Lv2_PageShift),
    GHeap_Lv3_TableSize     = 1 << (GHeap_Lv4_PageShift - GHeap_Lv3_PageShift),
    GHeap_Lv4_TableSize     = 1 << (GHeap_Lv5_PageShift - GHeap_Lv4_PageShift),
    GHeap_Lv5_TableSize     = 1 << (GHeap_Lv6_PageShift - GHeap_Lv5_PageShift),
  
    GHeap_Root_TableSize    = GHeap_Lv5_TableSize,
    GHeap_Root_PageShift    = GHeap_Lv5_PageShift,

#endif


    // System alignment size. Must be equal to GHeap_Lv1_PageSize and 
    // not less than the minimal system page alignment
    //--------------------------------------------------------------------
    GHeap_PageShift         = GHeap_Lv1_PageShift,
    GHeap_PageSize          = 1 << GHeap_PageShift,
    GHeap_PageMask          =      GHeap_PageSize-1,

    // Segment types. Must be conformed with GHeap_TinyBinSize.
    //--------------------------------------------------------------------
    GHeap_SegmentTiny1      = 0,  // Tiny blocks segment of size 1*MinAlignSize
    GHeap_SegmentTiny2      = 1,  // Tiny blocks segment of size 2*MinAlignSize
    GHeap_SegmentTiny3      = 2,  // etc...
    GHeap_SegmentTiny4      = 3,
    GHeap_SegmentTiny5      = 4,
    GHeap_SegmentTiny6      = 5,
    GHeap_SegmentTiny7      = 6,
    GHeap_SegmentTiny8      = 7,
    GHeap_SegmentBookkeeper = 8,  // Reserved for the bookkeeper
    GHeap_SegmentSystem     = 9,  // Direct system allocation
    GHeap_SegmentBitSet     = 10, // BitSet allocator segment
    GHeap_SegmentUndefined  = 16, // Undefined segment. Used in VisitMem() only.

    // Minimal possible alignment and size of allocations and empty 
    // blocks in the allocation engine.
    // GHeap_MinSize value MUST NOT be less than size of 4 pointers.
    // Actual alignment can be less than GHeap_MinSize, but it's impossible
    // to allocate memory blocks of less than GHeap_MinSize.
    // The bookkeeper allocates memory with the GHeap_MinSize alignment.
    //--------------------------------------------------------------------
#ifndef GFC_64BIT_POINTERS
    GHeap_MinShift          = 4,
#else
    GHeap_MinShift          = 5,
#endif
    GHeap_MinSize           = 1 << GHeap_MinShift,
    GHeap_MinMask           =      GHeap_MinSize-1,

    // The number of tiny block sizes: 
    // 1*MinAlignSize...GHeap_TinyBinSize*MinAlignSize. 
    // Currently must be 8.
    //--------------------------------------------------------------------
    GHeap_TinyBinSize      = 8,


    GHeap_DummyValue
};


// ***** GHeapMemVisitor
//
// Interface for visiting memory zones. See VisitMem(). It's a callback
// that allows you to see the internals. Mostly it's free zones, because
// the allocator in general cannot track the allocated (busy) zones. 
// Note the categories. The memory in different categories may overlap,
// for example, first there comes Cat_AllocBitSet for the whole segment size,
// then, Cat_AllocBitSetFree for every free zone. In general if the visitor
// just draws pixels in a bitmap, with colors that correspond to categories,
// you eventually will obtain a correct picture.
// Categories Cat_SysAlloc and Cat_SysAllocFree depend on the implementation
// of GSysAllocPaged. In particular, the call to "Visit" may do nothing, so,
// no Cat_SysAlloc and Cat_SysAllocFree events will occur at all. In your
// implementation of the visitor, you can also ignore these events.
//
// Function Visit():
// Parameters "address", "size", and "cat" are obvious.
// Parameter "seg" can be ignored, but it may help to collect and "compact"
// the segments when they are "far away" from each other. In other words,
// the "seg" is the same for all Cat_...Free events within one segment.
// And of course, it allows you to obtain more information, in particular,
// the heap, the visiting area belongs to.
// seg->pHeap is NULL for Cat_SysAlloc, Cat_SysAllocFree, Cat_Starter,
// Cat_Bookkeeper, and Cat_BookkeeperFree.
//
// Visiting flags are used in some of functions VisitMem, in particular,
// in GHeapEngine::VisitMem and they determine how detailed the information
// should be. VF_Heap means to visit heap segments only, VF_HeapDetailed
// includes the full traversal of the free zones. The same is concerning
// VF_Bookkeeper and VF_BookkeeperDetailed.
//------------------------------------------------------------------------
class GHeapMemVisitor
{
public:
    enum Category
    {
        Cat_SysAlloc,       // System areas. Depend on the SysAlloc implementation.
        Cat_SysAllocFree,   // Free zones within the system areas.
        Cat_Starter,        // Starter segments.
        Cat_StarterFree,    // Free zones within the starter segments.
        Cat_Bookkeeper,     // Bookkeeper segments.
        Cat_BookkeeperFree, // Free zones within the bookkeeper segments.
        Cat_SystemDirect,   // Segments allocated directly from the system.
        Cat_AllocBitSet,    // Bit-set segments (payload only.)
        Cat_AllocBitSetFree,// Free zones within bit-set segments.
        Cat_AllocTiny,      // Tiny blocks.
        Cat_AllocTinyFree,  // Free zones in tiny segments.
        Cat_DebugInfo,      // Debug Info
        Cat_DebugInfoFree,
        Cat_DebugPagePool,  // Debug Info detailed categories
        Cat_DebugPageFree,
        Cat_DebugDataPool,
        Cat_DebugDataBusy,
        Cat_DebugDataFree,
        Cat_Other           // Other segments (DLMalloc, and so on.)
    };

    enum VisitingFlags
    {
        VF_SysAlloc           = 0x01,
        VF_Starter            = 0x02,
        VF_Bookkeeper         = 0x04,
        VF_BookkeeperFree     = 0x08,
        VF_BookkeeperDetailed = VF_Bookkeeper | VF_BookkeeperFree,
        VF_Heap               = 0x10,
        VF_HeapFree           = 0x20,
        VF_HeapDetailed       = VF_Heap | VF_HeapFree,
        VF_DebugInfo          = 0x40,
        VF_DebugInfoDetailed  = 0x80 | VF_DebugInfo,
        VF_All                = 0xFFFFFFFF
    };
    virtual ~GHeapMemVisitor() {}
    virtual void Visit(const GHeapSegment* seg, UPInt address, UPInt size, Category cat) = 0;
};



//--------------------------------------------------------------------
class GHeapSegVisitor
{
public:
    enum Seg_Category
    {
        Seg_SysMem              = 1, // The order is important.
        Seg_PageMap             = 2,
        Seg_Bookkeeper          = 3,
        Seg_DebugInfo           = 4,
        Seg_Heap                = 5,
        Seg_UnusedMask          = 0x80,
        Seg_SysMemUnused        = Seg_UnusedMask | Seg_SysMem,
        Seg_PageMapUnused       = Seg_UnusedMask | Seg_PageMap,
        Seg_BookkeeperUnused    = Seg_UnusedMask | Seg_Bookkeeper,
        Seg_DebugInfoUnused     = Seg_UnusedMask | Seg_DebugInfo,
        Seg_HeapUnused          = Seg_UnusedMask | Seg_Heap
    };

    virtual ~GHeapSegVisitor() { }

    // Called for each segment in all heaps, plus for additional 
    // bookkeeping, page mapping and debug info segments. Heap
    // is NULL for all the bookkeeping segments. If Seg_SysAlloc 
    // category is present, all other categories overlap SysAlloc
    // segments. There can be some unused space in SysAlloc.
    virtual void Visit(UInt cat, const GMemoryHeap* heap, 
                       UPInt addr, UPInt size) = 0;
};


//------------------------------------------------------------------------
extern const UByte GHeapUpperBit[256];
extern const UByte GHeapLowerBit[256];

//------------------------------------------------------------------------
GINLINE UByte GHeapGetUpperBit(UPInt val)
{
#ifndef GFC_64BIT_POINTERS

    if (val & 0xFFFF0000)
    {
        return (val & 0xFF000000) ? 
            GHeapUpperBit[(val >> 24)       ] + 24: 
            GHeapUpperBit[(val >> 16) & 0xFF] + 16;
    }
    return (val & 0xFF00) ?
        GHeapUpperBit[(val >> 8) & 0xFF] + 8:
        GHeapUpperBit[(val     ) & 0xFF];

#else

    if (val & 0xFFFFFFFF00000000)
    {
        if (val & 0xFFFF000000000000)
        {
            return (val & 0xFF00000000000000) ?
                GHeapUpperBit[(val >> 56)       ] + 56: 
                GHeapUpperBit[(val >> 48) & 0xFF] + 48;
        }
        return (val & 0xFF0000000000) ?
            GHeapUpperBit[(val >> 40) & 0xFF] + 40:
            GHeapUpperBit[(val >> 32) & 0xFF] + 32;
    }
    else
    {
        if (val & 0xFFFF0000)
        {
            return (val & 0xFF000000) ? 
                GHeapUpperBit[(val >> 24)       ] + 24: 
                GHeapUpperBit[(val >> 16) & 0xFF] + 16;
        }
        return (val & 0xFF00) ?
            GHeapUpperBit[(val >> 8) & 0xFF] + 8:
            GHeapUpperBit[(val     ) & 0xFF];
    }

#endif
}

//------------------------------------------------------------------------
GINLINE UByte GHeapGetLowerBit(UPInt val)
{
#ifndef GFC_64BIT_POINTERS

    if (val & 0xFFFF)
    {
        return (val & 0xFF) ?
            GHeapLowerBit[ val & 0xFF]:
            GHeapLowerBit[(val >> 8) & 0xFF] + 8;
    }
    return (val & 0xFF0000) ?
            GHeapLowerBit[(val >> 16) & 0xFF] + 16:
            GHeapLowerBit[(val >> 24) & 0xFF] + 24;

#else

    if (val & 0xFFFFFFFF)
    {
        if (val & 0xFFFF)
        {
            return (val & 0xFF) ?
                GHeapLowerBit[ val & 0xFF]:
                GHeapLowerBit[(val >> 8) & 0xFF] + 8;
        }
        return (val & 0xFF0000) ?
                GHeapLowerBit[(val >> 16) & 0xFF] + 16:
                GHeapLowerBit[(val >> 24) & 0xFF] + 24;
    }
    else
    {
        if (val & 0xFFFF00000000)
        {
             return (val & 0xFF00000000) ?
                GHeapLowerBit[(val >> 32) & 0xFF] + 32:
                GHeapLowerBit[(val >> 40) & 0xFF] + 40;
        }
        return (val & 0xFF000000000000) ?
            GHeapLowerBit[(val >> 48) & 0xFF] + 48:
            GHeapLowerBit[(val >> 56) & 0xFF] + 56;
    }

#endif
}


#endif

