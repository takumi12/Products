...
#ifndef INC_GHeapStarter_H
#define INC_GHeapStarter_H
...
{
public:
    GHeapStarter(GSysAllocPaged* sysAlloc, 
                 UPInt granularity    = 4*GHeap_PageSize, 
                 UPInt headerPageSize = 1*GHeap_PageSize);

    GSysAllocPaged* GetSysAlloc() { return Granulator.GetSysAlloc(); }

    void*   Alloc(UPInt size, UPInt alignSize);
    void    Free(void* ptr, UPInt size, UPInt alignSize); // Size is required!

    void    VisitMem(GHeapMemVisitor* visitor) const;
    void    VisitSegments(class GHeapSegVisitor* visitor) const;

    UPInt   GetFootprint() const { return Granulator.GetFootprint(); }
    UPInt   GetUsedSpace() const { return Granulator.GetUsedSpace(); }

private:
    GHeapGranulator Granulator;
};
