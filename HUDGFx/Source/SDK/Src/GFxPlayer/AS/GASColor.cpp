...
void GASColorCtorFunction::GlobalCtor(const GASFnCall& fn)
{
    GFxASCharacter* ptarget = 0;
    if (fn.NArgs >= 1)
        ptarget = fn.Env->FindTargetByValue(fn.Arg(0));

    GPtr<GASColorObject> pcolor;
    if (!fn.ThisPtr || fn.ThisPtr->GetObjectType() != Object_Color)
        pcolor = *GHEAP_NEW(fn.Env->GetHeap()) GASColorObject(fn.Env, ptarget);
    else
    {
        pcolor = static_cast<GASColorObject*>(fn.ThisPtr);
        pcolor->SetTarget(ptarget);
    }
    fn.Result->SetAsObject(pcolor.GetPtr());
}

GASObject* GASColorCtorFunction::CreateNewObject(GASEnvironment* penv) const 
{
    return GHEAP_NEW(penv->GetHeap()) GASColorObject(penv, NULL);
}

GASFunctionRef GASColorCtorFunction::Register(GASGlobalContext* pgc)
{
    GASStringContext sc(pgc, 8);
    GASFunctionRef  ctor(*GHEAP_NEW(pgc->GetHeap()) GASColorCtorFunction(&sc));
    GPtr<GASObject> proto = *GHEAP_NEW(pgc->GetHeap()) GASColorProto(&sc, pgc->GetPrototype(GASBuiltin_Object), ctor);
    pgc->SetPrototype(GASBuiltin_Color, proto);
    pgc->pGlobal->SetMemberRaw(&sc, pgc->GetBuiltin(GASBuiltin_Color), GASValue(ctor));
    return ctor;
}
