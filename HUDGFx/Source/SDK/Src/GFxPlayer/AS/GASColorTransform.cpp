...
//
void GASColorTransformCtorFunction::GlobalCtor(const GASFnCall& fn)
{
    if (fn.ThisPtr && fn.ThisPtr->GetObjectType() == GASObject::Object_ColorTransform)
    {
        GPtr<GASColorTransformObject> pct = static_cast<GASColorTransformObject*>(fn.ThisPtr);
...
GASObject* GASColorTransformCtorFunction::CreateNewObject(GASEnvironment* penv) const 
{
    return GHEAP_NEW(penv->GetHeap()) GASColorTransformObject(penv);
}

GASFunctionRef GASColorTransformCtorFunction::Register(GASGlobalContext* pgc)
{
    GASStringContext sc(pgc, 8);
    GASFunctionRef  ctor(*GHEAP_NEW(pgc->GetHeap()) GASColorTransformCtorFunction(&sc));
    GPtr<GASObject> proto = *GHEAP_NEW(pgc->GetHeap()) GASColorTransformProto(&sc, pgc->GetPrototype(GASBuiltin_Object), ctor);
    pgc->SetPrototype(GASBuiltin_ColorTransform, proto);
    pgc->FlashGeomPackage->SetMemberRaw(&sc, pgc->GetBuiltin(GASBuiltin_ColorTransform), GASValue(ctor));
    return ctor;
}
