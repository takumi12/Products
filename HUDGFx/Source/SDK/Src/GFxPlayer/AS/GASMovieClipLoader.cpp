...
void GASMovieClipLoaderProto::UnloadClip(const GASFnCall& fn)
{
    ...
    if (fn.Arg(0).IsCharacter())
        ...
        psprite = (GFxASCharacter*) fn.Env->GetMovieRoot()->GetLevelMovie(fn.Arg(0).ToInt32(fn.Env));
    else
...
//////////////////
GASMovieClipLoaderCtorFunction::GASMovieClipLoaderCtorFunction(GASStringContext *psc)
...
{
    if (fn.ThisPtr && fn.ThisPtr->GetObjectType() == GASObject::Object_MovieClipLoader && 
        !fn.ThisPtr->IsBuiltinPrototype())
    {
        GASMovieClipLoader* ab = static_cast<GASMovieClipLoader*>(fn.ThisPtr);
...
GASObject* GASMovieClipLoaderCtorFunction::CreateNewObject(GASEnvironment* penv) const 
{
    return GHEAP_NEW(penv->GetHeap()) GASMovieClipLoader(penv);
}

GASFunctionRef GASMovieClipLoaderCtorFunction::Register(GASGlobalContext* pgc)
{
    GASStringContext sc(pgc, 8);
    GASFunctionRef  ctor(*GHEAP_NEW(pgc->GetHeap()) GASMovieClipLoaderCtorFunction(&sc));
    GPtr<GASObject> proto = *GHEAP_NEW(pgc->GetHeap()) GASMovieClipLoaderProto(&sc, pgc->GetPrototype(GASBuiltin_Object), ctor);
    pgc->SetPrototype(GASBuiltin_MovieClipLoader, proto);
    pgc->pGlobal->SetMemberRaw(&sc, pgc->GetBuiltin(GASBuiltin_MovieClipLoader), GASValue(ctor));
    return ctor;
}
