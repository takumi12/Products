...
{
    GPtr<GASPointObject> retpt = *GHEAP_NEW(fn.Env->GetHeap()) GASPointObject(fn.Env);
    bool set = false;
    if (fn.NArgs > 0) 
    {
...
void GASPointCtorFunction::Distance(const GASFnCall& fn)
{
    ...
    if (fn.NArgs > 1) 
    {
        GASObject* p1 = fn.Arg(0).ToObject(fn.Env);
        GASObject* p2 = fn.Arg(1).ToObject(fn.Env);
        if (( NULL != p1) && ( NULL != p2))
        {
...
//
void GASPointCtorFunction::GlobalCtor(const GASFnCall& fn)
{
    GPtr<GASPointObject> ppt;
    if (fn.ThisPtr && fn.ThisPtr->GetObjectType() == GASObject::Object_Point && 
        !fn.ThisPtr->IsBuiltinPrototype())
        ppt = static_cast<GASPointObject*>(fn.ThisPtr);
    else
        ppt = *GHEAP_NEW(fn.Env->GetHeap()) GASPointObject(fn.Env);
    fn.Result->SetAsObject(ppt.GetPtr());
...
GASObject* GASPointCtorFunction::CreateNewObject(GASEnvironment* penv) const 
{
    return GHEAP_NEW(penv->GetHeap()) GASPointObject(penv);
}

GASFunctionRef GASPointCtorFunction::Register(GASGlobalContext* pgc)
{
    GASStringContext sc(pgc, 8);
    GASFunctionRef pointCtor(*GHEAP_NEW(pgc->GetHeap()) GASPointCtorFunction(&sc));
    GPtr<GASObject> pointProto = 
        *GHEAP_NEW(pgc->GetHeap()) GASPointProto(&sc, pgc->GetPrototype(GASBuiltin_Object), pointCtor);
    pgc->SetPrototype(GASBuiltin_Point, pointProto);
    pgc->FlashGeomPackage->SetMemberRaw(&sc, pgc->GetBuiltin(GASBuiltin_Point), GASValue(pointCtor));
    return pointCtor;
}
