...
class GFxTextureGlyph : public GRefCountBaseNTS<GFxTextureGlyph, GFxStatMD_Fonts_Mem>
{
public:
    ...
    GFxTextureGlyph(const GFxTextureGlyph &src)
        : GRefCountBaseNTS<GFxTextureGlyph, GFxStatMD_Fonts_Mem>()
    {
        pImage      = src.pImage;
...
class GFxTextureGlyphData : public GRefCountBaseNTS<GFxTextureGlyphData, GFxStatMD_Fonts_Mem>
{
public:
...
private:
    ...
    UInt           TextureFlags, CharPadding;

    typedef GHashLH<GFxResourceId, GFxResourcePtr<GFxImageResource>,
                    GFixedSizeHash<GFxResourceId> >                 ImageResourceHash;

    // Texture glyphs structure; either built up by tha font packer
    // or loaded from GFX file by tag loaders.
    GArrayLH<GFxTextureGlyph>           TextureGlyphs;
...
public:
    ...
    UInt    GetTextureFlags() const                        { return TextureFlags;          }
    void    SetTextureFlags(UInt flags)                    { TextureFlags        = flags;  }
    UInt    GetCharPadding() const                         { return CharPadding;           }
    void    SetCharPadding(UInt padding)                   { CharPadding         = padding;}
    ...
    UInt                    GetTextureGlyphCount() const { return (UInt)TextureGlyphs.GetSize(); }
...
public:
    ...
    GFxShapeBase*           GetGlyphShape(UInt glyphIndex, UInt glyphSize);
    ...
    virtual GString         GetCharRanges() const;

     
    // *** Other APIs

    inline bool             HasVectorOrRasterGlyphs() const { return CodeTable.GetSize() != 0; }    
...
private:
    ...
    // GArray<GRenderer::Rect> BoundsTable;    // @@ this thing should be optional.
};
...
//
// This is used for DefineFont2 and DefineFont3 tags and when GFxFontCompactorParams 
...
{
public:
    typedef GFxFontCompactor::ContainerType ContainerType;
    ...
    const CompactedFontType& GetCompactedFont() const { return CompactedFont; }
...
private:
    ...
    CompactedFontType CompactedFont;
    UInt              NumGlyphs;
};
...
{
public:
    typedef GArrayUnsafeLH_POD<UByte, GFxStatMD_Fonts_Mem> ContainerType;
    typedef GFxCompactedFont<ContainerType> CompactedFontType;
...
public:
    ...
    GFxShapeBase*           GetGlyphShape(UInt glyphIndex, UInt glyphSize) const 
    { 
...
private:
    ...
    union {
        ...
        GArray<DisposeHandler*> *   pHandlerArray;  
    };
