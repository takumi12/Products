...
//------------------------------------------------------------------------
struct GFxGlyphDynaSlot : GListNode<GFxGlyphDynaSlot>
{
    enum 
    { 
        LockFlag = 0x8000U,
...
struct GFxGlyphBand
{
    ...
    UInt16 y,h;
    UInt16 RightSpace;
    GList2<GFxGlyphDynaSlot, GFxGlyphDynaSlot_Band> Slots;
};
...
{
public:
    enum { SID = GFxStatFC_GlyphCache_Mem };
    ...
    UInt GetNumPacked() const { return (UInt)GlyphHTable.GetSize(); }
    ...
    void SetEventHandler(GFxGlyphCacheEventHandler* h);

private:
    typedef GHashLH<GFxGlyphParam, GFxGlyphNode*, GFxGlyphParam, SID>   CachedGlyphsType;
    typedef GList2<GFxGlyphDynaSlot, GFxGlyphDynaSlot_Active>           ActiveSlotsType;
    ...
    GListAllocLH_POD<GFxGlyphDynaSlot, 127, SID>    Slots;
    GList<GFxGlyphDynaSlot>                         SlotQueue;
    UInt                                            SlotQueueSize;
    ActiveSlotsType                                 ActiveSlots;
    GListAllocLH_POD<GFxGlyphNode, 127, SID>        Glyphs;
    GArrayUnsafeLH_POD<GFxGlyphBand, SID>           Bands;
    CachedGlyphsType                                GlyphHTable;

    UInt                                            NumUsedBands;
    GFxGlyphCacheEventHandler*                      pEventHandler;
};
...
public:
   ...
   ~GFxGlyphRasterCache();
    ...
    UInt          GetTextureWidth()  const { return TextureWidth; }
    UInt          GetTextureHeight() const { return TextureHeight; }
    GTexture*     GetTexture(UInt i) const { return Textures[i].pTexture; }
...
private:
    ...
    Float                                           ScaleU;
    Float                                           ScaleV;
    GArrayPagedLH_POD<GlyphUpdateType, 6, 16, SID>  GlyphsToUpdate;
    GArrayUnsafeLH_POD<GTexture::UpdateRect, SID>   RectsToUpdate;
    TextureType                                     Textures[TexturePoolSize];

    GPtr<GImage>                                    TexUpdBuffer;
    GFxTextureUpdatePacker                          TexUpdPacker;
    GPtr<GImage>                                    KnockOutCopy;
    GArrayUnsafeLH_POD<UInt8, SID>                  BlurStack;
    GArrayUnsafeLH_POD<Float, SID>                  BlurSum;
    GCompoundShape                                  Shape;          // TBD: Revise SID
    GRasterizer                                     Rasterizer;     // TBD: Revise SID
    GFxGlyphScanlineFilter                          ScanlineFilter; // TBD: Revise SID
    GFxGlyphFitter                                  Fitter;         // TBD: Revise SID
