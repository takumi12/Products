...
class GFxMesh : public GNewOverrideBase<GStat_Default_Mem>
{
public:
    GFxMesh();
    ...
    UInt    GetTriangleCount() const { return (UInt)(TriangleIndices.GetSize() /3); }
    ...
    UPInt   GetNumBytes() const         { return TriangleIndices.GetNumBytes(); }
private:
    ...
    GArrayLH<UInt16>            TriangleIndices;
...
class GFxCachedStroke : public GNewOverrideBase<GStatRG_MeshStroke_Mem>
{
public:
    GFxCachedStroke();
    ~GFxCachedStroke();
    ...
    // Rendering.
    bool    Display(GFxDisplayContext &context, const GFxLineStyle& style,
...
private:
    ...
    SInt                    Style;
    GArrayLH<SInt16>        Coords;
    GRenderer::CachedData   CoordsCache;
    GArrayLH<UInt>          CoordCounts;    // Number of coordinates in each path.
...
class GFxMeshSet : public GNewOverrideBase<GStatRG_MeshFill_Mem>
{
public:
    enum KeyCategoryType
    {
        KeyCategory_Regular = 0,
        KeyCategory_Scale9Grid,
        KeyCategory_Morph
    };
    ...
    void            SetImgAdjustMatrices(const GArray<GFxScale9GridInfo::ImgAdjust>& adj);

    // I/O
    void    OutputCachedData(GFile* out);
    void    InputCachedData(GFile* in);

    void            SetMeshKey(KeyCategoryType keyCat, const void* keyData, UInt len, GFxCharacter* inst);
    bool            MeshKeyFits(KeyCategoryType keyCat, const void* keyData, UInt len) const;
    ...
    UPInt           GetNumBytes() const;
//    const GFxShapeBase* GetShapePtr() const { return pShape; }

    void            DoLock() { Locked = true;  }
    void            Unlock() { Locked = false; }
    bool            IsLocked() const { return Locked; }

    UInt32          GetNumStrokes() const;

private:
    void AllocKeyData(UInt len);
    void FreeKeyData();
    UInt GetMeshKeySize() const;
...
//      int LastFrameRendered;  // @@ we shouldn't spontaneously drop cached data I don't think...
    ...
    GArrayLH<GFxMesh,         GStatRG_MeshFill_Mem>   Meshes; // One mesh per style.
    GArrayLH<GFxCachedStroke, GStatRG_MeshStroke_Mem> Strokes;
    ...
    GArrayLH<MeshSubShape>              Shapes;
    UInt8                               KeyData[2*sizeof(void*)];

    GFxCharacter*                       pInstance;
    GArrayLH<GFxTexture9Grid>*          pTexture9Grids;
    ...
    GArrayLH<GRenderer::Matrix>*        pImgAdjustMatrices;
    UInt                                NumStrokes;
};
