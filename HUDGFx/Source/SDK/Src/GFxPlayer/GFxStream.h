...
public:
    ...
    GFxStream(GFile* pinput, GMemoryHeap* pheap,
              GFxLog *plog, GFxParseControl *pparseControl);
    GFxStream(const UByte* pbuffer, UInt bufSize, GMemoryHeap* pheap,
              GFxLog *plog, GFxParseControl *pparseControl);
    ~GFxStream();
    ...
    bool           ReadString(GString* pstr);   
    // For string that begins with an 8-bit length code.    
    bool           ReadStringWithLength(GString* pstr);     
    // Read version that allocated memory from heap and passes it to user.
    char*          ReadString(GMemoryHeap* pheap);
    char*          ReadStringWithLength(GMemoryHeap* pheap);
    ...
    int     ReadToBuffer(UByte* pdestBuf, UInt sz);
    ...
    GMemoryHeap*          GetHeap() const         { return FileName.GetHeap(); }
