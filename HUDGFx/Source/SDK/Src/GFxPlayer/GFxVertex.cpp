...
GFxVertexArray::GFxVertexArray(GMemoryHeap *pheap, VertexFormat vf, UInt size)    
{
    pHeap = pheap ? pheap : GMemory::GetHeapByAddress(this);
    Size = AllocSize = 0;
    pData = 0;
...
GFxVertexArray::GFxVertexArray(GMemoryHeap *pheap, const GFxVertexArray& src)    
{
    pHeap = pheap ? pheap : GMemory::GetHeapByAddress(this);
    Size = AllocSize = 0;
    pData = 0;
...
void    GFxVertexArray::Resize(UInt vertexCount)
{
    ...
    // Must re-allocate
#ifdef GFC_OS_PS3
    UByte* pnewData = (UByte*)GHEAP_ALLOC(pHeap, 12 + vertexCount * pVertexDesc->VertexSize,
                                          StatType);
#else
