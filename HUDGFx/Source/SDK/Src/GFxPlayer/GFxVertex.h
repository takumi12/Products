...
class GFxVertexArray : public GNewOverrideBase<GStatRG_Vertices_Mem>
{
public:
...
public: 
    GFxVertexArray(GMemoryHeap *pheap = 0, VertexFormat vf = GRenderer::Vertex_None, UInt size = 0);
    GFxVertexArray(GMemoryHeap *pheap, const GFxVertexArray& src);
    ~GFxVertexArray();

    UPInt   GetNumBytes() const 
    { 
        return AllocSize * pVertexDesc->VertexSize; 
    }

    void    ClearAndRelease()   { Resize(0); Reserve(0, true); }
