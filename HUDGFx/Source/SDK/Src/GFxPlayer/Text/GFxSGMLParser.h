...
public:
    GFxSGMLCharIter(const Char* p, UPInt sz): 
    ...
    {
        return (G_iswspace((wchar_t)c) != 0);
    }
    static bool IsAlpha(UInt32 c)
    {
        return (G_iswalpha((wchar_t)c) != 0);
    }
    static bool IsAlphaNum(UInt32 c)
    {
        return (G_iswalnum((wchar_t)c) != 0);
    }
...
template <class Char>
int GFxSGMLCharIter<Char>::StrCompare(const Char* wstr, const char* str, UPInt len) 
{
    if (len)
    {
        int f,l;
        ...
        return (0-(int)G_strlen(str));
}

template <class Char>
int GFxSGMLCharIter<Char>::StrCompare(const Char* dst, UPInt dstlen, const Char* src, UPInt srclen) 
{
    if (dstlen)
    {
        int f,l;
...
public:
    GFxSGMLParser(GMemoryHeap* pheap, const Char* phtml, UPInt htmlSize) :
      pHeap(pheap),
...
template <class Char>
void    GFxSGMLParser<Char>::AppendToBuf(const Char* pstr, UPInt sz)
{
    if (BufPos + sz > BufSize)
    {
        BufSize += sz;
        if (!pBuffer)
            pBuffer = (Char*)GHEAP_ALLOC(pHeap, BufSize * sizeof(Char), GFxStatMV_Text_Mem);
        else
            pBuffer = (Char*)GREALLOC(pBuffer, BufSize * sizeof(Char), GFxStatMV_Text_Mem);
    }
...
template <class Char>
bool      GFxSGMLParser<Char>::GetNextAttribute(const Char** ppattrName, UPInt* pattrNameSz)
{
    if (CurState == SGMLPS_ERROR)
        return false;
...
template <class Char>
bool    GFxSGMLParser<Char>::GetNextAttributeValue(const Char** ppattrValue, UPInt* pattrValueSz)
{
    bool rv = false;
    if (CurState == SGMLPS_START_ELEMENT_ATTR_VALUE)
    {
...
template <class Char>
void        GFxSGMLParser<Char>::ParseName(const Char** ppname, UPInt* plen)
{
    GASSERT(ppname && plen);
...
template <class Char>
bool        GFxSGMLParser<Char>::ParseStartElement(const Char** ppelemName, UPInt* pelemLen)
{
    bool rv = false;
...
template <class Char>
bool        GFxSGMLParser<Char>::ParseEndElement(const Char** ppelemName, UPInt* pelemLen)
{
    bool rv = false;
...
template <class Char>
bool        GFxSGMLParser<Char>::ParseContent(const Char** ppContent, UPInt* pcontentSize)
{
...
template <class Char>
bool GFxSGMLParser<Char>::ParseInt(int* pdestVal, const Char* pstr, UPInt len)
{
    if (len == 0)
        return false;
    int v = 0, sign = 1;
...
template <class Char>
bool GFxSGMLParser<Char>::ParseHexInt(UInt32* pdestVal, const Char* pstr, UPInt len)
{
    if (len == 0)
        return false;
    UInt32 v = 0;
...
template <class Char>
bool GFxSGMLParser<Char>::ParseFloat(Float* pdestVal, const Char* pstr, UPInt len)
{
    if (len == 0)
        return false;
    Double v = 0, sign = 1;
