...
#ifndef INC_TEXT_GFXTEXTLINEBUFFER_H
#define INC_TEXT_GFXTEXTLINEBUFFER_H
...
struct GFxTextImageDesc : public GRefCountBaseNTS<GFxTextImageDesc, GFxStatMV_Text_Mem>
{
    typedef GRenderer::Matrix   MatrixType;

    GPtr<GFxShapeWithStyles>    pImageShape;
...
public:
    ...
    public:
        ...
        GFxFontHandle*      GetFontHandle()  const  { return pFontHandle; }
    ...
    public:
        ...
        void AddFont(GFxFontHandle* pfont)
        {
            ...
            GASSERT(!pGlyphs[GlyphIndex].HasFmtFont());
            GASSERT(!pGlyphs[GlyphIndex].HasFmtColor()); // font should be set BEFORE color
            GASSERT(!pGlyphs[GlyphIndex].HasFmtImage()); // image cannot be set with font
        ...
        {
            pGlyphs[GlyphIndex].SetNextFormat();
            GASSERT(!pGlyphs[GlyphIndex].HasFmtColor());
            GASSERT(!pGlyphs[GlyphIndex].HasFmtImage()); // image cannot be set with color
        ...
        {
            pGlyphs[GlyphIndex].SetNextFormat();
            GASSERT(!pGlyphs[GlyphIndex].HasFmtFont()); // image cannot be set with font
            GASSERT(!pGlyphs[GlyphIndex].HasFmtColor());// image cannot be set with color
            GASSERT(!pGlyphs[GlyphIndex].HasFmtImage());
    ...
    struct LineData32 // 38 bytes
    {
        ...
        void    SetOffset(SInt x, SInt y) { OffsetX = x; OffsetY = y; }
        GPoint<SInt> GetOffset() const { return GPoint<SInt>(OffsetX, OffsetY); }
        SInt GetOffsetX() const { return OffsetX; }
        SInt GetOffsetY() const { return OffsetY; }
        void SetOffsetX(SInt x) { OffsetX = x; }
        void SetOffsetY(SInt y) { OffsetY = y; }
    ...
    public:
        ...
        void SetOffset(SInt x, SInt y) { (IsData8()) ? Data8.SetOffset(x, y) : Data32.SetOffset(x, y); }
        GPoint<SInt> GetOffset() const { return (IsData8()) ? Data8.GetOffset() : Data32.GetOffset(); }
        SInt GetOffsetX() const { return (IsData8()) ? Data8.GetOffsetX() : Data32.GetOffsetX(); }
        SInt GetOffsetY() const { return (IsData8()) ? Data8.GetOffsetY() : Data32.GetOffsetY(); }
        void SetOffsetX(SInt x) { (IsData8()) ? Data8.SetOffsetX(x) : Data32.SetOffsetX(x); }
        void SetOffsetY(SInt y) { (IsData8()) ? Data8.SetOffsetY(y) : Data32.SetOffsetY(y); }
...
public:
    ...
    UInt GetSize() const { return (UInt)Lines.GetSize(); }
    ...
    {
        InvalidateCache();
        RemoveLines(0, (UInt)Lines.GetSize());
    }
    ...
    public:
        ...
        { 
            return !pLineBuffer || CurrentPos >= pLineBuffer->Lines.GetSize() || (SInt)CurrentPos < 0; 
        }
    ...
    public:
        ...
        Line* InsertNewLine(UInt glyphCount, UInt formatDataElementsCount, LineType lineType)
        {
            ...
            UInt pos = GetIndex();
            if (pos == ~0u)
                pos = (UInt)pLineBuffer->Lines.GetSize();
...
public:
    ...
    {
        //TODO: GArray<Line*>  FreeLinesPool; ?
    public:
