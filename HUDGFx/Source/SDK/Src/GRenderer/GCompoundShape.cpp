...
GDECLARE_MEMORY_STAT_AUTOSUM_GROUP(GStatRG_Mem,   "RenderGen",    GStat_Mem)
GDECLARE_MEMORY_STAT(GStatRG_Vertices_Mem,        "Vertices",     GStatRG_Mem)
GDECLARE_MEMORY_STAT(GStatRG_MeshFill_Mem,        "MeshFill",     GStatRG_Mem)
GDECLARE_MEMORY_STAT(GStatRG_MeshStroke_Mem,      "MeshStroke",   GStatRG_Mem)
GDECLARE_MEMORY_STAT(GStatRG_Tessellator_Mem,     "Tessellator",  GStatRG_Mem)
GDECLARE_MEMORY_STAT(GStatRG_Rasterizer_Mem,      "Rasterizer",   GStatRG_Mem)
GDECLARE_MEMORY_STAT(GStatRG_Stroker_Mem,         "Stroker",      GStatRG_Mem)
GDECLARE_MEMORY_STAT(GStatRG_EdgeAA_Mem,          "EdgeAA",       GStatRG_Mem)
GDECLARE_MEMORY_STAT(GStatRG_StrokerAA_Mem,       "StrokerAA",    GStatRG_Mem)
GDECLARE_MEMORY_STAT(GStatRG_Other_Mem,           "RG_Other",     GStatRG_Mem)
...
//-----------------------------------------------------------------------
void GCompoundShape::ClearAndRelease()
{
    Clear();
...
//-----------------------------------------------------------------------
UPInt GCompoundShape::GetNumBytes() const
{
    return Vertices.GetNumBytes() + Paths.GetNumBytes();
}
...
{
    Paths.PushBack(SPath(this, (unsigned)Vertices.GetSize(), lStyle, rStyle, lineStyle));
    CurrPath = &Paths.Back();
    if(lStyle >= 0)
    {
...
{
    const GPointType& p = Vertices.Back();
                                    ...
                                    GCoordType* x2, GCoordType* y2) const
{
    ...
    *y2 = 0;
    if(Vertices.GetSize())
    {
        *x1 = *x2 = Vertices[0].x;
        *y1 = *y2 = Vertices[0].y;
        for(unsigned i = 1; i < Vertices.GetSize(); i++)
        {
