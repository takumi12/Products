...
void GStrokePath::ClosePath(bool forceClosing)
{
    ...
    if(Path.GetSize() > 1)
    {
        Closed = !Path[Path.GetSize()-1].CalcDistance(Path[0]);
    }

    while(Path.GetSize() > 1)
    {
        if(Path[Path.GetSize()-2].CalcDistance(Path[Path.GetSize()-1])) break;
        GStrokeVertexType t = Path[Path.GetSize()-1];
        Path.PopBack();
        Path.PopBack();
        Path.PushBack(t);
    }
    ...
    if(Closed)
    {
        while(Path.GetSize() > 2)
        {
            if(Path[Path.GetSize()-1].CalcDistance(Path[0])) break;
            Path.PopBack();
        }
        if(Path.GetSize() < 3) 
        {
