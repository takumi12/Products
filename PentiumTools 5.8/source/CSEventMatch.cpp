...
void CSBaseMatch::clearMatchInfo(void)
{
	...
	m_iMaxKillMonster = -1;
	m_iKillMonster = -1;
	SetPosition(WinFrameX - 230 / 2, 100);
}
...
void CSBaseMatch::RenderTime(void)
{
	...
	if (m_iMatchCountDownType > TYPE_MATCH_NONE && m_iMatchCountDownType < TYPE_MATCH_END)
	{
		...
		else
		{
			...
			g_pRenderText->RenderText(WinFrameX / 2, (int)y, lpszStr, 0, 0, RT3_WRITE_CENTER);
		}
	}
}
...
void CSDevilSquareMatch::RenderMatchResult(void)
{
	...
	g_pRenderText->RenderText(xPos[5], yPos, GlobalText[684], (WinFrameX - 230) / 2 + 210 - xPos[5], RT3_SORT_CENTER);
	yPos += 20;
