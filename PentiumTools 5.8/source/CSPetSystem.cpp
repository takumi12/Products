...
void CSPetDarkSpirit::MovePet(void)
{
	...
	if (o->AI == PET_FLY || o->AI == PET_FLYING)
	{
		...
		if ((!g_isCharacterBuff(Owner, eBuff_Cloaking)) && time_base_check)
		{
			for (int i = 0; i < 1; ++i)
			{
	...
	else if (o->AI == PET_ATTACK || o->AI == PET_ESCAPE)
	{
		...
		VectorAddTime(o->Position, Pos, o->Position);
...
void CSPetDarkSpirit::AttackEffect(CHARACTER* c, OBJECT* o)
{
	...
	case PET_ATTACK:
		if (c->AttackTime >= 0 && c->AttackTime <= 2 && m_PetTarget != NULL)
		{
			for (int i = 0; i < 10; i++)
			{
				CreateJointtime(BITMAP_LIGHT, o->Position, o->Position, o->Angle, 1, NULL, (float)(rand() % 40 + 20));
			}
			if (c->AttackTime == 1)
			{
				vec3_t Angle, Light;
			...
			if (o->BoneTransform != NULL)
			{
				...
				for (int i = rand() % 2; i < 66; i += 2)
				{
					if (!b->Bones[i].Dummy && i < b->NumBones)
					{
						...
						CreateParticletime(BITMAP_LIGHT, Pos, o->Angle, Light, 6, 1.3f);
					}
				}
			}
		}
