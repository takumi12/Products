...
UPDATE_WAY_POINT_ENTRY:
	if (m_dwCameraWalkState == CAMERAWALK_STATE_MOVE && m_listWayPoint.empty() == false)
	{
		...
		if (m_iDelayCount >= pTargetWayPoint->iDelay)
		{
			...
			float targetfCameraMoveAccel = timefac(pTargetWayPoint->fCameraMoveAccel);
			float originfCameraMoveAccel = timefac(pOriginWayPoint->fCameraMoveAccel);
			...
			else
			{
				...
				if (IsTourPaused())
				{
					if (m_fForceSpeed > 0)
					{
						m_CurrentCameraPos[0] += (fDirVector[0] * targetfCameraMoveAccel * m_fForceSpeed);
						m_CurrentCameraPos[1] += (fDirVector[1] * targetfCameraMoveAccel * m_fForceSpeed);
						m_vTourCameraPos[0] += (fTourDirVector[0] * targetfCameraMoveAccel * m_fForceSpeed);
						m_vTourCameraPos[1] += (fTourDirVector[1] * targetfCameraMoveAccel * m_fForceSpeed);
					}
					else if (m_fForceSpeed < 0)
					{
						m_CurrentCameraPos[0] += (fRvsDirVector[0] * originfCameraMoveAccel * m_fForceSpeed);
						m_CurrentCameraPos[1] += (fRvsDirVector[1] * originfCameraMoveAccel * m_fForceSpeed);
						m_vTourCameraPos[0] += (fTourDirVector[0] * originfCameraMoveAccel * m_fForceSpeed);
						m_vTourCameraPos[1] += (fTourDirVector[1] * originfCameraMoveAccel * m_fForceSpeed);
					}
				}
				else
				{
					m_CurrentCameraPos[0] += (fDirVector[0] * targetfCameraMoveAccel);
					m_CurrentCameraPos[1] += (fDirVector[1] * targetfCameraMoveAccel);
					m_vTourCameraPos[0] += (fTourDirVector[0] * targetfCameraMoveAccel);
					m_vTourCameraPos[1] += (fTourDirVector[1] * targetfCameraMoveAccel);
				}
