...
bool M33Aida::RenderAidaObjectVisual(OBJECT* pObject, BMD* pModel)
{
	...
	case 56:
		...
		if (rand() % 2 == 0)
		{
			CreateParticletime(BITMAP_WATERFALL_1, pObject->Position, pObject->Angle, Light, 2, pObject->Scale);
		}
		break;
	case 57:
		Vector(1.f, 1.f, 1.f, Light);
		CreateParticletime(BITMAP_WATERFALL_3, pObject->Position, pObject->Angle, Light, 4, pObject->Scale);
		break;
	case 58:
		Vector(1.f, 1.f, 1.f, Light);
		if (rand() % 3 == 0)
		{
			CreateParticletime(BITMAP_WATERFALL_2, pObject->Position, pObject->Angle, Light, 2, pObject->Scale);
		}
		break;
	case 59:
		if (pObject->HiddenMesh != -2)
		{
			...
			for (int i = 0; i < 20; ++i)
			{
				CreateParticletime(BITMAP_CLOUD, pObject->Position, pObject->Angle, Light, 1, pObject->Scale, pObject);
			}
		}
		break;
	case 60:
	{
		...
		if (time >= 0 && time < 10) {
			vec3_t Light;
			Vector(1.f, 1.f, 1.f, Light);
			CreateEffecttime(MODEL_BUTTERFLY01, pObject->Position, pObject->Angle, Light, 3, pObject);
		}
	...
	case 67:
		...
		CreateParticletime(BITMAP_FLAME, pObject->Position, pObject->Angle, pObject->Light, 7, pObject->Scale);
		break;
	case 70:
	{
		int time = timeGetTime() % 1024;
		if (rand() % 5 == 1 && (time >= 0 && time < 30))
		{
			...
			CreateEffecttime(MODEL_GHOST, pObject->Position, pObject->Angle, Light, 0, pObject, -1, 0, pObject->Scale);
		}
	}
	break;
	case 71:
...
bool M33Aida::RenderAidaMonsterVisual(CHARACTER* pCharacter, OBJECT* pObject, BMD* pModel)
{
	...
	case MODEL_MONSTER01 + 100:
	{
		...
		CreateParticletime(BITMAP_LIGHT + 1, Position, Angle, Light, 4, 4.0f);
		...
		if (pObject->CurrentAction == MONSTER01_ATTACK1 || pObject->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			if (pObject->AnimationFrame >= 4.0f)
			{
				...
				CreateParticletime(BITMAP_SMOKE, Position, Angle, Light, 27, 2.0f);
				Vector(1.0f, 1.0f, 1.0f, Light);
				CreateParticletime(BITMAP_LIGHT + 1, Position, Angle, Light, 2, 1.0f);
			}
			if (pObject->AnimationFrame >= 6.0f && pObject->CurrentAction == MONSTER01_ATTACK1)
			{
				...
				BoneManager::GetBonePosition(pObject, "Monster100_Footstepst", Relative, Position);
				Vector(1.0f, 1.0f, 1.0f, Light);
				CreateParticletime(BITMAP_LIGHT + 1, Position, Angle, Light, 3, 1.3f);
			}
			if (pCharacter->TargetCharacter >= 0 && pCharacter->TargetCharacter < MAX_CHARACTERS_CLIENT)
			{
				...
				if (pObject->AnimationFrame >= 6.0f && pObject->CurrentAction == MONSTER01_ATTACK1)
				{
					CreateParticletime(BITMAP_LIGHT + 1, vTemp, Angle, Light, 3, 1.3f);
				}
				if (pObject->AnimationFrame >= 6.0f && pObject->CurrentAction == MONSTER01_ATTACK2)
				{
					CreateJointtime(BITMAP_JOINT_THUNDER, to->Position, vTemp, pObject->Angle, 16);
				}
			}
		}
	...
	case MODEL_MONSTER01 + 101:
	{
		...
		CreateParticletime(BITMAP_WATERFALL_2, Position, Angle, Light, 1);
		...
		if (pObject->CurrentAction == MONSTER01_ATTACK1)
		{
			...
			if (pObject->AnimationFrame >= 4.0f)
			{
				...
				CreateParticletime(BITMAP_SMOKE, Position, Angle, Light, 25);

				if (pObject->AnimationFrame >= 5.0f && pObject->AnimationFrame <= 5.5f)
				{
					Vector(5.0f, 5.0f, 5.0f, Light);
					BoneManager::GetBonePosition(pObject, "Monster101_L_Arm", Relative, Position);
					CreateParticletime(BITMAP_SHOCK_WAVE, Position, Angle, Light, 3, 0.5f);
				}
			}
			...
			if (pCharacter->TargetCharacter >= 0 && pCharacter->TargetCharacter < MAX_CHARACTERS_CLIENT && pObject->AnimationFrame >= 6.9f)
			{
				...
				CreateJointtime(BITMAP_JOINT_ENERGY, vTemp, pObject->Position, to->Angle, 16, pObject, 20.0f);
			}
		}
	...
	case MODEL_MONSTER01 + 102:
	{
		...
		CreateParticletime(BITMAP_SPARK + 1, Position, Angle, Light, 7);
	...
	case MODEL_MONSTER01 + 104:
	{
		...
		for (int i = 0; i < 6; i++)
		{
			...
			CreateParticletime(BITMAP_SPARK + 1, Position, Angle, Light, 6);
		}
		if (pObject->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			CreateParticletime(BITMAP_SMOKE, Position, pObject->Angle, Light, 26);
		}
	...
	case MODEL_MONSTER01 + 105:
	{
		...
		if (pObject->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			CreateParticletime(BITMAP_TRUE_FIRE, Position, Angle, Light, 6, 5.0f);
