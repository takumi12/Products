...
bool GMNewTown::RenderObjectVisual(OBJECT* pObject, BMD* pModel)
{
	...
	case 54:
		...
		if (rand() % 4 == 0)
		{
			CreateParticletime(BITMAP_WATERFALL_2, pObject->Position, pObject->Angle, Light, 4, pObject->Scale);
		}
		break;
	case 58:
		CreateParticletime(BITMAP_WATERFALL_5, pObject->Position, pObject->Angle, Light, 0);
		break;
	case 59:
		Vector(1.f, 1.f, 1.f, Light);
		CreateParticletime(BITMAP_WATERFALL_3, pObject->Position, pObject->Angle, Light, 8, pObject->Scale);
		break;
	case 60:
		if (pObject->HiddenMesh != -2)
		{
			...
			for (int i = 0; i < 10; ++i)
			{
				if (gMapManager.WorldActive == WD_77NEW_LOGIN_SCENE)
					CreateParticletime(BITMAP_CLOUD, pObject->Position, pObject->Angle, Light, 3, pObject->Scale * 2, pObject);
				else
					CreateParticletime(BITMAP_CLOUD, pObject->Position, pObject->Angle, Light, 3, pObject->Scale, pObject);
			}
			pObject->HiddenMesh = -2;
		}
		break;
	case 61:
		...
		{
			Vector(1.f, 1.f, 1.f, Light);
			CreateParticletime(BITMAP_TRUE_BLUE, pObject->Position, pObject->Angle, Light, 0, pObject->Scale);
		}
		break;
	case 63:
	{
	...
	case 110:
		...
		if (rand() % 2 == 0)
		{
			...
			CreateParticletime(BITMAP_LIGHT, Position, pObject->Angle, Light, 11, 0.6f, pObject);
		}
		break;
	case 121:
	{
	...
	case 140:	case 141:	case 142:	case 143:	case 144:	case 145:	case 146:	case 147:
		if (pObject->HiddenMesh != -2)
		{
			...
			pNewObject->PriorAnimationFrame = pNewObject->AnimationFrame =
				rand() % gmClientModels->GetModel(pNewObject->Type)->Actions[pNewObject->CurrentAction].NumAnimationKeys;
