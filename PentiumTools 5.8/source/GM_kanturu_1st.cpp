...
bool M37Kanturu1st::MoveKanturu1stObject(OBJECT* pObject)
{
	...
	case 97:
		...
		if (pObject->Timer > 5.f)
			CreateParticletime(BITMAP_BUBBLE, pObject->Position, pObject->Angle, pObject->Light, 5);
		break;
	case 98:
...
bool M37Kanturu1st::RenderKanturu1stObjectVisual(OBJECT* pObject, BMD* pModel)
{
	...
	case 59:
		...
		{
			Vector(1.f, 1.f, 1.f, Light);
			CreateParticletime(BITMAP_SMOKE, pObject->Position, pObject->Angle, Light, 21, pObject->Scale);
		}
		break;
	case 61:
		if (rand() % 3 == 0)
		{
			Vector(1.f, 1.f, 1.f, Light);
			CreateParticletime(BITMAP_TRUE_FIRE, pObject->Position, pObject->Angle, Light, 0, pObject->Scale);
		}
		break;
	case 62:
	...
	case 92:
	{
		...
		if (rand() % 30 == 1)
			CreateJointtime(BITMAP_JOINT_THUNDER, StartPos, EndPos, pObject->Angle, 18, NULL, 50.f);
	}
	break;
	case 96:
		pModel->StreamMesh = 0;
	...
	case 98:
		if (rand() % 3 == 0)
		{
			...
			CreateParticletime(BITMAP_TWINTAIL_WATER, Position, pObject->Angle, Light, 2);
		}
		break;
	case 105:
	{
		...
		CreateParticletime(BITMAP_ENERGY, Position, pObject->Angle, Light, 0, 1.5f);
		...
		EndPos[2] += 10.f;
		if (rand() % 20 == 0)
			CreateJointtime(BITMAP_JOINT_THUNDER, StartPos, EndPos, pObject->Angle, 8, NULL, 40.f);
	}
	break;
	case 107:
		if (pObject->HiddenMesh != -2)
...
bool M37Kanturu1st::AttackEffectKanturu1stMonster(CHARACTER* c, OBJECT* o, BMD* b)
{
	...
	case MODEL_MONSTER01 + 108:
	{
		if (o->CurrentAction == MONSTER01_ATTACK1 || o->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			CreateParticletime(BITMAP_SPARK + 1, vPos, o->Angle, vLight, 10, 4.0f);
		}
	...
	case MODEL_MONSTER01 + 111:
	{
		if (o->CurrentAction == MONSTER01_ATTACK1 || o->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			CreateParticletime(BITMAP_SMOKE, vPos, o->Angle, vLight, index, Width);
...
bool M37Kanturu1st::RenderKanturu1stMonsterVisual(CHARACTER* c, OBJECT* o, BMD* b)
{
	...
	case MODEL_MONSTER01 + 106:
	{
		...
		CreateParticletime(BITMAP_SMOKE, vPos, o->Angle, o->Light, 42, o->Scale);
	...
	case MODEL_MONSTER01 + 113:
	{
		...
		if (o->CurrentAction == MONSTER01_WALK)
		{
			...
			CreateParticletime(BITMAP_SMOKE + 1, vPos, o->Angle, o->Light, 1, o->Scale);
			CreateEffecttime(MODEL_STONE1 + rand() % 2, vPos, o->Angle, o->Light);
		}
		return true;
	}
	break;
	...
	case MODEL_MONSTER01 + 108:
	{
		...
		else
		{
			...
			CreateParticletime(BITMAP_SMOKE + 3, vPos, o->Angle, vLight, 3, 2.0f);
			CreateParticletime(BITMAP_SMOKE + 3, vPos, o->Angle, vLight, 4, 1.0f);
			if (o->SubType == FALSE)
			{
				o->SubType = TRUE;
	...
	case MODEL_MONSTER01 + 111:
	{
		...
		if (o->CurrentAction == MONSTER01_DIE)
		{
			...
			CreateParticletime(BITMAP_SMOKE + 3, vPos, o->Angle, o->Light, 3, Scale);
			BoneManager::GetBonePosition(o, "KENTAUROS_BIP_TAIL_1", vRelative, vPos);
			CreateParticletime(BITMAP_SMOKE + 1, vPos, o->Angle, o->Light, 1, Scale);
			...
			{
				if (rand() % 10 == 0) {
					CreateParticletime(BITMAP_SMOKE + 1, o->Position, o->Angle, o->Light);
				}
			}
	}
	break;
