...
GLuint CGlobalBitmap::LoadImageUnZip(const std::string& filename, unsigned char* Data, long DataBytes, GLuint uiFilter, GLuint uiWrapMode)
{
	BITMAP_t* pBitmap = FindTexture(filename);
	if (pBitmap)
	{
	...
	{
		GLuint uiNewTextureIndex = GenerateTextureIndex();
		if (true == LoadImageUnZip(uiNewTextureIndex, filename, Data, DataBytes, uiFilter, uiWrapMode))
		{
			m_listNonamedIndex.push_back(uiNewTextureIndex);
...
bool CGlobalBitmap::LoadImageUnZip(GLuint uiBitmapIndex, const std::string& filename, unsigned char* Data, long DataBytes, GLuint uiFilter, GLuint uiWrapMode)
{
	unsigned int UICLAMP = GL_CLAMP_TO_EDGE;
	unsigned int UIREPEAT = GL_REPEAT;
	...
	if (0 == _stricmp(ext.c_str(), "jpg"))
		return OpenJpeg(uiBitmapIndex, filename, Data, DataBytes, uiFilter, uiWrapMode);
	else if (0 == _stricmp(ext.c_str(), "tga"))
		return OpenTga(uiBitmapIndex, filename, Data, DataBytes, uiFilter, uiWrapMode);
...
bool CGlobalBitmap::OpenJpeg(GLuint uiBitmapIndex, const std::string& filename, GLuint uiFilter, GLuint uiWrapMode)
{
	...
	/*std::size_t pos = filename_ozj.find("Data");

	if (pos != std::string::npos)
	{
		std::string newPath = filename_ozj; newPath.replace(pos, 4, "Data2");

		std::vector<BYTE> mibyte;


		fseek(infile, 0, SEEK_END);
		int chunkSize = ftell(infile);
		fseek(infile, 0, SEEK_SET);

		mibyte.resize(chunkSize, 0);

		fread(mibyte.data(), 1, chunkSize, infile);

		HANDLE file = CreateFile(newPath.c_str(), GENERIC_WRITE, FILE_SHARE_READ, 0, CREATE_ALWAYS, FILE_ATTRIBUTE_ARCHIVE, 0);

		if (file != INVALID_HANDLE_VALUE)
		{
			DWORD written;
			BOOL result = WriteFile(file, mibyte.data(), chunkSize, &written, NULL);
	...
	if (cinfo.output_width <= MAX_WIDTH && cinfo.output_height <= MAX_HEIGHT)
	{
		...
		pNewBitmap->Buffer.resize(BufferSize, 0);
		...
		BYTE* buff = pNewBitmap->Buffer.data();
		while (cinfo.output_scanline < cinfo.output_height)
		{
			...
			memcpy(buff + (cinfo.output_scanline - 1) * Width * 3, buffer[0], row_stride);
			offset += row_stride;
		}
		...
		glTexImage2D(GL_TEXTURE_2D, 0, 3, Width, Height, 0, GL_RGB, GL_UNSIGNED_BYTE, buff);
...
bool CGlobalBitmap::OpenTga(GLuint uiBitmapIndex, const std::string& filename, GLuint uiFilter, GLuint uiWrapMode)
{
	...
	FILE* infile = fopen(filename_ozt.c_str(), "rb");
	if (infile == NULL)
	{
		return false;
	}

	/*std::size_t pos = filename_ozt.find("Data");

	if (pos != std::string::npos)
	{
		std::string newPath = filename_ozt; newPath.replace(pos, 4, "Data2");
	...
	for (int y = 0; y < ny; y++)
	{
		...
		index += nx * 4;
		unsigned char* dst = &buff[(ny - 1 - y) * Width * pNewBitmap->Components];
	...
	glTexImage2D(GL_TEXTURE_2D, 0, 4, Width, Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, buff);
...
bool CGlobalBitmap::OpenJpeg(GLuint uiBitmapIndex, const std::string& filename, unsigned char* Data, long DataBytes, GLuint uiFilter, GLuint uiWrapMode)
{
	struct jpeg_decompress_struct cinfo;
	struct my_error_mgr jerr;
	...
	jpeg_stdio_buffer_src(&cinfo, &Data[24], (DataBytes - 24));
...
bool CGlobalBitmap::OpenTga(GLuint uiBitmapIndex, const std::string& filename, unsigned char* PakBuffer, long DataBytes, GLuint uiFilter, GLuint uiWrapMode)
{
	if (PakBuffer == NULL || DataBytes == 0)
	{
		return false;
	}
