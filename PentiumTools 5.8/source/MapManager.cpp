...
void CMapManager::Load() // OK
{
	...
	if (this->InHellas())
	{
		...
		gmClientModels->GetModel(MODEL_CUNDUN_PART6)->Actions[0].Loop = false;
		gmClientModels->GetModel(MODEL_CUNDUN_PART6)->Actions[0].PlaySpeed = 0.13f;
		gmClientModels->GetModel(MODEL_CUNDUN_PART7)->Actions[0].Loop = false;
		gmClientModels->GetModel(MODEL_CUNDUN_PART7)->Actions[0].PlaySpeed = 0.13f;
		gmClientModels->GetModel(MODEL_CUNDUN_PHOENIX)->Actions[0].Loop = true;
		gmClientModels->GetModel(MODEL_CUNDUN_GHOST)->Actions[0].Loop = false;
...
void CMapManager::LoadWorld(int Map)
{
	...
	if (this->WorldActive == WD_73NEW_LOGIN_SCENE || this->WorldActive == WD_77NEW_LOGIN_SCENE)
	{
	...
	if (CreateWaterTerrain(this->WorldActive) == false)
	{
		...
		if (M34CryWolf1st::IsCyrWolf1st() == true)
		{
			...
			LoadBitmap(FileName, BITMAP_RAIN_01, GL_NEAREST, GL_CLAMP_TO_EDGE, false);
		}
		else
		{
		...
		LoadBitmap(FileName, BITMAP_RAIN_02, GL_NEAREST, GL_CLAMP_TO_EDGE, false);
		sprintf(FileName, "%s\\rain03.tga", "World10");
		LoadBitmap(FileName, BITMAP_RAIN_03, GL_NEAREST, GL_CLAMP_TO_EDGE, false);
...
bool CMapManager::InHiddenHellas(int iMap)
{
	...
	return (iMap == WD_24HELLAS_7) ? true : false;
}
...
bool IsAsheron(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	}

	return ((World == WD_91GMACHERON1 || World == WD_92GMACHERON2));
}

bool IsDebenter(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	...
	return ((World == WD_95DEBENTER1 || World == WD_96DEBENTER2));
}

bool IsUrukMountain(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	...
	return ((World == WD_100URUK_MOUNTAIN || World == WD_101URUK_MOUNTAIN));
}

bool IsDevilSquareFinal(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	...
	return ((World == WD_102TORMENTED_SQUARE));
}

bool IsTormentedSquare(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	...
	return ((World == WD_103TORMENTED_SQUARE || World == WD_104TORMENTED_SQUARE || World == WD_105TORMENTED_SQUARE || World == WD_106TORMENTED_SQUARE));
}

bool IsNars(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	...
	return ((World == WD_110NARS1 || World == WD_111NARS2));
}

bool IsDungeonFloor(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	...
	return ((World >= WD_116DEEP_DUNGEON && World <= WD_120DEEP_DUNGEON));
}

bool IsKuberaMine(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	...
	return ((World >= WD_123KUBERA_MINE && World <= WD_127KUBERA_MINE));
}

bool IsAtlansAbbyss(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	}
	return ((World >= WD_128ATLANSE_ABYSS && World <= WD_130ATLANSE_ABYSS));
}

bool IsScorchedCanyon(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	}
	return ((World == WD_131SCORCHED_CANYON));
}

bool IsIcarusRed(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	}
	return ((World == WD_132SCARLET_ICARUS));
}

bool IsTempleOfArnil(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	}
	return ((World == WD_133TEMPLE_OF_ARNIL));
}

bool IsAshenAida(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	}
	return ((World == WD_134ASHEN_AIDA));
}

bool IsOldKethotum(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	}
	return ((World == WD_135OLD_KETHOTUM));
}

bool IsBlazeKethotum(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	}
	return ((World == WD_136BLAZEKE_KETHOTUM));
}

bool IsKanturuUnderGround(int World)
{
	if (World == -1)
	{
		World = gMapManager.WorldActive;
	}
	return ((World == WD_137KANTURU_UNDERGROUND));
}