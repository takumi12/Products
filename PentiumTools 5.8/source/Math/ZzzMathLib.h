...
#define VectorSubtractTime(a,b,c) {(c)[0]=(a)[0]-(b)[0]*FPS_ANIMATION_FACTOR;(c)[1]=(a)[1]-(b)[1]*FPS_ANIMATION_FACTOR;(c)[2]=(a)[2]-(b)[2]*FPS_ANIMATION_FACTOR;}
#define VectorAddTime(a,b,c) {(c)[0]=(a)[0]+(b)[0]*FPS_ANIMATION_FACTOR;(c)[1]=(a)[1]+(b)[1]*FPS_ANIMATION_FACTOR;(c)[2]=(a)[2]+(b)[2]*FPS_ANIMATION_FACTOR;}
...
inline void VectorCopy(const vec3_t vIn, vec3_t vOut)
{
	if (vIn && vOut)
	{
		vOut[0] = vIn[0];
...
void interpolatesum(vec3_t& a, vec3_t& b);
