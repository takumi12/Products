...
bool SEASON3B::CNewUIChatLogWindow::RenderMessages()
{
	...
	{
		fRenderPosY = fRenderPosY + (SCROLL_MIDDLE_PART_HEIGHT * (m_nShowingLines - GetCurrentRenderEndLine() - 1)) + FONT_LEADING;
	}
	else
	{
	...
	for (int i = 0; ; i++)
	{
		...
		if (m_msgEnable[pMsgText->GetType()])
		{
			...
			{
				POINT ptRenderPos = { (long)fRenderPosX + (long)WND_LEFT_RIGHT_EDGE, (long)fRenderPosY + (long)FONT_LEADING + ((long)SCROLL_MIDDLE_PART_HEIGHT * (long)i) };
...
bool SEASON3B::CNewUIChatLogWindow::RenderFrame()
{
	if (m_bShowFrame)
	{
		...
		RenderImage(IMAGE_DRAG_BTN, fRenderPosX, fRenderPosY - (float)RESIZING_BTN_HEIGHT, (float)WND_WIDTH, 10.0f);
		DisableAlphaBlend();
...
bool SEASON3B::CNewUIChatLogWindow::Render()
{
	...
	if (m_bShowChatLog)
	{
		...
		if (m_msgEnable[TYPE_SYSTEM_MESSAGE] == false)
		{
			...
			//SetMsgVisible(TYPE_ERROR_MESSAGE, true);
			RenderMessages();
			RecoverBackPosition();
			SetMsgVisible(TYPE_SYSTEM_MESSAGE, false);
			//SetMsgVisible(TYPE_ERROR_MESSAGE, false);
		}
