...
void SEASON3B::CNewUICommandWindow::InitButtons()
{
	...
	if (gmProtect->LookAndFeel == 1 || gmProtect->LookAndFeel == 2)
	{
		...
		{
			SetButtonImagen(&m_BtnCommand[i], Bitmap_Message_box, Bitmap_Message_box);
			m_BtnCommand[i].ChangeButtonInfo(m_Pos.x + add_x, add_y, 100.0, 25.0);
			add_y += 28.0;
		}
...
bool SEASON3B::CNewUICommandWindow::Render()
{
	...
	if (gmProtect->LookAndFeel == 1 || gmProtect->LookAndFeel == 2)
	{
		for (int i = COMMAND_TRADE; i < COMMAND_END; i++)
		{
			...
			m_BtnCommand[i].Render(213.0, 64.0);
		}
	}
	else
	{
