...
void SEASON3B::CNewUICursedTempleResult::RenderResultPanel()
{
	...
	if (m_WinState == 1)
	{
		RenderImage(IMAGE_CURSEDTEMPLERESULT_SUCCESS, (WinFrameX - 360) / 2, m_Pos.y - 110, 351.f, 115.f);
	}
	else
	{
		RenderImage(IMAGE_CURSEDTEMPLERESULT_FAILURE, (WinFrameX - 360) / 2, m_Pos.y - 110, 351.f, 115.f);
	}
