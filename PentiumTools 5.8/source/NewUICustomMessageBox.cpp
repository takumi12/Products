...
bool SEASON3B::CAddPointsReceiptMsgBoxLayout::SetLayout()
{
	...
	{
	case 0:
		sprintf_s(strText, "%s %s %s", GlobalText[3505], GlobalText[166], GlobalText[682]);
		break;
	case 1:
		sprintf_s(strText, "%s %s %s", GlobalText[3505], GlobalText[167], GlobalText[682]);
		break;
	case 2:
		sprintf_s(strText, "%s %s %s", GlobalText[3505], GlobalText[169], GlobalText[682]);
		break;
	case 3:
		sprintf_s(strText, "%s %s %s", GlobalText[3505], GlobalText[168], GlobalText[682]);
		break;
	case 4:
		sprintf_s(strText, "%s %s %s", GlobalText[3505], GlobalText[1900], GlobalText[682]);
		break;
	}
...
CALLBACK_RESULT SEASON3B::CAddPointsReceiptMsgBoxLayout::ProcessOk(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (InputIndex >= 0 && InputIndex < 5)
	{
		...
		{
		case 0:
			sprintf_s(strText, "/s %d", InputStat);
			break;
		case 1:
			sprintf_s(strText, "/a %d", InputStat);
			break;
		case 2:
			sprintf_s(strText, "/v %d", InputStat);
			break;
		case 3:
			sprintf_s(strText, "/e %d", InputStat);
			break;
		case 4:
			sprintf_s(strText, "/c %d", InputStat);
			break;
		}
		SendChat(strText);
	}
...
bool SEASON3B::CNewUIItem3DInputMsgBox::Create(DWORD dwMsgBoxType, DWORD dwInputType, int iInputBoxWidth, int iInputBoxHeight, int iLimitText, bool bIsPassword)
{
	...
	if (m_pInputBox)
	{
		m_pInputBox->Init(g_hWnd, iInputBoxWidth, iInputBoxHeight, iLimitText, bIsPassword);
		x = GetPos().x + (MSGBOX_WIDTH / 2) - (iInputBoxWidth / 2);
		y = GetPos().y + MSGBOX_TOP_HEIGHT - INPUTBOX_TOP_BLANK;
		m_pInputBox->SetPosition(x, y);
		m_pInputBox->SetTextColor(255, 255, 230, 210);
...
{
	PlayBuffer(25, 0, 0);
	g_MessageBox->SendEvent(pOwner, 1);
	return CALLBACK_BREAK;
}
...
void SEASON3B::CNewUIItem3DInputMsgBox::GetInputBoxText(unicode::t_char* strText)
{
	if (m_pInputBox)
	{
		m_pInputBox->GetText(strText);
...
void SEASON3B::CNewUIItem3DInputMsgBox::SetInputBoxOption(int iOption)
{
	if (m_pInputBox)
	{
		m_pInputBox->SetOption(iOption);
...
void SEASON3B::CNewUIItem3DInputMsgBox::SetInputBoxPosition(int x, int y)
{
	if (m_pInputBox)
	{
		m_pInputBox->SetPosition(x, y);
...
void SEASON3B::CNewUIItem3DInputMsgBox::SetInputBoxSize(int width, int height)
{
	if (m_pInputBox)
	{
		m_pInputBox->SetSize(width, height);
