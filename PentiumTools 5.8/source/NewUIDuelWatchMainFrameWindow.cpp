...
bool CNewUIDuelWatchMainFrameWindow::Create(CNewUIManager* pNewUIMng, CNewUI3DRenderMng* pNewUI3DRenderMng)
{
	...
	int x = ((WinFrameX - 640.0) / 2.0);
	m_BtnExit.SetPos(x + 604, WinFrameY - 29);
	m_BtnExit.ChangeButtonImgState(true, IMAGE_INVENTORY_EXIT_BTN, false);
	m_BtnExit.ChangeButtonInfo(x + 604, WinFrameY - 29, 36, 29);
...
bool CNewUIDuelWatchMainFrameWindow::Render()
{
	...
	POINT ptOrigin = { ((WinFrameX - 640.0) / 2.0), (long)(WinFrameY - 51.f) };
	...
	{
		RenderImage(IMAGE_DUELWATCH_MAINFRAME_SCORE, WinFrameX - 74 - 17 * i, 460 + add, 16.f, 17.f);
	}
