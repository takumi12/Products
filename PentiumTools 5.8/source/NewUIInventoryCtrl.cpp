...
void SEASON3B::CNewUIInventoryCtrl::RenderNumberOfItem()
{
	...
	for (; li != m_vecItem.end(); li++)
	{
		...
		{
			CUSTOM_ITEM_STACK* StackItem = GMProtect->GetItemStack(pItem->Type);
			if (StackItem != NULL && pItem->Durability > 1)
			{
				glColor3f(1.f, 0.9f, 0.7f);
				SEASON3B::RenderNumber(x + width - 6, y + 1, pItem->Durability);
			}
...
void SEASON3B::CNewUIInventoryCtrl::BackupPickedItem()
{
	if (ms_pPickedItem && EquipmentItem == false)
	{
		...
		else if (pItemObj->ex_src_type == ITEM_EX_SRC_EQUIPMENT)
		{
			...
			if (pEquipmentItemSlot->Type == ITEM_HELPER + 4 && !gMapManager.InChaosCastle())
			{
				SendRequestPetInfo(PET_TYPE_DARK_SPIRIT, 0, EQUIPMENT_WEAPON_LEFT);
			}
			DeletePickedItem();
...
bool SEASON3B::CNewUIInventoryCtrl::IsOverlayItem(ITEM* pSourceItem, ITEM* pTargetItem)
{
	...
	if (iSrcType == iTarType)
	{
		...
		{
			CUSTOM_ITEM_STACK* StackItem = GMProtect->GetItemStack(iSrcType);
			if (StackItem != NULL && (iSrcDurability < StackItem->MaxStack && iTarDurability < StackItem->MaxStack))
			{
				return true;
			}
		}
