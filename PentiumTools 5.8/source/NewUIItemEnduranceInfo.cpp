...
void SEASON3B::CNewUIItemEnduranceInfo::RenderWorld()
{
	if (g_pNewUIMiniMap->IsSuccess())
	{
		float PosY = 16.f;
		float PosX = GetWindowsX - 128;
		...
		if (GetScreenWidth() == ((int)GetWindowsX))
		{
			...
			if (m_iHideMap == 0)
			{
				int factor_view = 32 * m_iZoom;

				float TY = (double)((float)(Hero->PositionX - factor_view) / 255.0);
				float TX = (double)((float)(Hero->PositionY - factor_view) / 255.0);

				RenderBitmapLocalRotate(BITMAP_MINI_MAP_BEGIN, PosX, PosY, 128.f, 128.f, 45.f, TX, TY, (0.25 * m_iZoom), (0.25 * m_iZoom), true);

				float Rot = (((BYTE)((Hero->Object.Angle[2] + 22.5f) / 360.f * 8.f + 1.f) % 8) + 2) * 45.0;

				RenderFrameAnimation2(IMAGE_HUB_WORLD_USER, PosX + 64, PosY + 64, 20.0, 20.0, Rot, 32.0 / 512.0, 32.0 / 128.0, 1.25, 10, 30);
			}

			PosY -= 16.f;
			RenderBtns(PosX, PosY, 15.0, 15.0, 0);
			RenderBtns(PosX + 15.0, PosY, 15.0, 15.0, 1);
			RenderBtns(PosX + 30.0, PosY, 83.0, 15.0, 2);
			RenderBtns(PosX + 113, PosY, 15.0, 15.0, 3);
			if (m_iHideMap == 0 && (RelativemoveX != -1 || RelativemoveY != -1))
			{
				char targetposition[100];
				sprintf_s(targetposition, "[%d][%d]", RelativemoveX, RelativemoveY);
				g_pRenderText->RenderFont(PosX + 30.f, PosY, targetposition, 83.f, 16.f, 3);
			}
		}
	}
}
...
bool SEASON3B::CNewUIItemEnduranceInfo::UpdateMouseWorld()
{
	if (g_pNewUIMiniMap->IsSuccess())
	{
		if (GetScreenWidth() == ((int)GetWindowsX))
		{
			float PosY = 0;
			float PosX = GetWindowsX - 128;
			float iHeight = (m_iHideMap == 0 ? 143.f : 15.f);

			if (SEASON3B::CheckMouseIn(PosX, PosY, 15.f, 15.f) && SEASON3B::IsRelease(VK_LBUTTON))
			{
				if (m_iZoom < 4)
				{
					m_iZoom = m_iZoom * 2;
			...
			if (SEASON3B::CheckMouseIn(PosX + 15.f, PosY, 15.f, 15.f) && SEASON3B::IsRelease(VK_LBUTTON))
			{
				if (m_iZoom > 1)
				{
					m_iZoom = m_iZoom / 2;
			...
			if (SEASON3B::CheckMouseIn(PosX + 113.f, PosY, 15.f, 15.f) && SEASON3B::IsRelease(VK_LBUTTON))
			{
				...
				if (g_pPartyListWindow->IsEnableMove())
				{
					...
					{
						g_pPartyListWindow->SetPos(PosX);
					}
				}
				return true;
			}
			...
			return SEASON3B::CheckMouseIn(PosX, PosY, 128, iHeight);
		}
	}
	return false;
}

void SEASON3B::CNewUIItemEnduranceInfo::runtimemoverad()
{
	RelativemoveX = -1;
	RelativemoveY = -1;
	...
	{
		int PosY = 16.f;
		int PosX = GetWindowsX - 128;

		int visorwidth = 128;

		if (SEASON3B::CheckMouseIn(PosX, PosY, visorwidth, visorwidth))
		{
			const float runscale = (visorwidth / (64.f * m_iZoom));
			const float TX = (float)(Hero->PositionY - (32.f * m_iZoom));
			const float TY = (float)(Hero->PositionX - (32.f * m_iZoom));

			float relativeMouseX = ((int)MouseX - PosX) / runscale;
			float relativeMouseY = ((int)MouseY - PosY) / runscale;

			CollisionDetectRotate((32.f * m_iZoom), (32.f * m_iZoom), 315.f, relativeMouseX, relativeMouseY);

			RelativemoveY = (relativeMouseX + TX);
			RelativemoveX = (relativeMouseY + TY);

			if (SEASON3B::IsRelease(VK_LBUTTON))
			{
				g_pNewUIMiniMap->how_to_get(RelativemoveX, RelativemoveY);
			}
		}
	}
}
