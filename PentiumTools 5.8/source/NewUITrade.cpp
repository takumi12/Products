...
bool CNewUITrade::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	if (false == m_pYourInvenCtrl->Create(g_pNewUI3DRenderMng, g_pNewItemMng, this, x + 16, y + 68, COLUMN_TRADE_INVEN, ROW_TRADE_INVEN))
	{
	...
	if (false == m_pMyInvenCtrl->Create(g_pNewUI3DRenderMng, g_pNewItemMng, this, x + 16, y + 274, COLUMN_TRADE_INVEN, ROW_TRADE_INVEN))
	{
...
void SEASON3B::CNewUITrade::SetYourTradeGold(int nGold)
{
...
void CNewUITrade::ProcessToReceiveTradeResult(LPPTRADE pTradeData)
{
	...
	case 1:
		...
		int x = 260 * MouseX / WinFrameX;
		::SetCursorPos(x * WindowWidth / WinFrameX, MouseY * WindowHeight / WinFrameY);
