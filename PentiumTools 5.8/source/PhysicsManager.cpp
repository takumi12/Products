...
void CPhysicsVertex::UpdateForce(unsigned int iKey, DWORD dwType, float fWind)
{
	...
	{
	case PCT_RUBBER:
		m_vForce[2] += timefac(fRand * (fWind + 0.1f) * 1.f);
		break;
	case PCT_RUBBER2:
		m_vForce[2] += timefac(fRand * fWind) ;
		break;
	}
	...
	case PCT_ELASTIC_HALLOWEEN:
		m_vForce[0] += -timefac(fRand * fWind * 0.5f);
		m_vForce[2] += timefac(fRand * (fWind + 0.1f) * 0.5f * (float)sinf(WorldTime * 0.003f) * 5.0f);
		m_vForce[2] -= timefac(s_Gravity * fGravityRate * s_fMass * 50.0f);
		break;
#ifdef PBG_ADD_NEWCHAR_MONK_ITEM
	case PCT_ELASTIC_RAGE_L:
		m_vForce[0] += -timefac((fRand * fWind * 0.8f));
		break;
	...
	{
	case PCT_HEAVY:
		m_vForce[2] -= timefac(s_Gravity * fGravityRate * s_fMass * 180.0f);
		break;
	default:
		m_vForce[2] -= timefac(s_Gravity * fGravityRate * s_fMass * 100.0f);
		break;
	}
}
...
void CPhysicsCloth::InitForces()
{
	...
	{
		m_pVertices[iVertex].UpdateForce(abs(iSeed % m_iNumHor - iVertex % m_iNumHor) + abs(iSeed / m_iNumHor - iVertex / m_iNumHor), m_dwType, timefac(m_fWind));
	}
}
...
void CPhysicsManager::Move(float fTime)
{
	...
	CPhysicsManager::s_fWind += timefac(fPlus);
