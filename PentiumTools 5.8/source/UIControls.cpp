...
bool CUIRenderTextOriginal::Create(HDC hDC)
{
	...
	DIB_INFO->bmiHeader.biWidth = WinFrameX;		//. 640
	DIB_INFO->bmiHeader.biHeight = -(WinFrameY);		//. 480
...
void CUIRenderTextOriginal::WriteText(int iOffset, int iWidth, int iHeight)
{
	...
	SIZE FontDCSize = { WinFrameX, WinFrameY };
...
void CUIRenderTextOriginal::RenderText(int iPos_x, int iPos_y, const unicode::t_char* pszText, int iBoxWidth, int iBoxHeight, int iSort, OUT SIZE* lpTextSize)
{
...
void CUIRenderTextOriginal::RenderTextClipped(float iPos_x, float iPos_y, const unicode::t_char* pszText, int iBoxWidth, int iBoxHeight, int iSort, OUT SIZE* lpTextSize)
{
	...
	//	EnableAlphaTest();
	//	glColor4ub(GetRed(m_dwBackColor), GetGreen(m_dwBackColor), GetBlue(m_dwBackColor), GetAlpha(m_dwBackColor));
	//	RenderColor(RealBoxPos.x * g_fScreenRate_x, RealBoxPos.y * g_fScreenRate_y, RealBoxSize.cx * g_fScreenRate_x, RealBoxSize.cy * g_fScreenRate_y);
	//	EndRenderColor();
	//}
...
bool CUITextInputBox::UpdateProperty()
{
	int OriginalX = m_iWidth;
	int OriginalY = m_iHeight;

	if ((int)(OriginalX * g_fScreenRate_x) != m_iRealWindowX
		|| (int)(OriginalY * g_fScreenRate_y) != m_iRealWindowY)
	{
		UpdateBmiHeader(OriginalX, OriginalY);
	}
	return true;
}
...
void CUITextInputBox::Render()
{
	...
	if (m_bUseMultiLine == FALSE)
	{
		...
		{
			g_pRenderText->SetBgColor(0);
			g_pRenderText->SetTextColor(130, 232, 0, 255);
			g_pRenderText->RenderText(m_iPos_x, m_iPos_y, textPlaceholder.c_str(), m_iWidth, m_iHeight, RT3_SORT_CENTER);
		}
