		...
		if (pbw)
		{
			...
			if (!(iOption & UIADDWND_FORCEPOSITION))
			{
				for (m_WindowMapIter = m_WindowMap.begin(); m_WindowMapIter != m_WindowMap.end(); ++m_WindowMapIter)
				{
					...
					{
						if (iPos_x + pbw->GetWidth() + 20 <= WinFrameX)
							iPos_x += 20;
						if (iPos_y + pbw->GetHeight() + 20 <= WinFrameY)
							iPos_y += 20;
						if (iPos_x + pbw->GetWidth() + 20 > WinFrameX && iPos_y + pbw->GetHeight() + 20 > WinFrameY)
						{
							if (iPos_y % 10 == 9)
							{
								delete pbw;
...
void CUIWindowMgr::HideAllWindow(BOOL bHide, BOOL bMainClose)
{
	...
	if (bHide == FALSE)
	{
		...
		{
			OpenMainWnd(WinFrameX - 250, (WinFrameY - 48) - 170);
		}
	...
	if (GetState() == UISTATE_MOVE)
	{
		if (MouseLButton == true)
		{
			...
			if (m_iPos_x + MouseX - m_iMouseClickPos_x < 0)
				m_iPos_x = 0;
			else if (m_iPos_x + m_iWidth + MouseX - m_iMouseClickPos_x > WinFrameX)
				m_iPos_x = WinFrameX - m_iWidth;
			else
			...
			else if (m_iPos_y + m_iHeight + MouseY - m_iMouseClickPos_y > WinFrameY)
			{
				m_iPos_y = WinFrameY - m_iHeight;
			}
			else
			{
	...
	else if (GetState() == UISTATE_RESIZE)
	{
		if (MouseLButton == true)
		{
			if (m_iResizeDir == 135)
			{
				...
				if (m_iPos_x + m_iWidth + MouseX - m_iMouseClickPos_x > WinFrameX)
					m_iWidth = WinFrameX - m_iPos_x;
				else
				...
				if (m_iPos_y + m_iHeight + MouseY - m_iMouseClickPos_y > WinFrameY)
					m_iHeight = WinFrameY - m_iPos_y;
				else
	...
	case UI_MESSAGE_BTNLCLICK:
	{
		...
		case 1:
			if (m_iShowType == 1)
			{
				...
				if (m_iPos_x + m_iWidth > WinFrameX) m_iPos_x = WinFrameX - m_iWidth;
				Refresh();
			}
