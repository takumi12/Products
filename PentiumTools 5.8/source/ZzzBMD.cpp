...
bool BMD::runtime_render_effect(OBJECT* pObject, float Alpha, int RenderType, int Texture, BMD* oHighHierarchyModel)
{
	if (unknown != 0 && commandLua != NULL)
	{
		...
		{
			//g_ConsoleDebug->Write(5, "[%s][%1.f][%d][%d]", FileName, Alpha, RenderType, Texture);
			return true;
		}
	}
	return false;
		...
		if (m->NumTriangles)
		{
			...
			if (Texture != BITMAP_HIDE)
			{
				...
				case BITMAP_WATER:
					if (HideSkin == true)
						return;
					Texture = WaterTextureNumber + BITMAP_WATER;
					break;
				case BITMAP_HAIR:
					if (HideSkin == true)
						return;
				...
				if (Texture != -1)
				{
						...
						|| RenderFlag & RENDER_CHROME8)
					{
						...
						if (RenderFlag & RENDER_CHROME3 || RenderFlag & RENDER_CHROME4 || RenderFlag & RENDER_CHROME5 || RenderFlag & RENDER_CHROME7 || RenderFlag & RENDER_CHROME8)
						{
							if (Alpha < 0.99f)
							{
								BodyLight[0] *= timefac(Alpha);
								BodyLight[1] *= timefac(Alpha);
								BodyLight[2] *= timefac(Alpha);
							}
							EnableAlphaBlend();
						}
						...
						for (int k = 0; k < tp->Polygon; k++)
						{
							...
							if (Render > RENDER_CHROME4)
							{
								if (Render == RENDER_OIL)
								{
									...
									float TexCoordU = g_chrome[vi][0] * texp->TexCoordU + BlendMeshTexCoordU;
									float TexCoordV = g_chrome[vi][1] * texp->TexCoordV + BlendMeshTexCoordV;
									glTexCoord2f(TexCoordU, TexCoordV);
								}
								else if (Render == RENDER_CHROME8)
								{
									if (Alpha < 0.99)
										glColor4f(BodyLight[0], BodyLight[1], BodyLight[2], Alpha);
									else
...
bool BMD::Open2(char* DirName, char* ModelFileName, bool bReAlloc)
{
	...
	std::size_t pos = filename.find("Data");

	if (pos != std::string::npos)
	{
		std::string newPath = filename; newPath.replace(pos, 4, "Data2");

		std::vector<BYTE> mibyte;

		fseek(fp, 0, SEEK_END);
		int chunkSize = ftell(fp);
		fseek(fp, 0, SEEK_SET);

		mibyte.resize(chunkSize, 0);

		fread(mibyte.data(), 1, chunkSize, fp);
