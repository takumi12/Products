	...
	if (to != NULL && to->Live && to->Visible)
	{
		...
		if (Range <= 100.f)
		{
			...
			case MODEL_FIRE:
				...
				break;
			case BITMAP_ENERGY:
				CreateParticletime(BITMAP_SPARK + 1, o->Position, o->Angle, Light, 1, 6.f);
				break;
			case MODEL_LIGHTNING_ORB:
			{
				CreateEffecttime(MODEL_LIGHTNING_ORB, o->Position, o->Angle, o->Light, 1);
			}
			break;
			case MODEL_SNOW1:
			{
				for (int j = 0; j < 2; j++)
				{
					CreateEffecttime(MODEL_SNOW2 + rand() % 2, o->Position, o->Angle, o->Light);
					CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light);
				}
			...
			case MODEL_WOOSISTONE:
				if (Range <= 30.f)
				{
					for (int j = 0; j < 20; j++)
					{
						CreateEffecttime(MODEL_WOOSISTONE, o->Position, o->Angle, o->Light, 1);
						CreateParticletime(BITMAP_FIRE, o->Position, o->Angle, o->Light, 0, 1, o);
					}
					PlayBuffer(SOUND_BREAK01);
				}
				else
...
void CreateEffecttime(int Type, vec3_t Position, vec3_t Angle, vec3_t Light, int SubType, OBJECT* Owner, short PKKey, WORD SkillIndex, WORD Skill, WORD SkillSerialNum, float Scale, short int sTargetIndex)
{
	if (time_base_check)
		...
		if (!o->Live)
		{
			...
			case MODEL_WOLF_HEAD_EFFECT2:
			{
				...
				if (o->SubType == 3 || o->SubType == 5)
				{
					...
					o->LifeTime = 4.0f / gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_GIANTSWING].PlaySpeed;
				}
				else if (o->SubType == 4)
				{
					o->Velocity = 0.1f;
			...
			{
				Action_t* Action = gmClientModels->GetModelAction(MODEL_WIND_SPIN02, 0);

				if (Action != NULL)
				{
					Action->PlaySpeed = 0.2;
					o->Velocity = Action->PlaySpeed;
					o->Scale = Scale;
					o->Light[0] = Light[0];
					o->Light[1] = Light[1];
					o->Light[2] = Light[2];
...
void MoveParticle(OBJECT* o, int Turn)
{
	if (Turn)
	{
		...
		//VectorAddTime(o->Position, Position, o->Position);
		interpolatesum(o->Position, Position);
		//o->Position[0] += timefac(Position[0]);
		//o->Position[1] += timefac(Position[1]);
		//o->Position[2] += timefac(Position[2]);
	}
	else
	{
		interpolatesum(o->Position, o->Direction);
		//o->Position[0] += timefac(o->Direction[0]);
		//o->Position[1] += timefac(o->Direction[1]);
		//o->Position[2] += timefac(o->Direction[2]);
		//VectorAddTime(o->Position, o->Direction, o->Position);
	}
}
...
void CreateBomb2(vec3_t p, bool Exp, int SubType, float Scale)
{
	...
	{
		Vector((float)(rand() % 60 + 60 + 90), 0.f, (float)(rand() % 30), Angle);
		CreateParticletime(BITMAP_SPARK, Position, Angle, Light, 2);
	}
	Vector(1.f, 1.f, 1.f, Light);
	Vector(0.f, 0.f, 0.f, Angle);
	if (Exp)
	{
		CreateParticletime(BITMAP_EXPLOTION_MONO, Position, Angle, Light, 0, 4.f + Scale);
	}
	else
	{
		CreateParticletime(BITMAP_EXPLOTION + 1, Position, Angle, Light, 0, 4.f);
	}
}
...
void CreateBomb3(vec3_t vPos, int iSubType, float fScale)
{
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		if (fScale != 1.0f)
			CreateParticletime(BITMAP_SUMMON_SAHAMUTT_EXPLOSION, vBombPos, vAngle, vLight, 0, fScale);
		else
			CreateParticletime(BITMAP_SUMMON_SAHAMUTT_EXPLOSION, vBombPos, vAngle, vLight, 0, 0.15f * (rand() % 10));
	}

	Vector(1.0f, 0.5f, 0.2f, vLight);

	if (fScale != 1.0f)
		CreateParticletime(BITMAP_MAGIC + 1, vBombPos, vAngle, vLight, 0, 2.2f * fScale);
	else
		CreateParticletime(BITMAP_MAGIC + 1, vBombPos, vAngle, vLight, 0, 1.0f);
	...
	{
		if (fScale != 1.0f)
			CreateEffecttime(MODEL_STONE1 + rand() % 2, vBombPos, vAngle, vLight, 14, NULL, -1, 0, 0, 0, 2.2f * fScale);
		else
			CreateEffecttime(MODEL_STONE1 + rand() % 2, vBombPos, vAngle, vLight, 13);
	}
	Vector(1.0f, 1.0f, 1.0f, vLight);
	...
	{
		CreateParticletime(BITMAP_SPARK, vBombPos, vAngle, vLight, 2);
	}
}
...
void CreateInferno(vec3_t Position, int SubType)
{
	...
	for (int j = 0; j < 8; j++)
	{
		...
		if (SubType == 1)
		{
			CreateEffecttime(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 0);
			CreateEffecttime(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 0);
		}
		else if (SubType == 2 || SubType == 3)
		{
			CreateEffecttime(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 10);
			CreateEffecttime(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 10);
			CreateEffecttime(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 10);
		}
		else if (SubType == 5)
		{
			...
			for (int i = 0; i < 3; ++i)
				CreateEffecttime(MODEL_EFFECT_BROKEN_ICE0 + rand() % 3, p2, Angle, Light, 0);
		}
		else
		{
			CreateEffecttime(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 1);
			CreateEffecttime(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 0);
		}
	}
}
	...
	if (!o->Kind || o->SubType == 99)
	{
		if (CheckCharacterRange(o, 100.f, o->PKKey))
		{
			...
			case AT_SKILL_PIERCING:
				if (o->SubType == 2)
				{
					CreateJointtime(BITMAP_FLARE, o->Position, o->Position, o->Angle, 6, o, 30.0f);
				}
				else
				{
					o->Live = false;
...
void MoveEffect(OBJECT* o, int iIndex)
{
	...
	case MODEL_DRAGON:
	{
		...
		if (fTemp <= 800.0f && fTemp >= -800.0f)
		{
			o->Kind = 2;
			o->Distance = 0.0f;
			o->Angle[2] += timefac((0.5f * o->Distance / 2.0f));
			o->CollisionRange = true;
		}
		else
		{
			...
			{
				o->Angle[2] += timefac((0.5f * o->Gravity * 3.0f));
				o->Kind = 1;
			}
	...
	case MODEL_ARROW_AUTOLOAD:
	{
		...
		else if (o->SubType == 1)
		{
			...
			VectorAddTime(o->Position, pOwner->Position, o->Position);
	...
	case MODEL_KENTAUROS_ARROW:
	{
		...
		if (o->LifeTime <= 24)
		{
			...
			VectorScale(o->Direction, timepow(o->Velocity), vPos);
			VectorAdd(o->Position, vPos, o->Position);
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, vLight, 26, 0.2f);
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, vLight, 26, 0.2f);
		}
	...
	case MODEL_BUTTERFLY01:
	{
		...
		else if (o->SubType == 3)
		{
			...
			if (rand() % 2 == 0)
			{
				...
				CreateParticletime(BITMAP_SPARK, Position, o->Angle, o->Light, 7);
			}
		}
			...
			else
			{
				...
				case 5:
				{
					for (int i = 0; i < 3; ++i)
					{
						...
						CreateParticletime(BITMAP_LIGHT, Position, o->Angle, LightFlame, 5, 0.7f);
					}
				}
				break;
				case 2:
	...
	case MODEL_SPEAR:
		if (1 == o->SubType)
		{
			CreateJointtime(BITMAP_FLARE, o->Position, o->Position, o->Angle, 12, o, 100.0f);
		}
		else if (0 == o->SubType)
		{
			CreateJointtime(BITMAP_FLARE, o->Position, o->Position, o->Angle, 4, o, 50.0f);
		}
		break;
	case MODEL_SPEAR + 1:
	{
		...
		for (int j = 0; j < 3; j++)
		{
			...
			CreateJointtime(BITMAP_JOINT_HEALING, Position, o->Position, Angle, 6, NULL, 5.f);
		}
	}
	break;
	...
	case MODEL_HALLOWEEN_CANDY_STAR:
		if (o->SubType == 0 || o->SubType == 1)
		{
			...
			{
				CreateParticletime(BITMAP_FIRE, o->Position, o->Angle, o->Light, 5, 0.4f + rand() % 10 * 0.01f);
			}
		}
		break;
	case MODEL_HALLOWEEN_EX:
	...
	case MODEL_MOONHARVEST_MOON:
		...
		else if (o->SubType == 1)
		{
			...
			CreateParticletime(BITMAP_SMOKELINE1 + rand() % 3, o->Position, o->Angle, o->Light, 0, 1.0f);
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 11, 1.0f);


			if (rand() % 2 == 0)
			{
				CreateParticletime(BITMAP_WATERFALL_3, o->Position, o->Angle, o->Light, 3, 3.f);
			}
		}
		else if (o->SubType == 2)
		{
	...
	case BITMAP_FIRE_CURSEDLICH:
		if (o->SubType == 0)
		{
			...
			if (pModel)
			{
				...
				CreateParticletime(BITMAP_FIRE_CURSEDLICH, o->Position, o->Angle, vLight, 3, 1, pObject);
			}
		}
		else if (o->SubType == 1)
		{
			...
			for (int i = 0; i < 1; ++i)
			{
				...
				{
				case 0:
					CreateParticletime(BITMAP_FIRE_HIK1, vFirePosition, o->Angle, o->Light, 1, fScale);
					break;
				case 1:
					CreateParticletime(BITMAP_FIRE_CURSEDLICH, vFirePosition, o->Angle, o->Light, 5, fScale);
					break;
				case 2:
					CreateParticletime(BITMAP_FIRE_HIK3, vFirePosition, o->Angle, o->Light, 1, fScale);
					break;
				}
			}
		}
		...
		else if (o->SubType == 2)
		{
			...
			if (pModel)
			{
				...
				for (int i = 0; i < 8; ++i)
				{
					...
					{
					case 0:
						CreateParticletime(BITMAP_FIRE_HIK1, vFirePosition, o->Angle, vLight, 0, fScale);
						break;
					case 1:
						CreateParticletime(BITMAP_FIRE_CURSEDLICH, vFirePosition, o->Angle, vLight, 4, fScale);
						break;
					case 2:
						CreateParticletime(BITMAP_FIRE_HIK3, vFirePosition, o->Angle, vLight, 0, fScale);
						break;
					}
				}
			}
		...
		else if (o->SubType == 3)
		{
			for (int i = 0; i < 2; ++i)
			{
				...
				{
				case 0:
					CreateParticletime(BITMAP_FIRE_HIK1, o->Position, o->Angle, o->Light, 0, fScale);
					break;
				case 1:
					CreateParticletime(BITMAP_FIRE_CURSEDLICH, o->Position, o->Angle, o->Light, 4, fScale);
					break;
				case 2:
					CreateParticletime(BITMAP_FIRE_HIK3, o->Position, o->Angle, o->Light, 0, fScale);
					break;
				}
			}
		}
		else if (o->SubType == 4)
		{
			for (int i = 0; i < 2; ++i)
			{
				...
				{
				case 0:
					CreateParticletime(BITMAP_FIRE_HIK1_MONO, o->Position, o->Angle, o->Light, 0, fScale);
					break;
				case 1:
					CreateParticletime(BITMAP_FIRE_HIK2_MONO, o->Position, o->Angle, o->Light, 4, fScale);
					break;
				case 2:
					CreateParticletime(BITMAP_FIRE_HIK3_MONO, o->Position, o->Angle, o->Light, 0, fScale);
					break;
				}
			}
		}
	...
	case MODEL_SUMMONER_EQUIP_HEAD_SAHAMUTT:
		if (o->SubType == 0)
		{
			...
			vec3_t vLight;
			Vector(o->Alpha * 0.3f, o->Alpha * 0.3f, o->Alpha * 0.3f, vLight);
			CreateParticletime(BITMAP_FIRE_CURSEDLICH, o->Position, o->Angle, vLight, 1, 1, pObject);
		}
		else if (o->SubType == 1)
		{
			if (rand() % 2 == 0)
			{
				...
				{
					int iNumBones = pModel->NumBones;
					CreateEffecttime(BITMAP_FIRE_CURSEDLICH, pObject->Position, pObject->Angle, pObject->Light, 0, pObject, -1, rand() % iNumBones);
				}
			}
		}
		break;
	case MODEL_SUMMONER_EQUIP_HEAD_NEIL:
		if (o->SubType == 0)
		{
			...
			CreateParticletime(BITMAP_LIGHT + 2, o->Position, o->Angle, o->Light, 3, 0.30f, pObject);
			if (rand() % 2 == 0)
				CreateParticletime(BITMAP_LIGHT + 2, o->Position, o->Angle, o->Light, 3, 0.30f, pObject);
		}
		else if (o->SubType == 1)
		{
			...
			CreateParticletime(BITMAP_LIGHT + 2, pObject->Position, pObject->Angle, vColor, 5, 0.1f, pObject);
		}
		break;
	case MODEL_SUMMONER_EQUIP_HEAD_LAGUL:
	{
		...
		CreateParticletime(BITMAP_CLUD64, o->Position, o->Angle, vLight, 10, 1.f, pObject);
	}
	case MODEL_SUMMONER_CASTING_EFFECT1:
	case MODEL_SUMMONER_CASTING_EFFECT11:
	case MODEL_SUMMONER_CASTING_EFFECT111:
	...
	case MODEL_SUMMONER_SUMMON_SAHAMUTT:
	{
		...
		if (o->AnimationFrame >= 11.0f && time_base_check)
		{
		...
		if (pModel)
		{
			...
			for (int i = 0; i < 8; ++i)
			{
				...
				Vector(o->Alpha * 0.3f, o->Alpha * 0.3f, o->Alpha * 0.3f, vLight);
				CreateParticletime(BITMAP_FIRE_CURSEDLICH, vPos, o->Angle, vLight, 2, 5, o);
				CreateParticletime(BITMAP_FIRE_CURSEDLICH, vPos, o->Angle, vLight, 2, 4, o);
				CreateParticletime(BITMAP_FIRE_CURSEDLICH, vPos, o->Angle, vLight, 2, 3, o);
			}
		}
	}
	break;
	case MODEL_SUMMONER_SUMMON_NEIL:
	{
		...
		{
			o->Skill = 1;
			CreateEffecttime(MODEL_SUMMONER_SUMMON_NEIL_NIFE1, o->HeadTargetAngle, o->Angle, o->Light, o->SubType);
			if (o->SubType >= 1)
				CreateEffecttime(MODEL_SUMMONER_SUMMON_NEIL_NIFE2, o->HeadTargetAngle, o->Angle, o->Light, o->SubType);
			if (o->SubType >= 2)
				CreateEffecttime(MODEL_SUMMONER_SUMMON_NEIL_NIFE3, o->HeadTargetAngle, o->Angle, o->Light, o->SubType);
		}
		if (o->AnimationFrame > 10 && o->Skill == 1)
		{
			...
			CreateEffecttime(MODEL_SUMMONER_SUMMON_NEIL_GROUND1, vPosition, o->Angle, o->Light, o->SubType, o);
			CreateEffecttime(MODEL_SUMMONER_SUMMON_NEIL_GROUND1, o->HeadTargetAngle, o->Angle, o->Light, o->SubType);

			if (o->SubType >= 1)
				CreateEffecttime(MODEL_SUMMONER_SUMMON_NEIL_GROUND2, o->HeadTargetAngle, o->Angle, o->Light, o->SubType);
			if (o->SubType >= 2)
				CreateEffecttime(MODEL_SUMMONER_SUMMON_NEIL_GROUND3, o->HeadTargetAngle, o->Angle, o->Light, o->SubType);
	...
	case MODEL_SUMMONER_SUMMON_LAGUL:
	{
		if (o->SubType == 0)
		{
			...
			if (rand() % anEffectVolume[o->PKKey] == 0)
			{
				...
				Vector(1.0f, 1.0f, 1.0f, vLight);
				CreateParticletime(BITMAP_SMOKE, vPos, o->Angle, vLight, 57, 3.5f);

				VectorCopy(o->HeadTargetAngle, vPos);
				vPos[0] += ((float)(rand() % 400 - 200));
				vPos[1] += ((float)(rand() % 400 - 200));
				Vector(0.6f, 0.1f, 1.f, vLight);
				CreateEffecttime(BITMAP_CLOUD, vPos, o->Angle, vLight, 0, NULL, -1, 0, 0, 0, 2.0f);

				Vector(0.6f, 0.1f, 1.f, vLight);
				CreateParticletime(BITMAP_TWINTAIL_WATER, vPos, o->Angle, vLight, 1);
			}
		}
		else if (o->SubType == 1)
		{
	...
	break;
	case BITMAP_FIRE:
		CreateParticletime(BITMAP_FIRE, o->Position, o->Angle, Light, 9, 1.f, o->Owner);
		break;
	case BITMAP_FIRE + 1:
		Vector(1.f, 1.f, 1.f, Light);
		CreateParticletime(BITMAP_FIRE + 1, o->Position, o->Angle, Light, 1);
	...
	case BITMAP_MAGIC:
		if (o->SubType == 0)
		{
			CreateEffecttime(BITMAP_MAGIC, o->Position, o->Angle, o->Light, 1);
		...
		else if (o->SubType == 2 || o->SubType == 3 || o->SubType == 7)
		{
			...
			{
				if (o->SubType == 3)
					CreateParticletime(BITMAP_FLARE, o->Position, o->Angle, o->Light, 10, 0.19f, o);
				else
					CreateParticletime(BITMAP_FLARE, o->Position, o->Angle, o->Light, 0, 0.19f, o);
			}
		}
		...
		{
			CreateParticletime(BITMAP_FLARE, o->Position, o->Angle, o->Light, 12, 0.19f, o);
		}
		else if (o->SubType == 8)
		{
		...
		else if (o->SubType == 6)
		{
			...
			if (pModel)
			{
				...
				for (int i = 0; i < o->LifeTime._runvalue; i++)
				{
					...
					CreateParticletime(BITMAP_LIGHT, Position, o->Owner->Angle, Light, 8);
				}
			}
	...
	case BITMAP_SHINY + 6:
		...
		case 0:
			...
			CreateParticletime(o->Type, Position, o->Angle, o->Light, 0, o->Scale);
			break;
		case 1:
		case 2:
			if (o->Owner == NULL || o->Owner->Live == false)
			...
			else
			{
				...
				if (pModel)
				{
					...
					if (!pModel->Bones[iBone].Dummy)
					{
						...
						{
							vWorldPos[2] -= (20.f);
							CreateParticletime(o->Type, vWorldPos, o->Angle, o->Light, 0, o->Scale);
						}
					}
				}
			}
		...
		case 3:
		{
			...
			if (pModel)
			{
				...
				VectorAddTime(dp, vFirePosition, Position);
	...
	case BITMAP_PIN_LIGHT:
		...
		case 0:
			...
			{
				if (o->SubType == 3)
					CreateParticletime(o->Type, Position, o->Angle, o->Light, 1, o->Scale);
				else
					CreateParticletime(o->Type, Position, o->Angle, o->Light, 0, o->Scale);
			}
			break;
	...
	case BITMAP_ORORA:
		...
		else
		{
			...
			{
				o->Live = false;
				CreateEffecttime(o->Type, o->Position, o->Angle, o->Light, o->SubType, o->Owner);
			}
		}
		break;
	case BITMAP_SPARK + 2:
	...
	case BITMAP_GATHERING:
		...
		for (int j = 0; j < 3; ++j)
		{
			...
			if (o->SubType == 1)
			{
				...
				{
					CreateParticletime(BITMAP_SPARK + 1, Position, Angle, Light, 2, (rand() % 50 + 10) / 100.f, o);
				}
			...
			{
				CreateParticletime(BITMAP_SPARK + 1, Position, Angle, o->Light, 26, (rand() % 10 + 5) / 25.f, o);
			}
			else
			{
	...
	case BITMAP_JOINT_THUNDER:
	{
		...
		if (rand() % 10 > 4)
		{
			if (o->SubType == 1)
			{
				CreateJointtime(BITMAP_JOINT_THUNDER, vStartPosition, vPosition, o->Angle, 33, NULL, fScale);
			}
			else
			{
				CreateJointtime(BITMAP_JOINT_THUNDER, vStartPosition, vPosition, o->Angle, 16, NULL, fScale);
			}
		}
		...
		{
			if (rand() % 2 == 0)
				CreateEffecttime(BITMAP_MAGIC + 1, vPosition, o->Angle, vLight, 11, o);
		}

		if (rand() % 4 == 0)
			CreateParticletime(BITMAP_SMOKE, vPosition, o->Angle, vLight, 54, 2.8f);

		if (rand() % 4 == 0)
		{
			CreateEffecttime(MODEL_STONE1 + rand() % 2, o->Position, o->Angle, vLight, 13);
		}
		...
		if (o->LifeTime > 5)
		{
			...
			if (rand() % 5 == 0)
				CreateEffecttime(BITMAP_CHROME_ENERGY2, vPosition, o->Angle, vLight, 0);
		}
		...
		if (o->Owner != NULL && rand() % 5 == 0 && o->Owner->BoneTransform != NULL)
		{
			...
			for (int i = 0; i < iNumBones; ++i)
			{
				...
				CreateParticletime(BITMAP_LIGHT, vPos, vAngle, vLight, 5, fRandom);
			}
		}
	}
	break;
	...
	case BITMAP_FLAME:
		if (o->SubType == 0)
		{
			for (int j = 0; j < 6; j++)
			{
				...
				VectorAdd(Position, o->Position, Position);
				CreateParticletime(BITMAP_FLAME, Position, o->Angle, Light);
			}
			if (rand() % 8 == 0)
				CreateEffecttime(MODEL_STONE1 + rand() % 2, o->Position, o->Angle, o->Light);
			...
			if (o->Owner == &Hero->Object && o->LifeTime.duration(20))
			{
		...
		else if (o->SubType == 1 || o->SubType == 2)
		{
			for (int j = 0; j < 18; j++)
			{
				...
				if (o->SubType == 1)
					CreateParticletime(BITMAP_FLAME, Position, o->Angle, Light, 0, 1.2f);
				else if (o->SubType == 2)
					CreateParticletime(BITMAP_FIRE + 3, Position, o->Angle, Light, 13, 2.5f);
			}
		}
		else if (o->SubType == 3)
		{
			for (int j = 0; j < 3; j++)
			{
				CreateParticletime(BITMAP_FLAME, o->Position, o->Angle, Light, 6);
				Vector((float)(rand() % 10 - 5), (float)(rand() % 10 - 5), 40.f, Position);
				VectorAdd(Position, o->Position, Position);
				CreateParticletime(BITMAP_TRUE_FIRE, Position, o->Angle, Light, 0, 2.8f);
			}
			Vector((float)(rand() % 10 - 5), (float)(rand() % 10 - 5), -40.f, Position);
			VectorAdd(Position, o->Position, Position);
			CreateParticletime(BITMAP_SMOKE, Position, o->Angle, Light, 21, 0.8f);

			CreateEffecttime(MODEL_STONE1 + rand() % 2, o->Position, o->Angle, o->Light, 12);
		...
		else if (o->SubType == 5)
		{
			...
			VectorAdd(Position, o->Position, Position);
			CreateParticletime(BITMAP_FLAME, Position, o->Angle, Light, 0, o->Scale);
		}
		else if (o->SubType == 6)
		{
			Vector((float)(rand() % 32 - 16), (float)(rand() % 32 - 16), 0.f, Position);
			VectorAdd(Position, o->Position, Position);
			CreateParticletime(BITMAP_FLAME, Position, o->Angle, o->Light, 12, o->Scale);
		}
		break;
	case MODEL_RAKLION_BOSS_CRACKEFFECT:
		if (o->SubType == 0)
	...
	case BITMAP_FIRE_HIK2_MONO:
		if (o->SubType == 0)
		{
			for (int i = 0; i < 2; ++i)
			{
				...
				if (rand() % 3 != 0)
				{
					CreateParticletime(BITMAP_FLAME, Position, o->Angle, o->Light, 11, 1.4f);
				}

				if (rand() % 8 == 0)
				{
					CreateEffecttime(MODEL_ICE_SMALL, Position, o->Angle, o->Light, 0);
				}
			}
		}
		else if (o->SubType == 1)
	...
	case BITMAP_FIRE_RED:
		...
		CreateParticletime(BITMAP_FIRE_RED, Position, o->Angle, Light, 0, o->Scale);
		break;
	case BITMAP_SPARK + 1:
	{
		vec3_t Position;
		...
		for (int j = 0; j < 18; j++)
		{
			...
			if (j == 0)
				CreateParticletime(BITMAP_SPARK + 1, Position, o->Angle, Light, 1, Scale * 2.f);
			else
				CreateParticletime(BITMAP_SPARK + 1, Position, o->Angle, Light, 1, Scale);
		}
		break;
	}
	case BITMAP_ENERGY:
		if (o->SubType == 0)
		{
			...
			Vector(Luminosity, Luminosity, Luminosity, Light);
			CreateParticletime(BITMAP_ENERGY, o->Position, o->Angle, Light);
			CreateParticletime(BITMAP_SPARK + 1, o->Position, o->Angle, Light, 0, 4.f);
	...
	case MODEL_LIGHTNING_ORB:
	{
		...
		if (o->SubType == 0)
		{
			...
			CreateParticletime(BITMAP_MAGIC, o->Position, o->Angle, vLight, 0, 1.f);
			for (int i = 0; i < 3; ++i)
			{
				CreateParticletime(BITMAP_SPARK + 1, o->Position, o->Angle, o->Light, 13, 1.0f);
			}
			//CheckTargetRange(o);
		}
		else if (o->SubType == 1)
		{
			...
			if (o->LifeTime >= 15)
			{
				for (int i = 0; i < 5; ++i)
				{
					CreateParticletime(BITMAP_SPARK + 1, o->Position, o->Angle, o->Light, 20, 1.0f);
				}
			}
			// shockwave
			if (o->LifeTime >= 14)
			{
				...
				CreateParticletime(BITMAP_SHOCK_WAVE, o->Position, o->Angle, vLight, 0, 0.3f);
				CreateParticletime(BITMAP_SHOCK_WAVE, o->Position, o->Angle, vLight, 0, 0.3f);
			}
			...
			for (int i = 0; i < 2; ++i)
			{
				CreateEffecttime(MODEL_FENRIR_THUNDER, o->Position, o->Angle, vLight, 3, o);
			}
			...
			if (o->LifeTime <= 5)
			{
				for (int i = 0; i < 2; ++i)
				{
					CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 40);
				}
			}
	...
	case MODEL_CHAIN_LIGHTNING:
	{
		...
		case 2:
		{
			...
			if (pSourceObj == NULL || pSourceObj->Live == false
				|| pTargetObj == NULL || pTargetObj->Live == false || pTargetChar->Dead > 0)
			{
				o->LifeTime = 0;
				o->Live = false;
				break;
			...
			if (pSourceModel && pTargetModel)
			{
				...
				if (o->SubType == 0)
				{
					...
					CreateJointtime(BITMAP_JOINT_THUNDER, vPos, pTargetObj->Position, vAngle, 0, pTargetObj, 50.f, -1, 0, 0, -1, vLight);
					CreateJointtime(BITMAP_JOINT_THUNDER, vPos, pTargetObj->Position, vAngle, 0, pTargetObj, 10.f, -1, 0, 0, -1, vLight);
				...
				else if (o->SubType == 1 || o->SubType == 2)
				{
					...
					vTargetPos[2] += (80.0f);
					CreateJointtime(BITMAP_JOINT_THUNDER, vPos, pTargetObj->Position, pTargetObj->Angle, 0, pTargetObj, 50.f, -1, 0, 0, -1, vLight);
					CreateJointtime(BITMAP_JOINT_THUNDER, vPos, pTargetObj->Position, pTargetObj->Angle, 0, pTargetObj, 10.f, -1, 0, 0, -1, vLight);
				}
	...
	case MODEL_ALICE_DRAIN_LIFE:
	{
		...
		if (pSourceModel && pTargetModel)
		{
			...
			for (int i = 0; i < iCnt; i++)
			{
				...
				CreateParticletime(BITMAP_LIGHT + 2, vSourcePos, pSourceObj->Angle, vLight, 7, 1.8f);
			}
			...
			if (o->LifeTime <= 60)
			{
				for (int i = 0; i < iCnt; i++)
				{
					...
					Vector(1.0f, 0.2f, 0.2f, vLight);
					CreateParticletime(BITMAP_LIGHT + 2, vTargetPos, pTargetObj->Angle, vLight, 7, 1.8f);
				}
			}
			...
			if (o->LifeTime >= 66 && o->LifeTime <= 70)
			{
				...
				for (int i = 0; i < iCnt2; i++)
				{
					...
					CreateJointtime(BITMAP_DRAIN_LIFE_GHOST, vSourcePos, vTargetPos, o->Angle, 0, pSourceObj, 40.f, 0, 0, 0, -1, vLight);
				}
			}
	...
	case MODEL_ALICE_BUFFSKILL_EFFECT2:
	{
		if (o->SubType == 0 || o->SubType == 1 || o->SubType == 2)
		{
			...
			for (int i = 0; i < 3; ++i)
			{
				...
				if (o->SubType == 0 || o->SubType == 2)
				{
					CreateJointtime(BITMAP_JOINT_HEALING, vWorldPos, o->Position, vAngle, 15, o, 5.f, 0, 0, 0, 0, vLight);
				}
				else if (o->SubType == 1)
				{
					CreateJointtime(BITMAP_JOINT_HEALING, vWorldPos, o->Position, vAngle, 16, o, 5.f, 0, 0, 0, 0, vLight);
				}
			}
		}
		else if (o->SubType == 3 || o->SubType == 4)
		{
			...
			else
			{
				...
				if (!pModel->Bones[iBone].Dummy)
				{
					...
					if (o->SubType == 3)
					{
						CreateParticletime(BITMAP_LIGHT + 2, vWorldPos, o->Angle, o->Light, 6, o->Scale);
					...
					else if (o->SubType == 4)
					{
						...
						{
							vWorldPos[2] -= (20.f);
							CreateParticletime(BITMAP_TWINTAIL_WATER, vWorldPos, o->Angle, o->Light, 2);
						}
					}
				}
			}
	...
	case MODEL_LIGHTNING_SHOCK:
	{
		...
		if (o->SubType == 0)
		{
			...
			{
				Vector(0.f, -20.f, -75.f - timefac(o->Velocity), o->Direction);
			...
			if (pOwnerModel)
			{
				...
				CreateParticletime(BITMAP_MAGIC, o->Position, o->Angle, vLight, 0, 1.f * fScale);
				...
				for (int i = 0; i < 11; ++i)
				{
					...
					if (rand() % 3 > 0)
						continue;
					CreateParticletime(BITMAP_LIGHTNING_MEGA1 + rand() % 3, vPos, pObject->Angle, vLight, 0, fScale);
				}
		...
		else if (o->SubType == 1)
		{
			...
			for (int i = 0; i < 11; ++i)
			{
				fScale = (float)(rand() % 80 + 32) * 0.01f * 1.0f;
				Vector(o->Position[0] + (rand() % 70 - 35) * 1.0f, o->Position[1] + (rand() % 70 - 35) * 1.0f, o->Position[2] + (rand() % 70 - 35) * 1.0f, vPos);
				CreateParticletime(BITMAP_LIGHTNING_MEGA1 + rand() % 3, vPos, pObject->Angle, vLight, 0, fScale);	// 전기
			}
			...
			for (int i = 0; i < 6; ++i)
			{
				...
				AngleMatrix(vAngle, Matrix);
				VectorRotate(vDirection, Matrix, vPosition);
				VectorAddTime(vPosition, o->Position, vPosition);
				vPosition[2] = RequestTerrainHeight(vPosition[0], vPosition[1]) + 20;
				CreateParticletime(BITMAP_LIGHTNING_MEGA1 + rand() % 3, vPosition, pObject->Angle, vLight, 0, fScale);	// 전기
			}
			...
			for (int i = 0; i < 2; i++)
				CreateParticletime(BITMAP_SMOKE, vPosition, o->Angle, vLight, 58);
			...
			CreateEffecttime(MODEL_STONE1 + rand() % 2, vPosition, o->Angle, vLight, 13, o);
		}
		else if (o->SubType == 2)
		{
			if (o->Owner != NULL && o->Owner->BoneTransform != NULL)
			{
				...
				for (int i = 0; i < iNumBones; i++)
				{
					...
					if (rand() % 5 == 0)
					{
						...
						CreateParticletime(BITMAP_LIGHTNING_MEGA1 + rand() % 3, vPos, vAngle, vLight, 0, fRandom);
					}
				}

				Vector(1.0f, 0.0f, 0.0f, vLight);
				CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, vLight, 58);

				if (rand() % 4 == 0)
				{
					Vector(1.0f, 0.0f, 0.0f, vLight);
					CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, vLight, 54, 2.8f);
				}
			}
		}
	}
	...
	case BITMAP_LIGHT:
		if (o->SubType == 0)
		{
			...
			CreateParticletime(BITMAP_LIGHT, o->Position, o->Angle, o->Light, 1, o->Scale);
		...
		else if (o->SubType == 1)
		{
			...
			CreateParticletime(BITMAP_LIGHT, o->Position, Angle, Light, 4, Luminosity, o->Owner);
		...
		else if (o->SubType == 2)
		{
			...
			if (((int)(o->Velocity) % 50) == 0)
			{
				if (o->Owner != NULL)
				{
					...
					CreateJointtime(BITMAP_JOINT_HEALING, o->Position, o->Position, o->Angle, 9, o->Owner, 15.f);
				}
			}
		}
		else if (o->SubType == 3)
		{
			...
			if (o->LifeTime == (15) || o->LifeTime == (29))
			{
				if (o->Owner != NULL)
				{
					vec3_t Position;
	...
	case MODEL_SKILL_INFERNO:
		...
		{
			o->Scale += (timefac(0.01f));
			o->BlendMeshLight = o->LifeTime._runvalue / 5.f * (0.1f);
	...
	case MODEL_BIG_METEO3:
		...
		if ((TerrainWall[Index] & TW_NOGROUND) != TW_NOGROUND)
		{
			...
			if (o->Position[2] < Height)
			{
				...
				for (int j = 0; j < 5; j++)
					CreateEffecttime(MODEL_METEO1 + rand() % 2, o->Position, o->Angle, o->Light, 0);
			}
		}
		VectorAdd(o->Position, o->Direction, o->Position);
		o->Angle[0] += timefac(10.f / o->Scale);
		CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 3);
		return;
	case MODEL_PROTECT:
	...
	case MODEL_MAGIC1:
		...
		break;
	case MODEL_SKILL_WHEEL1:
		CreateEffect(MODEL_SKILL_WHEEL2, o->Position, o->Angle, o->Light, 4 - o->LifeTime._runvalue, o->Owner, o->PKKey, o->Skill, o->Kind);
		break;
	case MODEL_SKILL_WHEEL2:
		switch (o->SubType)
		{
		...
		if (gMapManager.InHellas())
		{
			...
			CreateParticletime(BITMAP_WATERFALL_5, Position, Angle, Light, 3);
		}
		else
		{
			for (int j = 0; j < 4; j++)
		...
		if (gMapManager.InHellas() && o->LifeTime.duration(4))
		{
			int PositionX = (int)(o->Position[0] / TERRAIN_SCALE);
			int PositionY = (int)(o->Position[1] / TERRAIN_SCALE);
	...
	case MODEL_SKILL_FISSURE:
	{
		if (o->LifeTime == 8)
		{
			for (int i = 0; i < 16; ++i)
			{
				...
				CreateJointtime(BITMAP_FLARE, Position, Position, Angle, 24, NULL, 90);
			}
		}
	...
	case MODEL_FISSURE:
	{
		...
		if (rand() % 2 == 0)
		{
			...
			VectorRotate(p, Matrix, Position);
			VectorAdd(Position, o->Position, Position);
			CreateEffecttime(MODEL_STONE1 + rand() % 2, Position, o->Angle, o->Light, 0);
		}
		...
		if (pModel)
		{
			...
			for (int i = 0; i < pModel->NumBones; i++) {
				if (!pModel->Bones[i].Dummy && rand() % 12 == 0)
				{
					...
					CreateParticletime(BITMAP_TRUE_FIRE, Position, o->Angle, Light, 3, 4.f, o);
					CreateParticletime(BITMAP_SMOKE, Position, o->Angle, Light, 21, 1.f);
				}
			}
		}
	}
	...
	case MODEL_SKILL_FURY_STRIKE + 1:
		...
		if (o->LifeTime > 15 && o->LifeTime.duration(3))
		{
	...
	case MODEL_CHANGE_UP_EFF:
	{
		...
		if (rand() % STwo == 0)
		{
			...
			if (o->SubType == 0 || o->SubType == 2)
			{
				CreateJointtime(BITMAP_FLARE, Loc, Loc, o->Angle, 50, NULL, 40);
			}

			if (o->LifeTime > 40)
				CreateEffecttime(BITMAP_MAGIC, o->Position, o->Angle, Light, 4, o, 4.0f);
	...
	case MODEL_DARK_ELF_SKILL:
	{
		...
		for (int j = 0; j < 6; j++)
		{
			CreateParticletime(BITMAP_SMOKE, Position, Angle, o->Light, 25);
		}
		Vector(Luminosity * 0.3f, Luminosity * 0.6f, Luminosity, Light);
	...
	case MODEL_MAGIC2:
		...
		if (o->SubType == 2)
		{
			...
			VectorCopy(o->Position, Position);
			Angle[2] += (rand() % 10 - 5);
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, Light, 11, (float)(rand() % 32 + 80) * 0.015f);
	...
	case MODEL_STORM:
		...
		case 0:
			...
			if (rand() % 2 == 0)
			{
				...
				CreateJointtime(BITMAP_JOINT_THUNDER, Position, o->Position, Angle, 0, o, 10.f);
			}
			if (rand() % 2 == 0)
			{
				Vector(o->Position[0] + 200.f, o->Position[1], o->Position[2] + 700.f, Position);
			...
			o->Position[2] = RequestTerrainHeight(o->Position[0], o->Position[1]);
			if (rand() % 4 == 0)
				CreateEffecttime(MODEL_STONE1 + rand() % 2, o->Position, o->Angle, o->Light, 2);
		...
		case 1:
			...
			VectorCopy(o->Position, Position);
			Position[2] += (100.f);
			CreateParticletime(BITMAP_SMOKE, Position, o->Angle, o->Light, 3);
			CreateParticletime(BITMAP_BUBBLE, Position, o->Angle, o->Light, 3, 0.1f);
		...
		case 7:
			EarthQuake = (float)(rand() % 8 - 4) * 0.1f;
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 28);
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 29);
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 30);
			if (rand() % 2 == 0)
				CreateEffecttime(MODEL_STONE1 + rand() % 2, o->Position, o->Angle, o->Light, 2);
			break;
		case 8:
	...
	case MODEL_STORM2:
	{
		...
		else if (o->SubType == 1)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				CreateParticletime(BITMAP_CLUD64, Position, o->Angle, Light, 1, 1.0f);
			}
			...
			CreateJointtime(BITMAP_JOINT_THUNDER, Pos1, Pos2, Angle, 20, o, 20.f);

			Pos2[0] = Pos1[0] + rand() % 100 - 50.0f;
			Pos2[1] = Pos1[1] + rand() % 100 - 50.0f;
			Pos2[2] = Pos1[2];
			CreateEffecttime(MODEL_STONE1 + rand() % 2, Pos2, o->Angle, Light, 2);
	...
	case MODEL_MAYASTONE3:
	{
		...
		if (success)
		{
			...
			Vector(0.f, 0.f, 0.f, Angle);
			CreateEffecttime(MODEL_SKILL_INFERNO, o->Position, Angle, Light, 2, o, 30, 0);
			...
			for (int i = 0; i < smokeNum; ++i)
			{
				...
				CreateParticletime(BITMAP_SMOKE, Position, o->Angle, Light, 11, (float)(rand() % 32 + 80) * 0.025f);
			}

			for (int j = 0; j < 6; j++)
				CreateEffecttime(MODEL_MAYASTONE4 + rand() % 2, o->Position, o->Angle, Light);

			CreateParticletime(BITMAP_EXPLOTION, Position, o->Angle, Light, 0, 4.0f);
			o->Live = false;
		}
	}
	break;
	...
	case MODEL_MAYASTONE5:
	{
		...
		if (rand() % 10 == 0)
			CreateParticletime(BITMAP_FIRE, o->Position, o->Angle, o->Light, 15);
	}
	break;
	case MODEL_MAYASTONEFIRE:
	{
	...
	case MODEL_MAYAHANDSKILL:
	{
		if (o->SubType == 0)
		{
			...
			VectorCopy(o->Position, vPos);
			vPos[2] += (100.f);
			CreateParticletime(BITMAP_SMOKE, vPos, o->Angle, o->StartPosition, 17, 10.0f);
			...
			CreateEffecttime(MODEL_MAYAHANDSKILL, o->Position, o->Angle, o->StartPosition, 1, NULL, -1, 0, 0, 0, o->Scale + 0.2f);
		...
		if (o->SubType == 1)
		{
			if (o->LifeTime > (44 - o->Owner->m_bySkillCount))
			{
				...
				for (int i = 0; i < iCount; ++i)
				{
					...
					VectorCopy(o->Position, Position);
					Position[2] += (100.f);
					CreateJointtime(BITMAP_JOINT_SPIRIT, Position, Position, Angle, 6, o, 60.f, 0, 0);
		...
		else if (o->SubType == 4)
		{
			if (o->LifeTime > (44 - o->Owner->m_bySkillCount))
			{
				...
				for (int i = 0; i < 36; ++i)
				{
					...
					VectorCopy(o->Position, Position);
					Position[2] += (100.f);
					CreateJointtime(BITMAP_JOINT_SPIRIT, Position, Position, Angle, 22, o, 2.0f, 0, 0);
					CreateJointtime(BITMAP_JOINT_SPIRIT, Position, Position, Angle, 23, o, 1.75f, 0, 0);
				}
			}
		}
		break;
	case MODEL_CIRCLE_LIGHT:
		if (o->SubType != 3 && o->SubType != 4)
		{
			...
			if (rand() % value == 0)
			{
				...
				VectorRotate(p, Matrix, Position);
				VectorAdd(Position, o->Position, Position);
				CreateEffecttime(MODEL_STONE1 + rand() % 2, Position, o->Angle, o->Light, 1);
			}
		...
		else if (o->SubType == 3)
		{
			...
			if (o->LifeTime > 30 && rand() % value == 0)
			{
				...
				CreateParticletime(BITMAP_FLARE_BLUE, Position, o->Angle, o->Light, 0);
				if (o->LifeTime > 40)
				{
					Position[2] += (600.f);
					Angle[2] = 45.f;
					CreateJointtime(BITMAP_FLARE_BLUE, Position, Position, Angle, 19, NULL, 40);
				}
			}
	...
	case MODEL_EFFECT_SAPITRES_ATTACK_2:
		...
		if (o->SubType == 0 || o->SubType == 10 || o->SubType == 12 || o->SubType == 13)
		{
			...
			if ((o->SubType == 0 || o->SubType == 12 || o->SubType == 13) && rand() % 10 == 0)
			{
				...
				{
					CreateParticletime(BITMAP_FIRE, o->Position, o->Angle, o->Light, 1 + rand() % 3);
				}
			}
		}
		else if (o->SubType == 1)
	...
	case MODEL_NARCONDRA_STONE3:
		...
		else if (o->SubType == 1)
		{
			...
			if (bSuccess)
			{
				...
				CreateEffecttime(MODEL_SKILL_INFERNO, o->Position, Angle, Light, 2, o);
				...
				for (int i = 0; i < 8; ++i)
				{
					...
					CreateParticletime(BITMAP_SMOKE, vPos, o->Angle, Light, 11, (float)(rand() % 40 + 60) * 0.025f);
				}

				for (int j = 0; j < 6; j++)
				{
					CreateEffecttime(MODEL_EFFECT_BROKEN_ICE0 + rand() % 3, vPos, o->Angle, Light, 0);
				}
				...
				CreateParticletime(BITMAP_EXPLOTION_MONO, vPos, o->Angle, Light, 1, 1.5f);
		...
		else if (o->SubType == 2)
		{
			...
			if (o->Position[2] < fHeight)
			{
				CreateEffecttime(o->Type, o->Position, o->Angle, o->Light, 0);
				o->Live = false;
			}
		}
		break;
	...
	case MODEL_SKULL:
		if (o->SubType == 1 && o->Owner != NULL)
		{
			...
			CreateParticletime(BITMAP_FIRE, o->Position, o->Angle, Light, 5, 1.2f);
				...
				else
				{
					...
					if (bUseEarthQuake == TRUE)
					{
						...
						for (int i = 0; i < 100; ++i)
						{
							...
							VectorCopy(o->Angle, Angle);
							Angle[2] += (rand() % 180);
							CreateParticletime(BITMAP_WATERFALL_5, Position, Angle, Light, 2);
						}
					}
					VectorCopy(o->Position, Position);
					...
					Position[2] -= (50.f);
					CreateParticletime(BITMAP_BUBBLE, Position, o->Angle, o->Light, 4);
			...
			else
			{
				if (o->SubType != 2 && o->LifeTime > 200 + 50 + 135)
				{
					...
					if (rand() % 2 == 0)
					{
						...
						if (rand() % 2 == 0)
							CreateJointtime(BITMAP_JOINT_SPIRIT2, Position, Position, Angle, 8, NULL, 50.f, 0, 0);
					}
					...
					if (rand() % 3 == 0)
					{
						vec3_t Light;
						Vector(0.5f, 0.5f, 0.5f, Light);
						CreateParticletime(BITMAP_SMOKE + 3, o->Position, o->Angle, Light, 2, (float)(rand() % 32 + 48) * 0.02f);
					}
				}
			}
			break;
	...
	case MODEL_WATER_WAVE:
		...
		for (int j = 0; j < 4; j++)
		{
			CreateParticletime(BITMAP_WATERFALL_5, o->Position, o->Angle, Light, 1);
		}
	...
	case MODEL_STAFF + 8:
		...
		if (o->Position[2] < Height)
		{
			VectorCopy(o->Position, Position);
			Position[2] += (80);
			CreateParticletime(BITMAP_EXPLOTION, Position, o->Angle, Light);
	...
	case MODEL_PIERCING:
		...
		if (o->SubType == 1)
		{
			...
			CreateJointtime(BITMAP_JOINT_THUNDER, o->Position, o->Position, o->Angle, 3, NULL, 20.f, 7); //  전기 
	...
	case MODEL_PIERCING + 1:
		if ((rand() % 2) == 0)
		{
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, Light, 0);
		}
		CreateParticletime(BITMAP_FIRE, o->Position, o->Angle, Light, 5);
		CreateParticletime(BITMAP_FIRE, o->Position, o->Angle, Light, 5);
	...
	case MODEL_ARROW_HOLY:
		if (o->SubType == 1)
		{
			...
			if (o->LifeTime == 13)
				CreateEffecttime(MODEL_PIERCING, o->Position, o->Angle, o->Light, 3, o);
			CheckClientArrow(o);
		...
		else
		{
			...
			if (o->SubType != 0)
			{
				if (o->LifeTime == 13)
				{
					CreateEffecttime(MODEL_PIERCING, o->Position, o->Angle, o->Light, 0, o);
				}
	...
	case MODEL_ARROW:
		...
		else if (o->SubType == 5)
		{
			...
			CreateParticletime(BITMAP_FIRE, o->Position, o->Angle, Light);
			CheckClientArrow(o);
		}
		else
		{
	...
	case MODEL_LACEARROW:
		if (o->Type == MODEL_ARROW_NATURE)
		{
			if (o->SubType == 1)
			{
				...
				if (o->LifeTime > 25 && o->LifeTime.duration(2))
				{
					for (int j = 0; j < 2; j++)
					{
			...
			else
			{
				...
				for (int j = 0; j < 4; j++)
				{
					Vector((float)(rand() % 32 - 16), (float)(rand() % 64 - 32), (float)(rand() % 32 - 16), Position);
					VectorAdd(Position, o->Position, Position);
					CreateParticletime(BITMAP_FLOWER01 + rand() % 3, Position, o->Angle, o->Light);
				}
				o->Angle[1] += timefac(30.f);
		...
		else if (o->Type == MODEL_LACEARROW)
		{
			...
			VectorCopy(o->Position, o->EyeLeft);
			CreateEffecttime(MODEL_WAVES, o->Position, o->Angle, o->Light, 3, NULL, 0);
		...
		else if (o->Type == MODEL_ARROW_WING)
		{
			...
			for (int j = 0; j < 4; j++)
			{
				Vector((float)(rand() % 16 - 8), (float)(rand() % 16 - 8), (float)(rand() % 16 - 8), Position);
				VectorAdd(Position, o->Position, Position);
				CreateParticletime(BITMAP_BUBBLE, Position, o->Angle, o->Light, 1);
			}
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 0);
	...
	case MODEL_DARK_SCREAM:
	{
		...
		CreateParticletime(BITMAP_FLAME, o->Position, o->Angle, o->Light, 8, (o->Scale - 0.4f) * 3.5f);
		if (o->Type == MODEL_DARK_SCREAM)
		{
			CheckClientArrow(o);
		}
	...
	case MODEL_CURSEDTEMPLE_PRODECTION_SKILL:
	{
		...
		if (effbmd)
		{
			...
			CreateEffecttime(BITMAP_SHOCK_WAVE, o->Owner->Position, o->Angle, o->Light, 10, o->Owner);
		}
	}
	break;
	case MODEL_CURSEDTEMPLE_RESTRAINT_SKILL:
	...
	case MODEL_ARROW_RING:
	{
		...
		CreateEffecttime(MODEL_WAVES, o->Position, o->Angle, o->Light, 4, NULL, 0);
	...
	case MODEL_ARROW_GAMBLE:
	{
		...
		for (int j = 0; j < 2; j++)
		{
			Vector((float)(rand() % 32 - 16), (float)(rand() % 64 - 32), (float)(rand() % 32 - 16), Position);
			VectorAdd(Position, o->Position, Position);
			CreateParticletime(BITMAP_SPARK + 1, Position, o->Angle, vLight, 30, 1.f);
		}

		VectorCopy(o->Position, o->EyeLeft);
		CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, vLight, 24, 1.0f);
		...
		case 2:
			if (o->Owner != NULL)
			{
				...
				if (o->SubType == 1)
				{
					...
					Vector(1.f, 1.f, 1.f, Light);
					CreateParticletime(BITMAP_FIRE + 2, Position, o->Angle, Light, 10, 2.f);
				}
			}
			else
			{
	...
	case MODEL_FIRE:
		...
		{
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Direction, 36, 1.0f + o->Scale, o);
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Direction, 37, 2.0f + o->Scale, o);
		...
		if (o->SubType == 5)
		{
			...
			if ((rand() % 5) != 0)
			{
				CreateParticletime(BITMAP_POUNDING_BALL, o->Position, o->Angle, Light);
			}
		}
		else
		{
			...
			if (o->SubType == 6 || o->SubType == 7)
			{
				for (int j = 0; j < 2; j++)
				{
					...
					Position[1] = o->Position[1];
					Position[2] = o->Position[2];
					CreateParticletime(BITMAP_FIRE, Position, o->Angle, Light, 5);
				}
			}
			else if (o->SubType != 3)
			{
			...
			else
			{
				...
				Vector(1.0f, 1.0f, 1.0f, Light);
				CreateParticletime(BITMAP_FIRE, Pos, Angle, Light, 8, o->Scale * 2.5f);
	...
	case BITMAP_FIRECRACKER:
		...
		CreateParticletime(BITMAP_FIRECRACKER, o->Position, o->Angle, o->Light, -1);
	...
	case BITMAP_FIRECRACKER0002:
	{
		if (rand() % 5 == 0)
		{
			...
			CreateEffecttime(BITMAP_FIRECRACKER0003, Position, o->Angle, vLight, 0, o, -1, 0, 0, 0, fScale);
		}
	}
	break;
	case BITMAP_FIRECRACKER0003:
	...
	case BITMAP_BLIZZARD:
		if (o->LifeTime <= 15)
		{
			if (o->SubType == 0)
			{
				...
				CreateParticletime(BITMAP_FIRE + 2, o->Position, o->Angle, Light, 7, o->Scale);
			...
			else if (o->SubType == 1)
			{
				...
				CreateParticletime(BITMAP_FIRE + 2, o->Position, o->Angle, Light, 11, o->Scale);
				...
				if (o->Position[2] < Height)
				{
					...
					if ((rand() % 5) == 0)
						CreateEffecttime(MODEL_ICE_SMALL, Position, o->Angle, o->Light);
				}
			}
		}
		break;
	case BITMAP_SHOTGUN:
	{
		...
		VectorRotate(p, Matrix, Position);
		VectorAdd(o->Position, Position, o->StartPosition);
		CreateParticletime(BITMAP_FIRE + 2, o->StartPosition, o->Angle, Light, 10, o->Scale);
	...
	case MODEL_SHINE:
		if (o->SubType == 0 && o->Owner != NULL && o->LifeTime > 10)
		{
			...
			CreateParticletime(BITMAP_SHINY, Position, o->Angle, o->Light, 2, Scale);
		}
		break;
	case MODEL_BLIZZARD:
		if (o->SubType == 0)
	...
	case MODEL_ARROW_DRILL:
		if (o->SubType == 0 || o->SubType == 2)
		{
			...
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, Light, 13);
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, Light, 13);
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, Light, 13);
	...
	case MODEL_PIERCING2:
		...
		if (o->SubType == 1)
		{
			if (o->LifeTime > 1)
			{
				...
				CreateEffecttime(MODEL_WAVES, o->Position, o->Angle, o->Light, 2, NULL, (int)o->LifeTime);
			}
		}
		else if (o->SubType == 2)
		{
			...
			if (o->Owner != NULL)
			{
				...
				for (int i = 1; i < o->Gravity; ++i)
				{
					...
					if (o->LifeTime < 10)
					{
						...
						CreateEffecttime(BITMAP_MAGIC + 1, Position, o->Angle, o->Light, 1, o);
					}

					AngleMatrix(o->Angle, Matrix);
					VectorRotate(o->Direction, Matrix, Position);
					VectorAdd(o->Position, Position, o->Position);
					CreateEffecttime(MODEL_TAIL, o->Position, o->Angle, o->Light, 0, o);
				}
	...
	case MODEL_GROUND_STONE2:
		if (o->LifeTime > 32 && o->LifeTime < 37)
		{
			...
			Vector(1.f, 0.8f, 0.6f, Light);
			CreateParticletime(BITMAP_SMOKE, Position, o->Angle, Light, 11, 2.f, o);
			CreateEffecttime(MODEL_STONE1 + rand() % 2, Position, o->Angle, o->Light, 10);
		}
	...
	case BITMAP_FLARE:
		if (o->SubType == 1)
		{
			CreateParticletime(BITMAP_FLARE, o->Position, o->Angle, Light, 11, 1.0f);
		}
		else if (o->SubType == 2)
		{
			CreateParticletime(BITMAP_FLARE_BLUE, o->Position, o->Angle, Light, 1, 1.0f);
		}
		else if (o->SubType == 3)
		{
			Vector(0.9f, 0.4f, 0.1f, Light);
			float fRandom = 3.5f + ((float)(rand() % 20 - 10) * 0.1f);
			CreateParticletime(BITMAP_LIGHT, o->Position, o->Angle, Light, 5, fRandom);
		}
		else
			if (o->LifeTime.duration(2))
			{
	...
	case MODEL_CUNDUN_DRAGON_HEAD:
		...
		if (o->Position[2] > 300 && o->Position[2] < 600)
		{
			...
			if (rand() % 2 == 0)
			{
				...
				CreateJointtime(BITMAP_JOINT_SPIRIT2, Position, Position, Angle, 14, NULL, 100.f, 0, 0);
			}
		}
		break;
	case MODEL_CUNDUN_PHOENIX:
	...
	case MODEL_CUNDUN_GHOST:
		...
		else if (o->AnimationFrame > 6.0f)
		{
			...
			Vector(1, 1, 1, Light);
			for (int i = 0; i < 3; ++i)
				CreateParticletime(BITMAP_SMOKE, Position, o->Angle, Light, 20, 10.f);
		}
		else
		{
			o->Skill += (1);
			...
			CreateEffecttime(MODEL_FIRE, Position, Angle, Light, 0, NULL, 0);
	...
	case MODEL_ARROW_TANKER_HIT:
	{
		...
		if (o->Position[2] < Height + 80.0f)
		{
			...
			CreateParticletime(BITMAP_ADV_SMOKE + 1, o->Position, o->Angle, Light);
		...
		if (o->Visible)
		{
			...
			if (b)
			{
				...
				CreateParticletime(BITMAP_FIRE + 1, o->Position, o->Angle, o->Light, 8, o->Scale - 0.4f, o);
				CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 38, o->Scale, o);
				Vector(1.0f, 0.4f, 0.0f, o->Light);
				CreateParticletime(BITMAP_FIRE + 1, o->Position, o->Angle, o->Light, 9, o->Scale - 0.4f, o);
			}
		}
		...
		else
		{
			if (o->SubType <= 1)
			{
				...
				else
				{
					...
					CreateParticletime(BITMAP_FIRE, o->Position, o->Angle, Light, 5, 2.f);
				}
			...
			if (o->SubType == 88 || o->SubType == 99)
			{
				...
				if (o->LifeTime > o->DamageTime)
				{
					Vector(1.f, 0.6f, 0.2f, Light);
					CreateParticletime(BITMAP_ADV_SMOKE + 1, o->Position, o->Angle, Light);
					CreateParticletime(BITMAP_FIRE, o->Position, o->Angle, Light, 2, 2.f);
			...
			else
			{
				...
				if (o->Position[2] < Height)
				{
					...
					{
						CreateEffecttime(BITMAP_CRATER, o->Position, o->Angle, o->Light);
						...
						else if (o->SubType == 99 || (o->SubType == 88 && o->Owner != NULL))
						{
							...
							CreateEffecttime(BITMAP_SHOCK_WAVE, o->Position, o->Angle, Light, 7);
						}
					}
					o->HiddenMesh = 99;
	...
	case MODEL_FLY_BIG_STONE2:
	{
		if (o->SubType <= 1)
		{
			...
			else
			{
				...
				if ((rand() % 2) == 0)
				{
					CreateParticletime(BITMAP_ADV_SMOKE + 1, o->Position, o->Angle, Light, 1, 2.f);
				}
			}
	...
	case MODEL_GOLEM_STONE:
		if (o->Type == MODEL_BIG_STONE_PART2 && o->SubType == 3)
		{
			...
			if (o->Position[2] < Height)
			{
				...
				if (rand() % 10 == 0)
					CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 24, 1.25f * o->Scale);
			}
			else
			{
				if (rand() % 2 == 0)
					CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 24, 0.2f);
			}
			break;
		}
		VectorAdd(o->Position, o->Direction, o->Position);
		...
		if (o->Type == MODEL_GOLEM_STONE)
		{
			if (rand() % 4 == 0)
			{
				CreateParticletime(BITMAP_TRUE_FIRE, o->Position, o->Angle, Light, 5, 2.8f);
				CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, Light, 21, 1.8f);
			}
			if ((rand() % 10) == 0)
			{
				CreateParticletime(BITMAP_SMOKE + 1, o->Position, o->Angle, Light);
			}
		}
	...
	case MODEL_FALL_STONE_EFFECT:
	{
		...
		else if (o->SubType == 2)
		{
			...
			if (o->Position[2] < fHeight)
			{
				...
				CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, vLight, 11, (float)(rand() % 20 + 30) * 0.020f);

				int iRand = rand() % 2 + 2;
				for (int i = 0; i < iRand; ++i)
				{
					float fScale = 0.03f + (rand() % 10) / 40.0f + o->Scale * 0.3f;
					CreateEffecttime(MODEL_FALL_STONE_EFFECT, o->Position, o->Angle, o->Light, 3, NULL, -1, 0, 0, 0, fScale);
				}
	...
	case MODEL_STUN_STONE:
		if (o->SubType == 0)
		{
			...
			if (o->Position[2] <= Height)
			{
				...
				if (o->ExtState == 0)
				{
					CreateEffecttime(BITMAP_CRATER, o->Position, o->Angle, o->Light, 1);
					o->ExtState = 1;
				}
			}
			else
		...
		if (o->SubType == 0)
		{
			...
			else if (o->LifeTime > 30)
			{
				...
				if (o->Scale > 1.f)
				{
					...
					CreateEffecttime(MODEL_MANA_RUNE, o->Position, o->Angle, o->Light, 1);
				}
			}
	...
	case MODEL_SKILL_JAVELIN:
		...
		if (o->LifeTime < 25)
		{
			...
			if (o->LifeTime < 20)
			{
				...
				{
					CreateParticletime(BITMAP_POUNDING_BALL, o->Position, o->Angle, o->Light, 1);
				}
			}
	...
	case MODEL_ARROW_IMPACT:
		...
		if (o->LifeTime < 2 && o->SubType == 0 && o->Owner != NULL)
		{
			...
			Position[2] += (1200.f);
			CreateJointtime(BITMAP_FLASH, Position, Position, o->Angle, 2, o, 50.f);
			CreateJointtime(BITMAP_FLASH, Position, Position, o->Angle, 3, o, 50.f);
		}
		else if (o->SubType == 1)
		{
			o->Live = false;
	...
	case MODEL_SWORD_FORCE:
		if (o->SubType == 0 || o->SubType == 2)
		{
			if (o->LifeTime > 12)
			{
				...
				if (o->SubType == 2)
				{
					CreateEffecttime(MODEL_SWORD_FORCE, o->Position, o->Angle, o->Light, 3, o);
				}
				else
					CreateEffecttime(MODEL_SWORD_FORCE, o->Position, o->Angle, o->Light, 1, o);
			}
			else
			{
	...
	case MODEL_PROTECTGUILD:
	{
		...
		if (o->LifeTime > 120)
		{
			...
			{
				for (int i = 0; i < 5; ++i)
					CreateParticletime(BITMAP_SPARK, o->Position, o->Angle, o->Light, 6, 1.5f, o);
			}
		}
		...
		if (b)
		{
			...
			if (o->LifeTime < 60 && o->LifeTime > 39)
			{
				...
				for (int i = 0; i < 1; ++i)
				{
					...
					CreateParticletime(BITMAP_SPARK, p, o->Angle, o->Light, 5, 2.0f, o);
				}
			}
		}
	}
	...
	case MODEL_MOVE_TARGETPOSITION_EFFECT:
	{
		if (o->SubType == 0)
		{
			...
			CreateParticletime(BITMAP_SPARK + 1, vPos, o->Angle, vLight, 24, 1.0f, o);
	...
	case MODEL_EFFECT_THUNDER_NAPIN_ATTACK_1:
	{
		if (o->SubType == 0)
		{
			...
			if (o->LifeTime >= 4)
			{
				...
				CreateParticletime(BITMAP_LIGHTNING_MEGA1 + rand() % 3, vPos, o->Angle, vLight, 0, o->Scale);
			}
	...
	case MODEL_EFFECT_SKURA_ITEM:
	{
		if ((o->SubType == 0) || (o->SubType == 1))
		{
			if (o->Owner->Live && o->Live)
			{
				...
				for (int i = 0; i < 7; ++i)
				{
					CreateParticletime(BITMAP_SHINY + 1, vtaWorldPos, o->Angle, o->Light, 5, 0.8f, o);
					CreateParticletime(BITMAP_CHERRYBLOSSOM_EVENT_PETAL, vtaWorldPos, o->Angle, rand() % 7 == 3 ? vLight2 : vLight1, 0, 0.5f);
				}
			...
			if (o->LifeTime <= 24)
			{
				if (o->LifeTime >= 15)
				{
					...
					for (int i = 0; i < 15; ++i)
					{
						...
						CreateParticletime(index, vPos, o->Angle, vLight, 8, fScale);
						Vector(1.0f, 1.0f, 1.0f, vLight);
						if (rand() % 2 == 0)
						{
							CreateParticletime(BITMAP_SMOKE, vPos, o->Angle, vLight, 55, 1.f);
						}
					}
				}
	...
	case MODEL_SWELL_OF_MAGICPOWER:
	{
		if (o->SubType == 0)
		{
			...
			if (o->LifeTime >= 30)
			{
				...
				for (int i = 0; i < 2; i++)
				{
					CreateJointtime(BITMAP_2LINE_GHOST, o->Position, o->Position, o->Angle, 1, o, 20.0f + rand() % 10, -1, 0, 0, -1, vLight);
				}
			}
		}
	}break;
	...
	case MODEL_SWELL_OF_MAGICPOWER_BUFF_EFF:
	{
		if (o->SubType == 0)
		{
			...
			if (iPastTime >= 6000)
			{
				...
				// Bip01 R Hand
				CreateEffecttime(MODEL_ARROWSRE06, o->Position, o->Angle, vLight, 1, o->Owner, 28);

				// Bip01 L Hand
				CreateEffecttime(MODEL_ARROWSRE06, o->Position, o->Angle, vLight, 1, o->Owner, 37);
		...
		if (0 == o->SubType)
		{
			if (o->LifeTime > 1)
			{
				for (INT i_ = 0; i_ < 4; ++i_)
				{
					...
					CreateParticletime(BITMAP_RAKLION_CLOUDS, v3ResultPos, v3ResultAngle, v3Light_, 1, fResultScale);
				}
			}
		}
	}break;
	case MODEL_1_STREAMBREATHFIRE:
	{
		...
		CreateParticletime(BITMAP_WATERFALL_3, v3Pos, o->Angle, o->Light, 11, 0.6f);
		CreateParticletime(BITMAP_SMOKE, v3Pos, o->Angle, o->Light, 52, 0.6f);
	...
	case MODEL_PKFIELD_ASSASSIN_EFFECT_RED_HEAD:
	{
		...
		{
			CreateEffecttime(BITMAP_FIRE_CURSEDLICH, vPosition, o->Angle, vLight, 4, o);
		}
		else
		{
			CreateEffecttime(BITMAP_FIRE_CURSEDLICH, vPosition, o->Angle, vLight, 3, o);
		}
	}
	break;
	case MODEL_PKFIELD_ASSASSIN_EFFECT_GREEN_BODY:
	...
	case MODEL_STATUE_CRUSH_EFFECT_PIECE03:
	{
		...
		if (rand() % 3 == 0)
		{
			CreateParticletime(BITMAP_SMOKE + 1, o->Position, o->Angle, o->Light, 6);
		}
	}
	break;
	case MODEL_STATUE_CRUSH_EFFECT_PIECE04:
						...
						case MODEL_SWORDLEFT01_EMPIREGUARDIAN_BOSS_GAION_:
						{
							for (int i = 0; i < iAccess; i++)
							{
								...
								if (rand() % 2 == 0)
								{
									...
									CreateParticletime(BITMAP_FIRE, vPosition, o->Angle, o->Light, 5, 1.35f);

									Vector((float)(rand() % 360), 0.f, (float)(rand() % 360), vAngle);
									AngleMatrix(vAngle, matRandomRotation);
									VectorRotate(vRandomDir, matRandomRotation, vRandomDirPosition);
									VectorAdd(vPosition, vRandomDirPosition, vResultRandomPosition);
									CreateJointtime(BITMAP_JOINT_THUNDER, vResultRandomPosition, vPosition, vAngle, 3, NULL, 10.f, 10, 10);
								}
							}
						}
						break;
			...
			if (o->LifeTime <= 24)
			{
				if (o->LifeTime >= 15)
				{
					...
					for (int i = 0; i < 15; ++i)
					{
						...
						if (rand() % 2 == 0)
						{
							CreateParticletime(BITMAP_SMOKE, vPos, o->Angle, vLight, 55, o->Scale * 0.9f);
						}
					}
				}
	...
	case MODEL_EFFECT_EG_GUARDIANDEFENDER_ATTACK2:
	{
		...
		CreateParticletime(BITMAP_FIRE + 2, o->Position, o->Angle, o->Light, 16, 2.5f);
	...
	case MODEL_WOLF_HEAD_EFFECT:
	{
		if (o->SubType == 0)
		{
			...
			if (o->LifeTime >= 3)
			{
				...
				CreateEffecttime(MODEL_WOLF_HEAD_EFFECT, o->Owner->Position, o->Owner->Angle, o->Owner->Light, 1, o->Owner);
				CreateEffecttime(MODEL_WOLF_HEAD_EFFECT, o->Owner->Position, o->Owner->Angle, o->Owner->Light, 2, o->Owner);
				o->Owner->Angle[2] = Angle[2];
	...
	case BITMAP_SBUMB:
	{
		if (o->LifeTime < 17)
		{
			...
			if (pCharacter)
			{
				...
				CreateParticletime(BITMAP_SBUMB, vPosition, o->Angle, vLight, 0, Scale * o->Scale, o->Owner);
			}
		}
		if (o->LifeTime == 15
	...
	case MODEL_DOWN_ATTACK_DUMMY_L:
	{
		if (o->Owner->Live)
		{
			...
			CreateJointtime(BITMAP_FORCEPILLAR, _StartPos, _StartPos, o->Angle, 0, o, 50.0f);
	...
	case MODEL_DOWN_ATTACK_DUMMY_R:
	{
		if (o->Owner->Live)
		{
			...
			{
				o->Live = false;
				CreateEffecttime(MODEL_DOWN_ATTACK_DUMMY_L, o->Owner->Position, o->Owner->Angle, o->Owner->Light, 0, o->Owner);
			}
			...
			vec3_t _StartPos, _EndPos;
			pModel->TransformByObjectBone(_StartPos, o, 0);
			CreateJointtime(BITMAP_FORCEPILLAR, _StartPos, _StartPos, o->Angle, 1, o, 50.0f);
	...
	case BITMAP_SWORDEFF:
	{
		...
		CreateJointtime(BITMAP_SWORDEFF, _StartPos, _EndPos, o->Owner->Angle, 0, o->Owner, 100.0f);
		...
		if (o->Owner->AnimationFrame > 6 && o->LifeTime > 50)
		{
			...
			CreateEffecttime(MODEL_SHOCKWAVE01, position, Angle, Light, 0, o->Owner, -1, 0, 0, 0, 1.0f);
			CreateEffecttime(MODEL_WOLF_HEAD_EFFECT2, o->Position, o->Angle, o->Light, 3, o->Owner);
			...
			if (pCharacter)
			{
				CreateEffecttime(BITMAP_DAMAGE1, pCharacter->Object.Position, Angle, Light, 0, o->Owner, -1, 0, 0, 0, 1.4f);
			}
#ifdef PBG_MOD_RAGEFIGHTERSOUND
		...
		if (o->Owner->AnimationFrame > 11 && o->LifeTime > 20)
		{
			...
			CreateEffecttime(MODEL_SHOCKWAVE02, position, Angle, Light, 0, o->Owner, -1, 0, 0, 0, 0.5f);
			CreateEffecttime(MODEL_WOLF_HEAD_EFFECT2, o->Position, o->Angle, o->Light, 5, o->Owner);

			Vector(1.0f, 1.0f, 1.0f, Light);
			Vector(-80.0f, 20.0f, 0.0f, vPosition);
			b->TransformByObjectBone(vPosition, o->Owner, 4, vPosition);
			Vector(90.0f, Angle[1], Angle[2], Angle);
			CreateEffecttime(MODEL_SHOCKWAVE_SPIN01, vPosition, Angle, Light, 0, o->Owner, -1, 0, 0, 0, 0.5f);
			CreateEffecttime(MODEL_SHOCKWAVE_SPIN01, vPosition, Angle, Light, 1, o->Owner, -1, 0, 0, 0, 0.9f);
			...
			{
				VectorCopy(pCharacter->Object.Position, vPosition);
				CreateEffecttime(BITMAP_SHINY + 4, vPosition, o->Angle, Light, 0, o->Owner, -1, 0, 0, 0, 1.9f);
			}
#ifdef PBG_MOD_RAGEFIGHTERSOUND
			StopBuffer(SOUND_RAGESKILL_GIANTSWING_ATTACK, true);
#endif //PBG_MOD_RAGEFIGHTERSOUND
	...
	case MODEL_WINDFOCE:
	{
		...
		else if (o->SubType == 1)		//지속적인거 
		{
			...
			VectorScale(o->Light, timepow(fFadeInOut * 0.25f + 0.2f), o->Light);
			o->Scale = 1.0f;
			VectorCopy(o->Owner->Position, o->Position);
		}
		else if (o->SubType == 2)
	...
	case BITMAP_SHINY + 4:
	{
		...
		CreateParticletime(BITMAP_SHINY + 4, vPosition, o->Angle, Light, 2, o->Scale);
	}
	break;
	case BITMAP_LIGHT_RED:
	{
			...
			if (g_isCharacterBuff(o->Owner, eBuff_Def_up_Ourforces))
			{
				if (rand() % 70 == 7)
				{
					if (!SearchEffect(MODEL_WINDFOCE_MIRROR, o->Owner))
					{
						...
						CreateEffecttime(MODEL_WINDFOCE_MIRROR, o->Owner->Position, o->Angle, vLight, 0, o->Owner, -1, 0, 0, 0, 1.0f);
					}
				}
			}
		}
	...
	case MODEL_WOLF_HEAD_EFFECT2:
	{
		...
		else if (o->SubType == 3)
		{
			...
			if (o->LifeTime >= 3)
			{
				...
				CreateEffecttime(MODEL_WOLF_HEAD_EFFECT2, o->Owner->Position, o->Owner->Angle, o->Owner->Light, 4, o->Owner);
				CreateEffecttime(MODEL_WOLF_HEAD_EFFECT2, o->Owner->Position, o->Owner->Angle, o->Owner->Light, 6, o->Owner);
				o->Owner->Angle[2] = Angle[2];
				o->Owner->Angle[0] = Angle[0];
			}
		}
		...
		if (o->Owner->Live)
		{
			...
			if (o->Owner->AnimationFrame > 4 && o->Owner->AnimationFrame < 5)
			{
				...
				for (int i = 0; i < fDelay; i++)
				{
					...
					CreateJointtime(BITMAP_GROUND_WIND, _StartPos, _EndPos, o->Angle, i, o, 150.0f);
					fAnimationFrame += (fSpeedPerFrame);
				}
			}
			...
			if (o->Owner->AnimationFrame > 5)
			{
				for (int j = 0; j < 2; ++j)
				{
					...
					for (int i = 0; i < 4; ++i)
					{
						CreateParticletime(BITMAP_ENERGY, vPos, o->Angle, o->Light, 7, fScale);
						CreateParticletime(BITMAP_LIGHTNING_MEGA1, vPos, o->Angle, o->Light, 0, fScale * 0.9f);

						CreateParticletime(BITMAP_SPARK + 1, vPos, o->Angle, o->Light, 3, 1.0f + (fLuminosity * 0.05f));
						CreateParticletime(BITMAP_SPARK + 1, vPos, o->Angle, o->Light, 8, 1.0f + (fLuminosity * 0.05f));

						if (i % 2)
						{
							CreateParticletime(BITMAP_SPARK, vPos, o->Angle, o->Light, 13);
						}
	...
	case BITMAP_LAVA:
	{
		...
		if (o->Owner->AnimationFrame > 2 && o->Owner->AnimationFrame < 8)
		{
			...
			for (int i = 0; i < fDelay; i++)
			{
				...
				CreateJointtime(BITMAP_LAVA, _StartPos, _StartPos, o->Owner->Angle, i, o->Owner, 20.0f);
				CreateJointtime(BITMAP_LAVA, _EndPos, _EndPos, o->Owner->Angle, 7 + i, o->Owner, 20.0f);
				fAnimationFrame += (fSpeedPerFrame);
			}
		}
		...
		{
			Vector(1.0f, 0.12f, 0.0f, vLight);
			CreateEffecttime(MODEL_SHOCKWAVE01, o->Owner->Position, vAngle, vLight, 1, o->Owner, -1, 0, 0, 0, 1.0f);
			o->LifeTime = 95;
		}
		...
		if (o->Owner->AnimationFrame > 5 && o->LifeTime > 80)
		{
			...
			CreateEffecttime(MODEL_SHOCKWAVE_GROUND01, o->Position, o->Angle, vLight, 1, o, -1, 0, 0, 0, 1.0f);
			o->LifeTime = 80;
		}
		...
		if (o->Owner->AnimationFrame > 5)
		{
			...
			{
			case 0:
				CreateParticletime(BITMAP_FIRE_HIK1, vPos, o->Angle, vLight, 0, fScale);
				break;
			case 1:
				CreateParticletime(BITMAP_FIRE_CURSEDLICH, vPos, o->Angle, vLight, 4, fScale);
				break;
			case 2:
				CreateParticletime(BITMAP_FIRE_HIK3, vPos, o->Angle, vLight, 0, fScale);
				break;
			}
			...
			for (int i = 0; i < 8; ++i)
			{
				...
				vPos[2] = tempPos[2];
				CreateParticletime(BITMAP_FIRE_HIK1, vPos, o->Angle, vLight, 5, fScale);

				vec3_t TempPos;
				VectorCopy(vPos, TempPos);
				TempPos[2] = 250.0f;
				CreateParticletime(BITMAP_SPARK, TempPos, o->Angle, o->Light, 12, 1.5f);
			}
		}
	}
	break;
	case MODEL_DRAGON_LOWER_DUMMY:
	{
		...
		if (o->AnimationFrame > 1 && o->AnimationFrame < 6)
		{
			...
			temp[2] = 250.0f;
			CreateParticletime(BITMAP_SPARK, temp, o->Angle, o->Light, 12, 1.5f);
		}
		...
		if (o->AnimationFrame > 1 && o->LifeTime > 80)
		{
			vec3_t vLight;
			Vector(1.0f, 1.0f, 1.0f, vLight);
			CreateEffecttime(BITMAP_LIGHT_RED, o->Position, o->Angle, vLight, 1, o, -1, 0, 0, 0, 15.0f);
			o->LifeTime = 80;
		}
	...
	case MODEL_TARGETMON_EFFECT:
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			{
			case 0:
				CreateParticletime(BITMAP_FIRE_HIK1, o->Position, o->Angle, vLight, 0, fScale);
				break;
			case 1:
				CreateParticletime(BITMAP_FIRE_CURSEDLICH, o->Position, o->Angle, vLight, 4, fScale);
				break;
			case 2:
				CreateParticletime(BITMAP_FIRE_HIK3, o->Position, o->Angle, vLight, 0, fScale);
				break;
			}
		}
	}
	...
	case MODEL_VOLCANO_OF_MONK:
	{
		if (o->SubType == 1)
		{
			...
			{
			case 0:
				CreateParticletime(BITMAP_FIRE_HIK1, vPos, o->Angle, vLight, 6, fScale);
				break;
			case 1:
				CreateParticletime(BITMAP_FIRE_CURSEDLICH, vPos, o->Angle, vLight, 9, fScale);
				break;
			case 2:
				CreateParticletime(BITMAP_FIRE_HIK3, vPos, o->Angle, vLight, 6, fScale);
				break;
			}
	...
	case MODEL_PHOENIX_SHOT_EFFECT:
	{
		...
		if (o->LifeTime == 70)
		{
			...
			{
				Position[0] = pCharacter->Object.Position[0];
				Position[1] = pCharacter->Object.Position[1];
				Position[2] = pCharacter->Object.Position[2];
				Angle[0] = 0;
				Angle[1] = 0;
	...
	case MODEL_WIND_SPIN03:
	{
		...
		Direction[0] *= ((50.0 - (double)o->LifeTime) * 2.0 + 25.0);
		Direction[1] *= ((50.0 - (double)o->LifeTime) * 2.0 + 25.0);
		//v524 = (double)o->LifeTime;
		Direction[2] = 0.0;
		o->Position[0] += timefac(Direction[0]);
		o->Position[1] += timefac(Direction[1]);
		o->Position[2] += timefac(Direction[2]);
		o->Angle[2] -= timefac(70.0);
		o->Light[0] *= timepow(0.95);
		o->Light[1] *= timepow(0.95);
		o->Light[2] *= timepow(0.95);
		...
		vTemp2[0] = o->Position[0];
		vTemp2[1] = o->Position[1];
		vTemp2[2] = o->Position[2];
		vTemp2[2] = 0.0;

		vTemp2[0] = ow->Position[0] - vTemp2[0];
		vTemp2[1] = ow->Position[1] - vTemp2[1];
		vTemp2[2] = ow->Position[2] - (float)0.0;

		vTemp1[0] = vTemp1[0] - ow->Position[0];
		vTemp1[1] = vTemp1[1] - ow->Position[1];
		vTemp1[2] = vTemp1[2] - ow->Position[2];
		...
		if (pCharacter)
		{
			...
			{
				CreateParticle(BITMAP_SPARK, Position, Angle, Light, 14, 10.0, 0);
			}

			Light[0] = 1.0;
			Light[1] = 0.7;
			Light[2] = 0.2;
			CreateEffect(MODEL_WIND_SPIN01, vTemp1, ow->Angle, Light, 0, ow, -1, 0, 0, 0, 0.1, -1);
			CreateEffect(MODEL_WIND_SPIN01, vTemp1, ow->Angle, Light, 0, ow, -1, 0, 0, 0, 0.1, -1);
			CreateEffect(MODEL_WIND_SPIN01, vTemp1, ow->Angle, Light, 0, ow, -1, 0, 0, 0, 0.1, -1);
			...
			Light[1] = 1.0;
			Light[2] = 1.0;
			CreateEffect(MODEL_WIND_SPIN02, vTemp1, ow->Angle, Light, 0, ow, -1, 0, 0, 0, 0.25, -1);

			Light[0] = (float)1.0 * 0.2;
			Light[1] = (float)0.6 * 0.2;
			Light[2] = (float)0.2 * 0.2;
			...
			for (int i88 = 0; i88 < 8; ++i88)
			{
				Angle[2] += 30.0;
				vTemp1[2] = 130.0;
				CreateEffect(BITMAP_TWLIGHT, vTemp1, Angle, Light, 4, ow, -1, 0, 0, 0, 2.5, -1);
			}
		...
		LABEL_3611:
			if (o->LifeTime <= 30)
			{
				...
				vTemp1[1] = pCharacter->Object.Position[1];
				vTemp1[2] = pCharacter->Object.Position[2];
				o->Position[0] = vTemp1[0];
				o->Position[1] = vTemp1[1];
				o->Position[2] = vTemp1[2];
				o->Scale += timefac(0.08);
				o->Light[0] *= timepow(0.97);
				o->Light[1] *= timepow(0.97);
				o->Light[2] *= timepow(0.97);
			}
			o->Position[2] = ow->Position[2] + 100.0;
			if (o->Live && !o->SubType)
			{
				...
				vTemp1[0] = o->Position[0];
				vTemp1[1] = o->Position[1];
				//v538 = o->Position[2];
				vTemp1[2] = 130.0;
				o->Angle[2] = (double)o->LifeTime * 40.0 + 90.0;
				CreateEffect(BITMAP_TWLIGHT, vTemp1, o->Angle, Light, 5, o, -1, 0, 0, 0, 3.5, -1);
				o->Angle[2] = (double)o->LifeTime * 20.0 + 90.0;
				CreateEffect(BITMAP_TWLIGHT, vTemp1, o->Angle, Light, 5, o, -1, 0, 0, 0, 3.5, -1);
			}
		}
	}
	...
	case MODEL_WIND_SPIN02:
	{
		...
		if (pModel)
		{
			...
			o->Light[1] *= timepow(0.8f);
			o->Light[2] *= timepow(0.8f);
			float Velocity = o->Velocity / 5.0;
			pModel->PlayAnimation(&o->AnimationFrame, &o->PriorAnimationFrame, &o->PriorAction, Velocity, o->Position, o->Angle);
		}
	}
	break;
	case MODEL_WIND_SPIN01:
					...
					if (o->SubType == 1)
					{
						...
						if (pModel)
						{
							...
							if (rand() % 5 == 0)
							{
								...
								CreateParticletime(BITMAP_SMOKE, vPos, o->Angle, vLight, 57, 2.0f);
								Vector(0.7f, 0.7f, 1.f, vLight);
								CreateParticletime(BITMAP_TWINTAIL_WATER, vPos, o->Angle, vLight, 1);
							}
							RenderObject(o);
