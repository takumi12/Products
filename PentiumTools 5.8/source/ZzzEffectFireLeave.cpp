...
void CreateBonfire(vec3_t Position, vec3_t Angle)
{
	...
	CreateParticletime(BITMAP_FIRE, Position, Angle, Light, rand() % 4);
	...
	if (rand() % 4 == 0)
	{
		...
		CreateJointtime(BITMAP_JOINT_SPARK, Position, Position, a);
	}
...
void CreateFire(int Type, OBJECT* o, float x, float y, float z)
{
	...
	VectorAddTime(o->Position, Position, Position);
	...
	case 0:
		...
		Vector(Luminosity, Luminosity * 0.6f, Luminosity * 0.4f, Light);
		if (rand() % 2 == 0)
			CreateParticletime(BITMAP_FIRE, Position, o->Angle, Light, rand() % 4);
		AddTerrainLight(Position[0], Position[1], Light, 4, PrimaryTerrainLight);
		break;
	case 1:
		if (rand() % 2 == 0)
	...
	case 2:
		if (rand() % 2 == 0)
			CreateParticletime(BITMAP_SMOKE, Position, o->Angle, o->Light, 2);
		break;
	}
}
...
bool MoveDevilSquareRain(PARTICLE* o)
{
	...
	if (o->Position[2] < Height)
	{
		...
		if (rand() % 4 == 0)
			CreateParticletime(BITMAP_RAIN_02, o->Position, o->Angle, o->Light);
		else
			CreateParticletime(BITMAP_RAIN_03, o->Position, o->Angle, o->Light);
	}
	return true;
}
...
bool MoveHeavenRain(PARTICLE* o)
{
	...
	else
	{
		...
		VectorAddTime(o->Angle, o->TurningForce, o->Angle);
