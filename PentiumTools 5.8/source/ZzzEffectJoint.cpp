...
void CreateJointtime(int Type, vec3_t Position, vec3_t TargetPosition, vec3_t Angle, int SubType, OBJECT* Target, float Scale, short PK, WORD SkillIndex, WORD SkillSerialNum, int iChaIndex, const float* vColor, short int sTargetIndex)
{
	if (time_base_check)
...
void MoveJoint(JOINT* o, int iIndex)
{
		...
		&& (o->Type != BITMAP_FLASH || o->SubType!= 6))
	{
		...
		if (o->Velocity != 0.0f)
		{
			...
			VectorRotate(p, Matrix, Position);
			if ((o->Type == BITMAP_JOINT_FORCE && o->SubType >= 2 && o->SubType <= 6)
				|| (o->Type == BITMAP_FLARE && o->SubType == 7))
			{
				o->Position[0] += (Position[0]);
				o->Position[1] += (Position[1]);
	...
	o->fotogramavirtual = false;

	switch (o->Type)
	{
	case BITMAP_JOINT_LASER:
		VectorCopy(o->Target->Position, o->TargetPosition);
		o->TargetPosition[2] += timefac(130.f);
	...
	case BITMAP_JOINT_ENERGY:
		...
		case 57:
			...
			case 3:
				...
				CreateParticle(BITMAP_LIGHTNING + 1, o->Position, o->Angle, Light, 0, 1.0, NULL, o->fotogramavirtual);
				break;
			case 14:
			case 15:
				Luminosity = sinf(WorldTime * 0.002f) * 0.3f + 0.8f;
				Vector(Luminosity, Luminosity * 0.1f, Luminosity * 0.1f, Light);
				CreateParticle(BITMAP_LIGHTNING + 1, o->Position, o->Angle, Light, 4, 1.0, NULL, o->fotogramavirtual);
				break;
			case 22:
			case 23:
		...
		case 46:
		{
			...
			else if (o->SubType == 44 && o->LifeTime <= 120 && o->LifeTime >= 10)
			{
				if (o->LifeTime >= 20)
				{
					...
					VectorCopy(o->Target->Position, o->TargetPosition);
					o->TargetPosition[2] += (timefac(120.f));
			...
			if (o->SubType == 6 || o->SubType == 9)
			{
				CreateParticletime(BITMAP_LIGHTNING + 1, o->Position, o->Angle, o->Light, 3, 0.05f);
			}
			...
			else if (o->SubType == 16)
			{
				...
				CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, Light, 31, 0.8f);
			}
			...
			{
				CreateParticletime(BITMAP_LIGHTNING + 1, o->Position, o->Angle, o->Light);
			}
		}
		break;
		case 42:
		{
			if (o->LifeTime >= 60)
			{
				...
				o->Light[0] += timefac(0.4f * 0.005f);
				o->Light[1] += timefac(0.3f * 0.005f);
				o->Light[2] += timefac(2.2f * 0.005f);
			}
			else
			{
			...
			Vector(Luminosity * 0.4f, Luminosity, Luminosity * 0.8f, Light);
			AddTerrainLight(o->Position[0], o->Position[1], Light, 2, PrimaryTerrainLight);
			CreateParticletime(BITMAP_LIGHTNING + 1, o->Position, o->Angle, o->Light, 5, 0.12f);
		}
		break;
		case 43:
		{
	...
	case BITMAP_JOINT_HEALING:
		...
		else if (o->SubType == 9)
		{
			...
			o->Position[0] = o->TargetPosition[0] + sinf((o->MultiUse * 0.1)) * o->Direction[0];
			o->Position[1] = o->TargetPosition[1] + cosf((o->MultiUse * 0.1)) * o->Direction[0];
			o->Position[2] = o->TargetPosition[2] + 10 + (90 - (double)o->LifeTime);
			...
			if (o->LifeTime < 50)
			{
				VectorScale(o->Light, timepow(1.f/1.1f), o->Light);
			}
	...
	case BITMAP_2LINE_GHOST:
	{
		if (o->SubType == 0)
		{
			...
			CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 59, 1.0f);
		}
		else if (o->SubType == 1)
		{
			float TargetAngleH;
	...
	case BITMAP_JOINT_SPIRIT2:
		if (0 == o->SubType || o->SubType == 5 || o->SubType == 19)
		{
			...
			{
				if (o->SubType == 5)
					CreateEffecttime(MODEL_LASER, o->Position, o->Angle, o->Light, 3);
				else
					CreateEffecttime(MODEL_LASER, o->Position, o->Angle, o->Light);
				...
				if (o->LifeTime.duration(15) && o->Target == &Hero->Object)
				{
			...
			{
				CreateEffecttime(MODEL_SKULL, o->Position, o->Angle, o->Light);
			}
			VectorCopy(o->Target->Position, o->TargetPosition);
			...
			if (o->SubType != 5)
			{
				...
				o->Direction[0] += timefac(randuin * 0.2f);
				o->Direction[2] += timefac(randuin * 0.8f);
			...
			|| o->SubType == 22 || o->SubType == 23)
		{
			...
			else if (o->LifeTime > 18 && o->LifeTime < 20 && (o->SubType == 2 || o->SubType == 21))
			{
				...
				if (o->SubType == 2)
					CreateJointtime(BITMAP_FLARE, Position, Position, Angle, 2, NULL, 40);
			}
		...
		else if (o->SubType == 11)
		{
			...
			CreateParticletime(BITMAP_FIRE + 1, Position, o->Angle, Light, 5, 0.9f);
			if (rand() % 200 == 0)
			{
				CreateJointtime(BITMAP_JOINT_SPIRIT, Position, Position, o->Angle, 12, NULL, 0.9f);
			}
		...
		else if (o->SubType == 12)
		{
			...
			CreateParticletime(BITMAP_SMOKE + 3, Position, o->Angle, Light, 0, (float)(rand() % 32 + 48) * 0.01f);
			Vector(1, 1, 1, Light);
			CreateParticletime(BITMAP_FIRE + 1, Position, o->Angle, Light, 6, 0.6f);
		}
		else if (o->SubType == 14)
		{
			Vector(1, 1, 1, Light);
			...
			int r = rand() % 5;
			if (r == 0)
				CreateParticletime(BITMAP_WATERFALL_3, o->Position, o->Angle, o->Light, 2);
			else if (r <= 2)
				CreateParticletime(BITMAP_WATERFALL_4, o->Position, o->Angle, o->Light, 2);
			else
				CreateParticletime(BITMAP_WATERFALL_5, o->Position, o->Angle, o->Light, 4);
		}
		else if (o->SubType == 15)
		{
			Vector(1, 1, 1, Light);
			...
			if (o->LifeTime < 35)
			{
				CreateParticletime(BITMAP_FIRE, Position, o->Angle, Light, 7, 1.0f);
				...
				if (rand() % 2 == 0)
				{
					...
					for (int i = 0; i < 2; ++i)
					{
						...
						CreateJointtime(BITMAP_JOINT_SPIRIT, Position, Position, Angle, 14, NULL, 0.9f);
					}
				}
				if (o->Target != NULL)
				{
		...
		else if (o->SubType == 18)
		{
			...
			VectorCopy(o->Position, Position);
			Vector(1, 1, 1, Light);
			CreateParticletime(BITMAP_SMOKE, Position, o->Angle, Light, 19, 7.f);
		}
		else if (o->SubType == 20)
		{
			if (o->LifeTime < 42)
		...
		else if (o->SubType == 24)
		{
			...
			o->Direction[0] += ((float)(rand() % 32 - 16) * timepow(0.2f));
			o->Direction[2] += ((float)(rand() % 32 - 16) * timepow(0.8f));
	...
	case BITMAP_JOINT_FIRE:
		...
		VectorRotate(Position, Matrix, p);
		VectorAddTime(o->Position, p, o->Position);
		break;
	case MODEL_SPEARSKILL:
		if (o->m_iChaIndex != -1)
		{
		...
		if (o->SubType == 2)
		{
			...
			float fRate1 = max(0.0f, min((float)(o->LifeTime._runvalue - 10) / (float)10, 1.0f));
		...
		else if (o->SubType == 5 || o->SubType == 6 || o->SubType == 7)
		{
			for (int i = 0; i < 3; ++i)
			{
				...
				AngleMatrix(o->Angle, Matrix);
				VectorRotate(o->Direction, Matrix, Position);
				VectorAddTime(o->StartPosition, Position, o->Position);
				CreateTail(o, Matrix);
		...
		else if (o->SubType == 8)
		{
			...
			o->StartPosition[2] += timefac(15.f);
		}
		else
		{
			if (!o->Target->Live)
			...
			if (o->SubType == 10 || o->SubType == 11)
			{
				...
				if (rand() % 10 == 0)
				{
					...
					CreateParticletime(BITMAP_SPARK + 1, o->Position, o->Angle, Light, 19);
				}
			}
	...
	case MODEL_FENRIR_SKILL_THUNDER:
		for (int j = 0; j < o->MaxTails; j++)
		{
			...
			if (Distance < o->Velocity * 1.5f)
			{
				if (o->Scale == 50.f)
				{
					CreateParticletime(BITMAP_ENERGY, o->Position, o->Angle, o->Light);
					vec3_t Position;
					if (rand() % 8 == 0)
					{
	...
	case BITMAP_JOINT_LASER + 1:
		...
		for (int j = 0; j < o->MaxTails; j++)
		{
			...
			{
				VectorCopy(o->Target->Position, o->TargetPosition);
				o->TargetPosition[2] += timefac(80.f);
			}
			...
			if (Distance <= o->Velocity * 2.f)
			{
				...
				if (rand() % 2 == 0)
				{
					Vector((float)(rand() % 64 - 32), (float)(rand() % 64 - 32), (float)(rand() % 64 - 32), Position);
					VectorAdd(Position, o->TargetPosition, Position);
					CreateParticletime(BITMAP_FIRE, o->Position, o->Angle, o->Light);
				}
				break;
			}
			//light
	...
	case BITMAP_JOINT_THUNDER:
		...
		for (int j = 0; j < o->MaxTails; j++)
		{
			...
			case 19:
				...
				break;
			case 20:
				o->TargetPosition[2] += timefac(100.f);
	...
	case BITMAP_JOINT_THUNDER + 1:
...

		{
			...
			for (int i = 0; i < o->MaxTails - 5; ++i)
			{
				if (o->SubType == 6)
				{
					o->Position[0] += timefac(rand() % 20 - 10);
					o->Position[1] += timefac(rand() % 20 - 10);
					o->Position[2] -= timefac(13.f);
				}
				else
				{
					o->Position[0] += timefac(rand() % 20 - 10);
			...
			if (o->SubType == 2)
			{
				...
				CreateSprite(BITMAP_SHINY + 1, o->TargetPosition, 1.5f, Light, NULL, (float)((rand() % 360)));
				CreateParticletime(BITMAP_TRUE_BLUE, o->TargetPosition, o->Angle, Light);
			}
			else if (o->SubType == 3)
			{
				Vector(o->Light[0] * 0.5f, o->Light[1] * 0.6f, o->Light[2], Light);
				CreateEffecttime(MODEL_ICE, o->TargetPosition, o->Angle, Light);
			}
			else if (o->SubType == 6)
			{
				o->TargetPosition[2] = RequestTerrainHeight(o->TargetPosition[0], o->TargetPosition[1]) + 30.f;
				CreateParticletime(BITMAP_TRUE_BLUE, o->TargetPosition, o->Angle, Light, 0, 2.0f);
			}
		}
		else if (o->SubType == 4)
		{
			if (o->Target != NULL)
			{
				...
				for (int i = 0; i < o->TargetIndex[0]; ++i)
				{
					...
					o->Position[0] += timefac(rand() % 20 - 10);
					o->Position[1] += timefac(rand() % 20 - 10);
					o->Position[2] += timefac(rand() % 20 - 10);
				}
		...
		else if (o->SubType == 8)
		{
			...
			for (int i = 0; i < o->MaxTails - 5; ++i)
			{
				o->Position[0] += timefac(rand() % 10 - 5);
				o->Position[1] += timefac(rand() % 10 - 5);
				o->Position[2] -= timefac(20.0f);
				CreateTail(o, Matrix);
			}
			VectorSubtract(o->TargetPosition, o->Position, Position);
		...
		else if (o->SubType == 11)
		{
			...
			for (int i = 0; i < o->MaxTails - 5; i++)
			{
				...
				o->Position[0] += timefac(rand() % (iScale * 2) - iScale);
				o->Position[1] += timefac(rand() % (iScale * 2) - iScale);
				o->Position[2] += timefac(rand() % (iScale * 2) - iScale);
				CreateTail(o, Matrix);
			}
			VectorCopy(o->TargetPosition, o->Position);
			CreateTail(o, Matrix);
	...
	case BITMAP_FLARE_BLUE:
		...
		else if (o->SubType == 40)
		{
			...
			for (int i = 0; i < MAX_TAILS; ++i)
			{
				...
				vPos[2] = rSin;
				o->TargetPosition[0] += timefac(o->Direction[0] * (lMax - o->LifeTime._runvalue));
				o->TargetPosition[1] += timefac(o->Direction[1] * (lMax - o->LifeTime._runvalue));
		...
		else if (o->SubType == 43)
		{
			if (o->Target->Live)
			{
				...
				if (rand() % 3 == 0)
				{
					...
					CreateJointtime(BITMAP_FLARE, o->Position, o->Position, o->Angle, 44, o->Target, o->Scale, 0, 0, 0, 0, o->Light);
				}
			}
			else
			{
		...
		else if (o->SubType == 12 || o->SubType == 13 || o->SubType == 14 || o->SubType == 17 || o->SubType == 15 || o->SubType == 18)
		{
			if (o->Target != NULL)
			{
				...
				if (o->SubType == 14 && o->Live == true)
				{
					...
					CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 34, 1.0f);
					CreateParticletime(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 35, 1.0f);
					//CreateParticle(BITMAP_SPARK+1, o->Position, o->Angle, o->Light, 12, 1.0f);
					Vector(0.7f, 0.3f, 0.3f, o->Light);
					CreateParticletime(BITMAP_SPARK + 1, o->Position, o->Angle, o->Light, 10, 4.0f);
					Vector(1.0f, 1.0f, 0.2f, o->Light);
				}
				else if (o->SubType == 15 && o->Live == true)
				{
					CreateParticletime(BITMAP_SHINY, o->Position, o->Angle, o->Light, 3, 0.5f);
					CreateParticletime(BITMAP_SHINY, o->Position, o->Angle, o->Light, 3, 0.5f);
				}
	...
	case BITMAP_JOINT_FORCE:
		if (o->SubType == 10 || o->SubType == 8)
		{
			...
			for (int j = 0; j < 8; ++j)
			{
				if (o->NumTails < o->MaxTails - 1)
				{
					...
					o->Direction[2] -= (timefac(11.f));
					if (o->SubType == 10)
					{
						if ((rand() % 2) == 0)
						{
							CreateParticletime(BITMAP_FIRE, o->Position, o->Angle, o->Light, 0);
						}

						CreateJointtime(BITMAP_JOINT_THUNDER, Light, o->Position, o->Angle, 3, NULL, rand() % 10 + 5.f, 5, 10);
						CreateJointtime(BITMAP_JOINT_THUNDER, Light, o->Position, o->Angle, 3, NULL, rand() % 8 + 4.f, 5, 10);
					}
					else if (o->SubType == 8)
					{
						CreateJointtime(BITMAP_JOINT_THUNDER, o->Position, o->Position, o->Angle, 3, NULL, rand() % 10 + 5.f, 5, 10);
						CreateJointtime(BITMAP_JOINT_THUNDER, o->Position, o->Position, o->Angle, 3, NULL, rand() % 8 + 4.f, 5, 10);
					}
				}
				if (o->SubType == 10)
				{
			...
			for (int j = 0; j < 8; ++j)
			{
				if (o->NumTails < o->MaxTails - 1)
				{
					...
					if (o->SubType == 0)
					{
						...
						CreateJointtime(BITMAP_JOINT_THUNDER, Light, o->Position, o->Angle, 3, NULL, rand() % 10 + 5.f, 5, 10); //  전기 
						CreateJointtime(BITMAP_JOINT_THUNDER, Light, o->Position, o->Angle, 3, NULL, rand() % 8 + 4.f, 5, 10); //  전기 
					}
				}
				if (o->SubType == 0)
				{
		...
		else if (o->SubType >= 2 && o->SubType <= 6)
		{
			...
			if (o->LifeTime.duration(5) && o->SubType == 2)
			{
		...
		else if (o->SubType == 7 || o->SubType == 20)
		{
			...
			if (o->LifeTime < 20)
			{
				o->Direction[2] += (timefac(0.5f));
			}
			else
			{
				o->Direction[2] += timefac(o->Direction[0]);
			}
			if (o->NumTails >= o->MaxTails - 1)
			{
				o->m_bCreateTails = false;
		...
		if (o->SubType == 0)
		{
			if (o->LifeTime > 16)
			{
				...
				for (int i = 0; i < 10; ++i)
				{
					...
					VectorAddTime(o->Position, vPos, o->Position);
				}
			}
		}
		else if (o->SubType == 1)
		...
		if (o->SubType == 5 || o->SubType == 6 || o->SubType == 7)
		{
			if (o->Target != NULL)
			{
				...
				for (int i = 0; i < MaxTails; ++i)
				{
					...
					if (o->SubType % 2)
					{
						o->TargetPosition[1] += timefac(40.f);
					}
					else
					{
						o->TargetPosition[1] -= timefac(40.f);
					...
					o->TargetPosition[0] -= timefac(0.15f);
			...
			else if (o->SubType >= 1 && o->SubType <= 4 || (o->SubType >= 11 && o->SubType <= 13))
			{
				if (o->Weapon <= 0)
				{
					...
					for (int i = 1; i < o->MultiUse; ++i)
					{
						...
						if (o->SubType == 2 || o->SubType == 4)
						{
							o->TargetPosition[1] -= timefac(20.f);
						}
	...
	case BITMAP_FLASH:
		...
		else if (o->SubType == 6)
		{
			...
			{
				CreateParticletime(BITMAP_EXPLOTION + 1, o->StartPosition, o->Angle, o->Light, 0, 3.f);
				o->Scale = 100.f;
			}
			else
			{
	...
	case BITMAP_SWORDEFFOR2_BLUR:
	{
		...
		if (pModel)
		{
			...
			if (owner->AnimationFrame < 18.0 && owner->AnimationFrame >= 3.0 && time_base_check)
			{
		...
		if (o->Live && o->NumTails > 0 && o->RenderFace != 0)
		{
			...
			for (int j = 0; j < o->NumTails; j++)
			{
				...
				if (o->Type == BITMAP_JOINT_THUNDER || o->Type == BITMAP_JOINT_THUNDER + 1)
				{
					Light1 *= timepow(2.f);
					Light2 *= timepow(2.f);
					Light1 -= timefac(Scroll);
					Light2 -= timefac(Scroll);
				}
				...
				else
				{
					...
					else if (o->Type == BITMAP_JOINT_HEALING && (o->SubType == 9 || o->SubType == 10))
					{
						if (o->Target != NULL)
						{
							...
							o->Light[0] *= (timepow(0.9978f));
							o->Light[1] *= (timepow(0.9978f));
							o->Light[2] *= (timepow(0.9978f));
							Vector(o->Light[0] - fJointHeight, o->Light[1] - fJointHeight, o->Light[2] - fJointHeight, Light);
							glColor3fv(Light);
					...
					if ((o->RenderFace & RENDER_FACE_TWO) == RENDER_FACE_TWO)
					{
						if (o->Type == BITMAP_JOINT_THUNDER || o->Type == BITMAP_JOINT_THUNDER + 1)
						{
							L1 += timefac(Scroll * 2.f);
							L2 += timefac(Scroll * 2.f);
						}
						glBegin(GL_QUADS);
						glTexCoord2f(L1, V1);
						glVertex3fv(o->Tails[j][0]);
...
{
	iParam += static_cast<int>(WorldTime / 40);
