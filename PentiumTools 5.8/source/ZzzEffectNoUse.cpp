...
void MappingEffect(int SrcIndex, int DstIndex, int Flag)
{
	...
	unsigned char* s = src->Buffer.data();
	unsigned char* d = dst->Buffer.data();
	...
	glTexImage2D(GL_TEXTURE_2D, 0, src->Components, (int)src->Width, (int)src->Height, 0, GL_RGB, GL_UNSIGNED_BYTE, dst->Buffer.data());
}
