...
void ItemConvert(ITEM* ip, BYTE Attribute1, BYTE Attribute2, BYTE Attribute3)
{
	...
	CUSTOM_WING_INFO* fw = GMItemWings->Find(ip->Type);
	...
	int Weapon02Kind1 = p->Kind1;
	int Weapon02Kind2 = p->Kind2;
	int Weapon02Kind3 = p->Kind3;
	...
	if (p->Defense > 0)
	{
		...
		else
		{
			...
			if (fw != NULL)
			{
				ip->Defense += (min(9, Level) * fw->DefenseConstA);	// ~ +9
			}
			else
			{
			...
			if (fw != NULL || (ip->Type >= ITEM_WING + 36 && ip->Type <= ITEM_WING + 40) || ip->Type == ITEM_WING + 43 || ip->Type == ITEM_WING + 50)
			{
				switch (Level - 9)
				{
				case 6:
					ip->Defense += 10; break;	// +15
				case 5:
					ip->Defense += 9; break;	// +14
				case 4:
					ip->Defense += 8; break;	// +13
				case 3:
					ip->Defense += 7; break;	// +12
				case 2:
					ip->Defense += 6; break;	// +11
				case 1:
					ip->Defense += 5; break;	// +10
				default:
					break;
				};
			}
			else
			{
				...
				{
				case 6:
					ip->Defense += 9; break;	// +15
				case 5:
					ip->Defense += 8; break;	// +14
				case 4:
					ip->Defense += 7; break;	// +13
				case 3:
					ip->Defense += 6; break;	// +12
				case 2:
					ip->Defense += 5; break;	// +11
				case 1:
					ip->Defense += 4; break;	// +10
				default:
					break;
				};
			}
	...
	else if (ip->Type == ITEM_HELPER + 30 || ip->Type == ITEM_WING + 49)
	{
		...
		{
		case 6:
			ip->SpecialValue[ip->SpecialNum] += 9; break;	// +15
		case 5:
			ip->SpecialValue[ip->SpecialNum] += 8; break;	// +14
		case 4:
			ip->SpecialValue[ip->SpecialNum] += 7; break;	// +13
		case 3:
			ip->SpecialValue[ip->SpecialNum] += 6; break;	// +12
		case 2:
			ip->SpecialValue[ip->SpecialNum] += 5; break;	// +11
		case 1:
			ip->SpecialValue[ip->SpecialNum] += 4; break;	// +10
		default: break;
		};
		...
		if ((excelWing >> 3) & 0x01 && (ip->Type != ITEM_WING + 49))
		{
	...
	else if (fw != NULL)
	{
		if (excelWing & 0x01)
		{
			ip->SpecialValue[ip->SpecialNum] = fw->NewOptionValue1;
			ip->Special[ip->SpecialNum] = fw->NewOptionIndex1;
			ip->SpecialNum++;
		}
		if ((excelWing >> 1) & 0x01)
		{
			ip->SpecialValue[ip->SpecialNum] = fw->NewOptionValue2;
			ip->Special[ip->SpecialNum] = fw->NewOptionIndex2;
			ip->SpecialNum++;
		}
		if ((excelWing >> 2) & 0x01)
		{
			ip->SpecialValue[ip->SpecialNum] = fw->NewOptionValue3;
			ip->Special[ip->SpecialNum] = fw->NewOptionIndex3;
			ip->SpecialNum++;
		}
		if ((excelWing >> 3) & 0x01)
		{
			ip->SpecialValue[ip->SpecialNum] = fw->NewOptionValue4;
			ip->Special[ip->SpecialNum] = fw->NewOptionIndex4;
			ip->SpecialNum++;
		}
	}
	...
	if ((Attribute1 >> 2) & 1)
	{
		...
		else if ((ip->Type >= ITEM_WING && ip->Type <= ITEM_WING + 6) || (ip->Type >= ITEM_WING + 41 && ip->Type <= ITEM_WING + 42))
		{
			ip->Special[ip->SpecialNum] = AT_LUCK; ip->SpecialNum++;
		}
		else if (ip->Type == ITEM_HELPER + 30 || ip->Type == ITEM_WING + 49 )
	...
	if (Option3)
	{
		...
		else
		{
			...
			else if (ip->Type == ITEM_WING + 40 || (ip->Type == ITEM_WING + 50) )
			{
				if ((excelWing >> 4) & 0x01)
				{
					ip->SpecialValue[ip->SpecialNum] = Option3 * 4;
			...
			else if (fw != NULL)
			{
				if ((excelWing >> 4) & 0x01)
				{
					ip->SpecialValue[ip->SpecialNum] = Option3 * fw->OptionValue1;
					ip->Special[ip->SpecialNum] = fw->OptionIndex1;
					ip->SpecialNum++;
				}
				else if ((excelWing >> 5) & 0x01)
				{
					ip->SpecialValue[ip->SpecialNum] = Option3 * fw->OptionValue2;
					ip->Special[ip->SpecialNum] = fw->OptionIndex2;
					ip->SpecialNum++;
				}
				else
				{
					ip->SpecialValue[ip->SpecialNum] = Option3 * fw->OptionValue3;
					ip->Special[ip->SpecialNum] = fw->OptionIndex3;
					ip->SpecialNum++;
				}
			}
		}
	...
	if (Weapon02Kind2 == ItemKind2::BOOK)
	...
	else if ((ip->Type >= ITEM_HELPER + 8 && ip->Type < ITEM_HELPER + 12) || (ip->Type == ITEM_HELPER + 20 && Level == 0) || (ip->Type == ITEM_HELPER + 20 && Level == 3) )
		...
		ip->Part = EQUIPMENT_AMULET;
	else if ((ip->Type >= ITEM_WING + 49 && ip->Type <= ITEM_WING + 50) || fw != NULL)
		ip->Part = EQUIPMENT_WING;
	else
		ip->Part = -1;
}
