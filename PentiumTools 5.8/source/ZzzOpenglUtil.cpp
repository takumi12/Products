...
void RenderPointRotate(int Texture, float ix, float iy, float iWidth, float iHeight, float x, float y, float Width, float Height, float Rotate, float Rotate_Loc, float uWidth, float vHeight, int Num)
{
	...
	if (Num > -1)
	{
		...
		dx = dx * (float)(GetWindowsX / WindowWidth);
...
void RenderBitmapLocalProjection(int Texture, float x, float y, float Width, float Height, vec3_t Angle, float u, float v, float uWidth, float vHeight, bool Scale)
{
	float Matrix[3][4];
	vec3_t p[4], p2[4];
	if (Scale)
