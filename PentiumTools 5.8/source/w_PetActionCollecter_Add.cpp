...
bool PetActionCollecterAdd::Effect(OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender)
{
	...
	if (rand() % 3 == 0)
	{
		...
		CreateParticletime(BITMAP_LIGHT + 3, Position, obj->Angle, Light, 1, 1.f);
	}
...
bool PetActionCollecterSkeleton::Effect(OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender)
{
	...
	if (m_bIsMoving == TRUE)
	{
		...
		for (int i = 0; i < 2; ++i)
		{
			...
			{
			case 0:
				CreateParticletime(BITMAP_FIRE_HIK1_MONO, vPosition, vAngle, vLight, 4, obj->Scale, obj);
				break;
			case 1:
				CreateParticletime(BITMAP_FIRE_HIK2_MONO, vPosition, vAngle, vLight, 8, obj->Scale, obj);
				break;
			case 2:
				CreateParticletime(BITMAP_FIRE_HIK3_MONO, vPosition, vAngle, vLight, 5, obj->Scale, obj);
				break;
			}
		}
