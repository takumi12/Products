...
bool PetActionUnicorn::Effect(OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender)
{
	...
	if (rand() % 3 == 0)
		CreateEffecttime(BITMAP_PIN_LIGHT, Position, obj->Angle, Light, 4, obj, -1, 0, 0, 0, 0.45f);
	...
	if (rand() % 2 == 0)
		CreateParticletime(BITMAP_SMOKE, Position, obj->Angle, Light, 67, 1.0f);

	Vector(0.7f, 0.7f, 1.0f, Light);
	b->TransformPosition(BoneTransform[4], vRelativePos, Position, false);
	CreateParticletime(BITMAP_SMOKELINE1, Position, obj->Angle, Light, 4, 0.6f, obj);
	CreateParticletime(BITMAP_SMOKELINE2, Position, obj->Angle, Light, 4, 0.6f, obj);
	CreateParticletime(BITMAP_SMOKELINE3, Position, obj->Angle, Light, 4, 0.6f, obj);
	return TRUE;
}
