...
bool ConfigParser::TokenizeLine(TokenLine* ptokens, UInt max)
{
	...
	do {
		...
		//pLog->LogMessage(2, "[TokenizeLine] %s", tok.GetString().ToCStr());
...
bool ConfigParser::MatchLine(GArray<Token>& tokens, const char* pformat, ...)
{
	va_list vl;
	va_start(vl, pformat);
	...
	while (*pformat != 0)
	{
		...
		case '%':
			...
			if (*(pformat + 1) == 'i')
			{
				//pLog->LogMessage(2, "[IsString] %s", tok.GetString().ToCStr());
				if (!tok.IsId(va_arg(vl, const char*)))
				{
					va_end(vl);
					return 0;
				}
			...
			else if (*(pformat + 1) == 'I')
			{
				...
				{
					va_end(vl);
					//pLog->LogMessage(2, "[i] %s", tok.GetString().ToCStr());
					return 0;
				}
				GString* pstr = va_arg(vl, GString*);
				*pstr = tok.GetString();
			...
			else if (*(pformat + 1) == 'n')
			{
				...
				{
					va_end(vl);
					//pLog->LogMessage(2, "[n] %s", tok.GetString().ToCStr());
					return 0;
				}
		...
		default:
			...
			{
				va_end(vl);
				//pLog->LogMessage(2, "[IsChar] %s", tok.GetString().ToCStr());
				return 0;
			}
		}
...
void FontConfigSet::Parse(ConfigParser* pparser)
{
	...
	//pLog->LogMessage(2, "[FontConfigSet::Parse][%s]", pparser->ParentPath.ToCStr());
	...
	while (pparser->TokenizeLine(&line, 6))
	{
		...
		// [FontConfig] - unnamed
		if (ConfigParser::MatchLine(tokens, "[%i]", "FontConfig"))
		{
		...
		// [FontConfig "English"]
		else if (ConfigParser::MatchLine(tokens, "[%i%s]", "FontConfig", &s1))
		{
			GPtr<FontConfig> pfontConfig = *new FontConfig;
		...
		{
			// Empty lines ok.
			if (ConfigParser::MatchLine(tokens, ""))
				continue;

			if (Configs.GetSize() == 0)
			{
				fprintf(stderr, "Error %d: FontConfig(%d) - file mappings outside of [FontConfig] section\n", _numerador, line.LineIndex);
				continue;
			}
			...
			// map "Arial" = "takoma"
			if (ConfigParser::MatchLine(tokens, "%i%s=%s", "map", &s1, &s2))
			{
				if (!pconfig->pFontMap)
			...
			else if (ConfigParser::MatchLine(tokens, "%i%s=%s%I", "map", &s1, &s2, &s3))
			{
				if (!pconfig->pFontMap)
					pconfig->pFontMap = *new GFxFontMap;
			...
			else if (ConfigParser::MatchLine(tokens, "%i%s=%s%p", "map", &s1, &s2, &scaleFactor))
			{
				if (!pconfig->pFontMap)
					pconfig->pFontMap = *new GFxFontMap;
			...
			else if (ConfigParser::MatchLine(tokens, "%i%s=%s%I%p", "map", &s1, &s2, &s3, &scaleFactor))
			{
				if (!pconfig->pFontMap)
					pconfig->pFontMap = *new GFxFontMap;
			...
			else if (ConfigParser::MatchLine(tokens, "%i%s=%s%I%p%P%P", "map", &s1, &s2, &s3, &scaleFactor, &offx, &offy))
			{
				if (!pconfig->pFontMap)
					pconfig->pFontMap = *new GFxFontMap;
			...
			else if (ConfigParser::MatchLine(tokens, "%i%s=%s%I%I", "map", &s1, &s2, &s3, &s4))
			{
				if (!pconfig->pFontMap)
					pconfig->pFontMap = *new GFxFontMap;
			...
			else if (ConfigParser::MatchLine(tokens, "%i%s=%s%I%I%p", "map", &s1, &s2, &s3, &s4, &scaleFactor))
			{
				if (!pconfig->pFontMap)
					pconfig->pFontMap = *new GFxFontMap;
			...
			else if (ConfigParser::MatchLine(tokens, "%i%s=%s%I%I%p%P%P", "map", &s1, &s2, &s3, &s4, &scaleFactor, &offx, &offy))
			{
				if (!pconfig->pFontMap)
					pconfig->pFontMap = *new GFxFontMap;
			...
			// fontlib "myfile.swf"
			else if (ConfigParser::MatchLine(tokens, "%i%s", "fontlib", &s1))
			{
			...
			else if (ConfigParser::MatchLine(tokens, "%i%s=%s", "tr", &s1, &s2))
			{
