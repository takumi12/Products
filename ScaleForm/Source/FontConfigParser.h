...
public:
	...
	struct Token
	{
		...
		bool        IsId(const char* pid) const
		{
			...
			//pLog->LogMessage(2, "[IsId][Type: %d][pid: %s][pid: %s]", (int)Type, s1.ToCStr(), s2.ToCStr());

			return ((Type == Tok_Id) && Length == s2.GetLength() && !s1.CompareNoCase(s2));
		}
	...
	static bool MatchLine(GArray<Token>& tokens, const char* pformat, ...);
};
