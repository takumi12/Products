...
public:
    ...
    GFxVersionType  GetGFxVersion() const                   { return GFxVersion; }

    // Compression
    virtual GFxAmpMessage*  Compress() const;
    virtual GFxAmpMessage*  Uncompress()                    { return NULL; }
...
{
public:
    GFxAmpMessageImageData(UInt32 imageId = 0);
    ...
    // Accessors
    GFxAmpStream*           GetImageData() const;
    void                    SetImageData(GFxAmpStream* imageData);
    bool                    IsPngFormat() const;
    void                    SetPngFormat(bool pngFormat);

protected:
    GFxAmpStream*           ImageData;
    bool                    PngFormat;
};

// Sent by client to server
// Base class holds the Image ID
class GFxAmpMessageFontRequest : public GFxAmpMessageInt
{
public:
    GFxAmpMessageFontRequest(UInt32 fontId = 0);
    virtual ~GFxAmpMessageFontRequest() { }

    // GFxAmpMessage overrides
    virtual bool            AcceptHandler(GFxAmpMsgHandler* handler) const;
    virtual GString         ToString() const        { return "Font request"; }
};

// Message with the contents of an image
// Sent by server to client upon request
// The base class member holds the corresponding image ID
class GFxAmpMessageFontData : public GFxAmpMessageInt
{
public:
    GFxAmpMessageFontData(UInt32 fontId = 0);
    virtual ~GFxAmpMessageFontData();

    // GFxAmpMessage overrides
    virtual bool            AcceptHandler(GFxAmpMsgHandler* handler) const;
    virtual GString         ToString() const        { return "Font data"; }
    virtual void            Read(GFxAmpStream& str);
    virtual void            Write(GFxAmpStream& str) const;
...
class GFxAmpMessageCompressed : public GFxAmpMessage
{
public:
    GFxAmpMessageCompressed();
    ~GFxAmpMessageCompressed();

    // Message overrides
    virtual GString         ToString() const        { return "Compressed message"; }
    virtual void            Read(GFxAmpStream& str);
    virtual void            Write(GFxAmpStream& str) const;

    virtual GFxAmpMessage*  Uncompress();

    // Accessors
    void                    AddCompressedData(UByte* data, UPInt dataSize);
    const GArrayLH<UByte>&  GetCompressedData() const;

private:
    GArrayLH<UByte>         CompressedData;
};
