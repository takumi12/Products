...
class GFxAmpGlyphCacheEventHandler;
...
public:
    // Singleton accessor
    static GFxAmpServer& GSTDCALL   GetInstance();

    // Singleton initialization and uninitialization
    // Called from GSystem
    static void GSTDCALL            Init(UPInt arena = 0, GSysAllocPaged* arenaAllocator = NULL);
    static void GSTDCALL            Uninit();
    ...
    bool        IsInstructionProfiling() const;
    ...
    bool        IsFunctionAggregation() const   { return !IsState(Amp_NoFunctionAggregation); }
    ...
    void        SetNoBroadcast(); // disable broadcasting before connection is opened
    ...
    bool        HandleFontRequest(const GFxAmpMessageFontRequest* message);
    ...
    void        AddSound(UPInt soundMem);
    void        RemoveSound(UPInt soundMem);
...
private:
    ...
    GPtr<GFxAmpGlyphCacheEventHandler>          FontCacheHandler;
    ...
    typedef GHashSet< GPtr<GFxImageResource> > ResourceSet;
    typedef GStringHash<ResourceSet> HeapResourceMap;
