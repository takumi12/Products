...
bool GFxDrawingContext::SameLineStyle(Float lineWidth, 
                                      UInt  rgba, 
                                      bool  hinting, 
                                      GFxLineStyle::LineStyle scaling, 
                                      GFxLineStyle::LineStyle caps,
...
{
    if (Shapes->GetNumLineStyles() == 0)
        return false;

    if (PathPacker.GetLine() == 0)
        return false;

    const GFxLineStyle& ls = Shapes->GetLastLineStyle();

    if (ls.GetComplexFill() != 0)
        return false;

#ifndef GFC_NO_FXPLAYER_STROKER
    return ls.GetColor() == rgba &&
           ls.GetWidth() == UInt(lineWidth) &&
           ls.GetStrokeHinting() == hinting &&
           ls.GetScaling() == scaling &&
           ls.GetStartCap() == caps &&
           ls.GetJoin() == joins &&
           ls.GetMiterSize() == miterLimit;
#else
    GUNUSED2(hinting, scaling);
    GUNUSED3(caps, joins, miterLimit);
    return ls.GetColor() == rgba && ls.GetWidth() == UInt(lineWidth);
#endif
}

bool GFxDrawingContext::NoLine() const
{
    return Shapes->GetNumLineStyles() == 0 || PathPacker.GetLine() == 0;
}
...
bool GFxDrawingContext::IsSolidFill(UInt rgba) const
{
    return FillStyle.IsSolidFill() && FillStyle.GetColor() == rgba;
}
