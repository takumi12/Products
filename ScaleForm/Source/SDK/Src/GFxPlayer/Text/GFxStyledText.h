...
// Paragraphs are stored and operated by the GFxStyledText class.
class GFxTextParagraph : public GNewOverrideBase<GFxStatMV_Text_Mem>
{
    friend class GFxStyledText;
    friend class GFxTextCompositionString;

    typedef GRangeDataArray<GPtr<GFxTextFormat>, GArrayLH<GRangeData<GPtr<GFxTextFormat> > > > TextFormatArrayType;
...
// with rich text. GFxTextDocView formats the styled text and the 
// GFxTextLineBuffer renders it.
class GFxStyledText : public GRefCountBaseNTS<GFxStyledText, GFxStatMV_Text_Mem>
{
...
public:
    ...
    struct HTMLImageTagInfo : public GNewOverrideBase<GFxStatMV_Text_Mem>
    {
        ...
        HTMLImageTagInfo(GMemoryHeap* pheap) : Url(pheap), Id(pheap), Width(0), Height(0), 
            VSpace(0), HSpace(0), ParaId(~0u), Alignment(Align_BaseLine) {}
    };

    typedef GArrayDH<HTMLImageTagInfo>          HTMLImageTagInfoArray;
    typedef GArrayLH<ParagraphPtrWrapper>       ParagraphArrayType;
...
private:
    ...
    {
        static int Compare(const GFxTextParagraph* pp1, UPInt index)
        {
        ...
        static int Less(const GFxTextParagraph* pp1, UPInt index)
        {
...
public:
    ...
    UInt GetParagraphsCount() const { return (UInt)Paragraphs.GetSize(); }
...
protected:
    ...
    {
        if (!pTextAllocator)
            return GMemory::GetHeapByAddress(this);
    ...
    GFxTextAllocator* GetAllocator() const
    {
        if (!pTextAllocator)
        {
            ...
            pTextAllocator = *GHEAP_NEW(pheap) GFxTextAllocator(pheap);
        }
