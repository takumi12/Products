...
void GFxTextEditorKit::Advance(Double timer)
{
...
(UPInt charIndex, GRectF* pcursorRect, UInt* plineIndex, UInt* pglyphIndex, bool avoidComposStr, 
 GFxTextLineBuffer::Line::Alignment* plineAlignment)
{
    GASSERT(pcursorRect);    
    ...
    if (lineIndex != ~0u)
    {
        ...
        pcursorRect->Top  = Float(line.GetOffsetY());
...
bool GFxTextEditorKit::ScrollToPosition(UPInt pos, bool avoidComposStr, bool wideCursor)
{
    ...
    UInt lineIndex;
    bool rv = false;
    GFxTextLineBuffer::Line::Alignment lineAlignment;
    ...
    if (CalcCursorRectOnScreen(pos, &newCursorRect, &lineIndex, NULL, avoidComposStr, &lineAlignment))
    {
        ...
        if (!textRect.Contains(newCursorRect) && pDocView->GetTextAutoSize() == GFxTextDocView::TAS_None)
        {
            ...
            SInt delta = GFX_EDIT_HSCROLL_DELTA;
            if (lineAlignment != GFxTextLineBuffer::Line::Align_Left)
                delta = 0;
            if (newCursorRect.Right > textRect.Right)
            {
                // make sure whole cursor is visible.
                hscroll += SInt(newCursorRect.Left - textRect.Right + delta);

                GRectF lastChar;
                if (pos > 0 && pDocView->GetExactCharBoundaries(&lastChar, pos - 1))
                    hscroll = G_Min((SInt)(lastChar.Left - GFX_TEXT_GUTTER), hscroll);
                hscroll = G_Max(0, hscroll);
            }
            else if (newCursorRect.Left < textRect.Left)
            {
                hscroll -= SInt(textRect.Left - newCursorRect.Left) + delta;
                hscroll = G_Max(0, hscroll);
            }
            GASSERT(hscroll >= 0);
        ...
        if (pos != GFC_MAX_UPINT)
        {
            ...
            if (IsSelectable() && !IsMouseCaptured())
            {
                ...
                if (doubleClicked || tripleClick)
                {
                    ...
                    UPInt indexInPara;
                    GFxStyledText::ParagraphsIterator it = pDocView->GetStyledText()->GetParagraphByIndex(pos, &indexInPara);
                    if (!it.IsFinished())
                    {
                        GFxTextParagraph* ppara = *it;
                        ...
                        if (doubleClicked)
                        {
                            ...
                            for (; st > 0; --st)
                            {
                                if (G_iswspace(ptext[st - 1]) || !G_iswalnum(ptext[st - 1]))
                                    break;
                            }
                            for (; en < textLen; ++en)
                            {
                                if (G_iswspace(ptext[en]) || !G_iswalnum(ptext[en]))
                                    break;
                            }
                        }
                        else
                    ...
                    if (!IsReadOnly())
                    {
                        if (pDocView->IsMultiline())
                        {
                            ...
                            else
                            {
                                ...
                                newPos = G_PMin(beginSel, endSel) + 1;
                                needChangeNotify = true;
                            }
                        }
                    }
        ...
        if (wc >= 32)
        {
            ...
            //if (wc)
            {
                ...
                else
                {
                    ...
                    newPos = G_PMin(beginSel, endSel) + res;
                    needChangeNotify = true;
                }
            }
...
void GFxTextEditorKit::InvalidateSelectionColors()
{
...
{
    pComposStr = *GHEAP_AUTO_NEW(this) GFxTextCompositionString(this);
    return pComposStr;
}
...
{
    GFxTextFormat fmt(pAllocator->GetHeap()); // create empty text format
...
#endif //#ifndef GFC_NO_IME_SUPPORT
#endif //#ifndef GFC_NO_TEXT_INPUT_SUPPORT
