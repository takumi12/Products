...
#ifndef INC_GHeapBitSet1_H
#define INC_GHeapBitSet1_H
...
public:
    ...
    //--------------------------------------------------------------------
    static UInt GetValue(const UInt32* buf, UPInt num)
    {
        return (buf[num >> 5] >> (num & 31)) & 1;
    }

    static UInt SetBit(UInt32* buf, UPInt num)
    {
        return buf[num >> 5] |= UInt32(1) << (num & 31);
    }

    static UInt ClrBit(UInt32* buf, UPInt num)
    {
        return buf[num >> 5] &= ~(UInt32(1) << (num & 31));
    }
