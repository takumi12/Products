...
{   
    ...
    struct EventPoolEntry  : public GNewOverrideBase<GStat_Default_Mem>
    {
        HANDLE          hEvent;
...
class GThreadList : public GNewOverrideBase<GStat_Default_Mem>
{
    ...
    struct ThreadHashOp
    {
        UPInt operator()(const GThread* ptr)
        {
...
int GThread::GetOSPriority(ThreadPriority p)
{
    switch(p)
    {
    case GThread::CriticalPriority:      return THREAD_PRIORITY_TIME_CRITICAL;
    case GThread::HighestPriority:       return THREAD_PRIORITY_HIGHEST;
    case GThread::AboveNormalPriority:   return THREAD_PRIORITY_ABOVE_NORMAL;
    case GThread::NormalPriority:        return THREAD_PRIORITY_NORMAL;
    case GThread::BelowNormalPriority:   return THREAD_PRIORITY_BELOW_NORMAL;
    case GThread::LowestPriority:        return THREAD_PRIORITY_LOWEST;
    case GThread::IdlePriority:          return THREAD_PRIORITY_IDLE;
    }
...
unsigned WINAPI GThread_Win32StartFn(void * phandle)
{
    ...
    BOOL ret = ::SetThreadPriority(GetCurrentThread(), GThread::GetOSPriority(pthread->Priority));
    GFC_DEBUG_ERROR(ret == 0, "Could not set thread priority");
    GUNUSED(ret);
