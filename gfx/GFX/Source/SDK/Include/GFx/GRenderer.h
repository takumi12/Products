...
public:
    ...
    virtual bool        LoadFromImage(GImageBase*) { return false; }
...
public:
    ...
    virtual void        DrawBlurRect(GTexture* psrcin, const GRectF& insrcrect, const GRectF& indestrect, const BlurFilterParams& params, bool islast = false) = 0;
    virtual void        DrawColorMatrixRect(GTexture* psrcin, const GRectF& insrcrect, const GRectF& destrect, const Float *matrix, bool islast = false) = 0;

    // Optional API for filters. Ability to disable filters; Callbacks to allow the renderer
    // to save off/restore the contents of the current render target.
    void                SetForceDisabledFilters(bool disable) { GUNUSED(disable); }	
