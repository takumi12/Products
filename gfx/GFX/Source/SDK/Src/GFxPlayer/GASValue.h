...
struct GASValueProperty : public GASRefCountBase<GASValueProperty>
{
    GASFunctionRef              GetterMethod;
    GASFunctionRef              SetterMethod;

    GASValueProperty(GASRefCountCollector* pCC,
                     const GASFunctionRef& getterMethod, 
                     const GASFunctionRef& setterMethod);
    ...
    {
        GASRefCountBaseType::CallForEachChild<GASValueProperty>(prcc, operation);
    }
    void Finalize_GC() 
    {
...
public:
    ...
    # undef new
    #endif
        GFC_MEMORY_REDEFINE_NEW(GASValue, GFxStatMV_ActionScript_Mem)
    #if defined(GFC_BUILD_DEFINE_NEW) && defined(GFC_DEFINE_NEW)
...
//        UInt32          Dummy;
    ...
    GASValue(type typeVal, const GASString& str)
    {
        T.Type = (UByte)typeVal;
    ...
    GASValue(GASStringContext* psc, GASCFunctionPtr func);
    GASValue(const GASFunctionRef& func);
    GASValue(const GASFunctionRef& getter, 
...
#ifndef GFC_NO_GC
    typedef GRefCountCollector<GFxStatMV_ActionScript_Mem> Collector;
    template <class Functor>
    void ForEachChild_GC(Collector* prcc) const;
    void Finalize_GC();
#endif // GFC_NO_GC
    ...
    GINLINE GASString   ToString(GASEnvironment* penv, int precision = -1) const
    {
    ...
    GASObject*          ToObject(const GASEnvironment* penv) const;
    ...
    // if function name resolving may occur; in other cases NULL may be used.
    GASFunctionRef      ToFunction(const GASEnvironment* penv) const;
    GASFunctionRef      ToResolveHandler() const;
    ...
    void    SetAsResolveHandler(const GASFunctionRefBase& func);
    ...
    void    Add (GASEnvironment* penv, int v2);
    void    Sub (GASEnvironment* penv, int v2)   { SetNumber(ToNumber(penv) - (GASNumber)v2); }
    ...
    void    Div (GASEnvironment* penv, const GASValue& v);   
...
protected:
    GASFunctionRef ResolveFunctionName(const GASEnvironment* penv) const;
};

// This class might be used to preserve GASValue members from deletion.
// This is mostly actual for preserving characters from deletion, since the
// GASValue stores only GFxCharHandle and the character itself might be freed.
// This class stores addrefed GFxASCharacter* if the character is stored in GASValue.
class GASValueGuard
{
    const GASEnvironment*   pEnv;
    GASValue                Value;
    GFxASCharacter*         pChar;
public:
    explicit GASValueGuard(const GASEnvironment* penv) : pEnv(penv), pChar(NULL) { GASSERT(pEnv); }
    GASValueGuard(const GASEnvironment* penv, const GASValue& val);
    ~GASValueGuard();

    GASValueGuard& operator=(const GASValue& val);
};
...
#define CHECK_THIS_PTR(fn,classname)\
    do {\
    if (!fn.CheckThisPtr(GASObjectInterface::Object_##classname)) \
        { fn.ThisPtrError(#classname, __FILE__, __LINE__); return; } \
    } while(0)
#define CHECK_THIS_PTR2(fn,classname1,classname2)\
    do {\
        bool ctptr1 = fn.CheckThisPtr(GASObjectInterface::Object_##classname1); \
        bool ctptr2 = fn.CheckThisPtr(GASObjectInterface::Object_##classname2); \
        if (!ctptr1 && !ctptr2) { \
