...
class GFxEdgeAAGenerator
{
    ...
    bool    SetVertexColor(const GEdgeAA::VertexType &v,
                           GFxVertexRef& cv,
    ...
    static bool isTexture(UInt id)
    { return (id & (StyleTextureBit | TransparencyBit)) == StyleTextureBit; }

    static bool isTransparent(UInt id)
    { return (id & TransparencyBit) != 0; }

    static bool stylesEq(int s1, int s2)
    { return (s1 & StyleMask) == (s2 & StyleMask); }

public:
    enum 
    {
        TransparencyBit = GEdgeAA::TransparencyBit,
        StyleTextureBit = GEdgeAA::StyleTextureBit,
        StyleMask       = GEdgeAA::StyleMask
    };
    ...
    UPInt   GetNumBytes() const { return EdgeAA.GetNumBytes(); }
    void    ClearAndRelease()   { EdgeAA.ClearAndRelease(); }

    // Processes & sorts edges if necessary
    void    ProcessAndSortEdges(GTessellator &tess, 
                                const GFxFillStyle *pfills, UPInt fillsNum, 
    ...
    UInt    GenerateTexturedMeshes(GFxVertexArray *pvertices, 
