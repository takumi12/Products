...
//------------------------------------------------------------------------
void GFxGlyphFitter::ClearAndRelease()
{ 
...
void GFxGlyphFitter::MoveTo(int x, int y)
{
    ...
    VertexType  v;
    c.NumVertices = 1;
    c.StartVertex = (UInt)Vertices.GetSize();
    v.x = SInt16(x);
    v.y = SInt16(y);
    Contours.PushBack(c);
    Vertices.PushBack(v);
...
void GFxGlyphFitter::detectEvents(FitDir dir)
{
    ...
    for(i = 0; i < (UInt)Contours.GetSize(); ++i)
    {
        const ContourType& c = Contours[i];
        if(c.NumVertices > 2)
        {
                                     ...
                                     int lowerCaseTop, int upperCaseTop)
{
    ...
    GArrayUnsafeLH_POD<SInt16>& ramp = (dir == FitX) ? LerpRampX : LerpRampY;
