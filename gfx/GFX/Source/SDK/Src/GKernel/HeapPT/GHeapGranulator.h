...
#ifndef INC_GHeapGranulator_H
#define INC_GHeapGranulator_H
...
// less than GHeap_PageSize.
//------------------------------------------------------------------------


//------------------------------------------------------------------------
struct GHeapHdrPage : GListNode<GHeapHdrPage>
{
    UPInt        UseCount;
    UPInt        Filler[5]; // Just for better alignment to sizeof(GHeapTreeSeg)
//  GHeapTreeSeg Segments[PageCapacity];
};
...
public:
    ...
    //--------------------------------------------------------------------
    typedef GList<GHeapHdrPage>                     HdrListType;
    typedef GList2<Segment, SegListAccessor>        SegListType;
    typedef GRadixTree<Segment, SegTreeAccessor>    SegTreeType;


public:
    // Public interface functions
    //--------------------------------------------------------------------
    GHeapGranulator(GSysAllocPaged* sysAlloc, 
                    UPInt minSize     = 256,
                    UPInt granularity = 64*1024,
                    UPInt hdrPageSize = GHeap_PageSize);
    ...
    const GHeapTreeSeg* GetAllocSegment(const void* ptr) const;

    void    VisitMem(GHeapMemVisitor* visitor, 
                     GHeapMemVisitor::Category catSegm,
                     GHeapMemVisitor::Category catFree) const;

    void    VisitSegments(GHeapSegVisitor* visitor, UInt catSeg, UInt catUnused) const;
...
private:
    ...
    { 
        return (UByte*)seg->HdrPage + HdrPageSize == seg->Buffer; 
    }
    ...
    void visitSegments(const Segment* node, GHeapSegVisitor* visitor, UInt cat) const;
