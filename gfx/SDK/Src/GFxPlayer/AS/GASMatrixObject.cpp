...
void GASMatrixProto::Clone(const GASFnCall& fn)
{
    ...
    GPtr<GASMatrixObject> m = *GHEAP_NEW(fn.Env->GetHeap()) GASMatrixObject(fn.Env);
...
void GASMatrixProto::Concat(const GASFnCall& fn)
{
    ...
    if (fn.NArgs > 0)
    {
        GASObject* arg = fn.Arg(0).ToObject(fn.Env);
...
void GASMatrixProto::TransformPoint(const GASFnCall& fn)
{
    ...
    if (fn.NArgs > 0 && (arg = fn.Arg(0).ToObject(fn.Env)) != NULL &&
       arg->GetObjectType() == GASObjectInterface::Object_Point)
    {
        GMatrix2D m = pthis->GetMatrix(fn.Env);
        GASValue ptprop[GFxPoint_NumProperties];
        ((GASPointObject*)arg)->GetProperties(fn.Env->GetSC(), ptprop);

        GPtr<GASPointObject> ppt = *GHEAP_NEW(fn.Env->GetHeap()) GASPointObject(fn.Env);
...
#else
    GFC_DEBUG_WARNING(1, "Point ActionScript class was not included in this GFx build.");
    fn.Result->SetUndefined();
...
GASObject* GASMatrixCtorFunction::CreateNewObject(GASEnvironment* penv) const 
{
    return GHEAP_NEW(penv->GetHeap()) GASMatrixObject(penv);
}


void GASMatrixCtorFunction::GlobalCtor(const GASFnCall& fn)
{
    GPtr<GASMatrixObject> pmatrix;
    if (fn.ThisPtr && fn.ThisPtr->GetObjectType() == GASObject::Object_Matrix && 
        !fn.ThisPtr->IsBuiltinPrototype())
        pmatrix = static_cast<GASMatrixObject*>(fn.ThisPtr);
    else
        pmatrix = *GHEAP_NEW(fn.Env->GetHeap()) GASMatrixObject(fn.Env);
...
GASFunctionRef GASMatrixCtorFunction::Register(GASGlobalContext* pgc)
{
    GASStringContext sc(pgc, 8);
    GASFunctionRef  ctor(*GHEAP_NEW(pgc->GetHeap()) GASMatrixCtorFunction(&sc));
    GPtr<GASObject> proto = *GHEAP_NEW(pgc->GetHeap()) GASMatrixProto(&sc, pgc->GetPrototype(GASBuiltin_Object), ctor);
    pgc->SetPrototype(GASBuiltin_Matrix, proto);
    pgc->FlashGeomPackage->SetMemberRaw(&sc, pgc->GetBuiltin(GASBuiltin_Matrix), GASValue(ctor));
    return ctor;
}
