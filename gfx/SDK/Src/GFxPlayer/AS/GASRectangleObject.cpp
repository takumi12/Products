...
{
    GPtr<GASPointObject> gpt = *GHEAP_NEW(penv->GetHeap()) GASPointObject(penv);
...
void GASRectangleProto::Clone(const GASFnCall &fn)
{
    ...
    GASSERT(p_this);
    GPtr<GASRectangleObject> p_clone = *GHEAP_NEW(fn.Env->GetHeap()) GASRectangleObject(fn.Env);
    GASValue params[GFxRectangle_NumProperties];
...
void GASRectangleProto::Intersection(const GASFnCall& fn)
{
    ...
    GPtr<GASRectangleObject> obj = *GHEAP_NEW(fn.Env->GetHeap()) GASRectangleObject(fn.Env);
...
//
void GASRectangleCtorFunction::GlobalCtor(const GASFnCall& fn)
{
    GPtr<GASRectangleObject> prect;
    if (fn.ThisPtr && fn.ThisPtr->GetObjectType() == GASObject::Object_Rectangle && 
        !fn.ThisPtr->IsBuiltinPrototype())
        prect = static_cast<GASRectangleObject*>(fn.ThisPtr);
    else
        prect = *GHEAP_NEW(fn.Env->GetHeap()) GASRectangleObject(fn.Env);
...
GASObject* GASRectangleCtorFunction::CreateNewObject(GASEnvironment* penv) const 
{
    return GHEAP_NEW(penv->GetHeap()) GASRectangleObject(penv);
}

GASFunctionRef GASRectangleCtorFunction::Register(GASGlobalContext* pgc)
{
    GASStringContext sc(pgc, 8);
    GASFunctionRef  rectCtor(*GHEAP_NEW(pgc->GetHeap()) GASRectangleCtorFunction(&sc));
    GPtr<GASObject> rectProto = *GHEAP_NEW(pgc->GetHeap()) GASRectangleProto(&sc, pgc->GetPrototype(GASBuiltin_Object), rectCtor);
    pgc->SetPrototype(GASBuiltin_Rectangle, rectProto);
    pgc->FlashGeomPackage->SetMemberRaw(&sc, pgc->GetBuiltin(GASBuiltin_Rectangle), GASValue(rectCtor));
    return rectCtor;
}
