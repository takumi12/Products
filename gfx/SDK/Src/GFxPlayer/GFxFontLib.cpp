...
{    
    ...
    else if (pweakLib)
    {
        ...
        GFxLoader loader(pstates->GetFileOpener(), pstates->GetZlibSupport(), pstates->GetJpegSupport());
        loader.SetResourceLib(pweakLib->GetResourceLib());

        GPtr<GFxMovieDefBindStates> bs = *new GFxMovieDefBindStates(pstates);
        GPtr<GFxLoadStates> pls = *new GFxLoadStates(loader.GetLoaderImpl(), pstates, bs);    

        // Get movieDef. Most of the time this will just look up MovieDefImpl
        // in the library without performing the actual binding.
        pfontDefImpl = 
            *loader.GetLoaderImpl()->CreateMovie_LoadState(pls, pdataDef, GFxLoader::LoadWaitCompletion);
    }
    if (!pfontDefImpl)
        return 0;
