...
// an instance of GFxStyledText as a storage of rich text data, GFxTextLineBuffer
// instance to perform rendering of the formatted text and the GFxTextEditorKit
// to perform editing, GFxTextHighlight to perform highlighting. 
// This class contains functionality for formatting rich text.
class GFxTextDocView : public GRefCountBaseNTS<GFxTextDocView, GFxStatMV_Text_Mem>
{
...
public:
    typedef GHashIdentityDH<const GFxTextFormat*, GPtr<GFxFontHandle>, GFxStatMV_Text_Mem> FontCache;
public:
    // editor's commands
    enum CommandType
    {
    ...
    class DocumentListener : public GRefCountBaseNTS<DocumentListener, GFxStatMV_Text_Mem>
    {
    protected:
        enum
        {
    ...
    public:
        ...
        virtual GString GetCharacterPath() { return GString(); }
    };
    ...
    struct ImageSubstitutor : public GNewOverrideBase<GFxStatMV_Text_Mem>
    {
        struct Element
        {
...
protected:
    ...
    GFxFontHandle* FindFont(FindFontInfo* pfontInfo, bool quietMode);
...
public:
    ...
    GFxTextDocView(GFxTextAllocator* pallocator, GFxFontManager* pfontMgr, GFxLog* plog);
    ~GFxTextDocView();

    GMemoryHeap* GetHeap() const { return pDocument->GetHeap(); }

    UPInt GetLength() const { return pDocument->GetLength(); }
    GString GetText() const;
    GString GetHtml() const;
    ...
    void ParseHtml(const GString& str, bool condenseWhite, 
        GFxStyledText::HTMLImageTagInfoArray* pimgInfoArr = NULL, const GFxTextStyleManager* pstyleMgr = NULL,
        const GFxTextFormat* defTxtFmt = NULL, const GFxTextParagraphFormat* defParaFmt = NULL)
    {
    ...
    void AppendHtml(const wchar_t* pwStr, UPInt strLen = GFC_MAX_UPINT, bool condenseWhite = false, GFxStyledText::HTMLImageTagInfoArray* pimgInfoArr = NULL);
    void AppendHtml(const char* putf8Str, UPInt utf8Len = GFC_MAX_UPINT, bool condenseWhite = false, GFxStyledText::HTMLImageTagInfoArray* pimgInfoArr = NULL);
    ...
    void  SetOutline(UInt v) { Outline = UInt8(v); }
    UInt  GetOutline() const { return Outline; }
    ...
    void  SetShadowAlpha(Float a) { Filter.ShadowAlpha = UByte(G_Clamp(a * 255.0f, 0.0f, 255.0f)); Filter.ShadowColor.SetAlpha(Filter.ShadowAlpha); }
...
protected:
    ...
    UPInt GetBeginSelection() const      { return G_PMin(BeginSelection, EndSelection); }
    UPInt GetEndSelection()   const      { return G_PMax(BeginSelection, EndSelection); }
...
protected:
    ...
    struct HighlightDesc : public GNewOverrideBase<GFxStatMV_Text_Mem>
    {
