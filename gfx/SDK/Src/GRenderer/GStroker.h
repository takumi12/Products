...
{
public:
    enum { SID = GStatRG_Stroker_Mem };

    typedef GArrayPagedLH_POD<GStrokeVertexType, 6, 16, SID> ContainerType;

    GStrokePath() : Closed(false) {}

    UPInt GetNumBytes() const
        { return Path.GetNumBytes(); }

    void Clear()
        { Path.Clear(); Closed = false; }

    void ClearAndRelease()
        { Path.ClearAndRelease(); Closed = false; }

    void AddVertex(const GStrokeVertexType& v);

    void ClosePath(bool forceClosing);

    UPInt GetNumVertices() const { return Path.GetSize(); }

    const GStrokeVertexType& GetVertexPrev(UPInt i) const
    {
        if(i == 0) i += Path.GetSize();
        return Path[i - 1];
    }

    const GStrokeVertexType& GetVertex(UPInt i) const
    {
        return Path[i];
    }

    GStrokeVertexType& GetVertex(UPInt i)
    {
        return Path[i];
    }

    const GStrokeVertexType& GetVertexNext(UPInt i) const
    {
        if(++i >= Path.GetSize()) i -= Path.GetSize();
        return Path[i];
    }

    const GStrokeVertexType& GetVertexForw(UPInt i) const
    {
        i += 2;
        if(i >= Path.GetSize()) i -= Path.GetSize();
        return Path[i];
...
GINLINE void GStrokePath::AddVertex(const GStrokeVertexType& v)
{
    if(Path.GetSize() > 1)
    {
        if(!Path[Path.GetSize()-2].CalcDistance(Path[Path.GetSize()-1]))
        {
            Path.PopBack();
        }
    }
...
public:
    ...
    UPInt   GetNumBytes() const { return Path.GetNumBytes(); }
    void    Clear()             { Path.Clear(); }
    void    ClearAndRelease()   { Path.ClearAndRelease(); }
